...
- [ ] Test rollback: `nx run server:migration:revert`
...
- [ ] Add npm dependency: `@google-cloud/storage`
- [ ] Create `providers/gcs.provider.ts`
- [ ] Implement all StorageService methods
- [ ] Support credentials from file or environment

### 3.5 Create Storage Provider Factory

- [ ] Create provider factory based on STORAGE_PROVIDER env
- [ ] Support "minio", "gcs", "s3" values
- [ ] Log which provider is being used on startup
- [ ] Throw clear error if invalid provider
...
- [ ] Create DocumentParsingJob record
...
- [ ] Create `GET /document-parsing-jobs/:id` endpoint
- [ ] Return job status, progress, errors
- [ ] Include document ID when completed
...
- [ ] Test PDF extraction end-to-end (requires Kreuzberg running)
- [ ] Test DOCX extraction end-to-end
...
- [ ] Test upload PDF → parse → create document flow
- [ ] Test upload DOCX → parse → create document flow
- [ ] Test upload with autoExtract=true chaining
- [ ] Test job status polling during processing
- [ ] Test download original file via signed URL
...
- [ ] Add kreuzberg to main service list
- [ ] Document relationship with emergent application

### 8.2 Update emergent Application Docs

- [ ] Add storage configuration to `.env.example`
- [ ] Document Kreuzberg integration in API docs
- [ ] Document supported file formats

### 8.3 Workspace Integration

- [ ] Add Kreuzberg to workspace health check
- [ ] Add MinIO to workspace health check
- [ ] Add Kreuzberg logs to logs MCP server aliases
- [ ] Add MinIO logs to logs MCP server aliases
- [ ] Update `workspace_list_containers` output

### 8.4 Monitoring

- [ ] Add Kreuzberg service to health dashboard
- [ ] Add MinIO service to health dashboard
...
- [ ] `emergent-infra/kreuzberg/` directory structure created
- [ ] `docker compose up` starts Kreuzberg and MinIO successfully
- [ ] `./scripts/health-check.sh` passes all checks
- [ ] MinIO console accessible at http://localhost:9001
- [ ] PDF upload extracts text correctly
- [ ] DOCX upload extracts text correctly
- [ ] Plain text files bypass Kreuzberg (direct storage)
- [ ] Original files stored in MinIO documents bucket
- [ ] Signed URLs work for file downloads
- [ ] Failed extractions retry with backoff
- [ ] KREUZBERG_ENABLED=false disables parsing gracefully
- [ ] Document deletion cleans up storage files
...
- [ ] Average PDF processing time < 10 seconds (for <5MB files)
- [ ] Storage upload latency < 2 seconds
- [ ] Kreuzberg service uptime > 99%
- [ ] MinIO service uptime > 99%
- [ ] Zero data loss during extraction or storage failures
