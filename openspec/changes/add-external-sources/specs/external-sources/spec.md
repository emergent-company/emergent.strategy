...
The External Sources module provides a pluggable framework for importing, tracking, and synchronizing documents from external systems (Google Drive, Dropbox, URLs, etc.). It enables both conversational imports via chat and programmatic imports via API/webhooks, with support for periodic sync and retry mechanisms.
