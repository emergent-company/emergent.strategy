...
- [ ] 2.3 Document authentication setup in API e2e tests (authHeader) and browser e2e tests (Playwright fixtures)
- [ ] 2.4 Document database configuration in API e2e tests
...
- [ ] 3.4 Standardize API e2e test setup using createE2EContext()
- [ ] 3.5 Standardize authentication in API tests using authHeader() helper
- [ ] 3.6 Standardize authentication in browser tests using Playwright fixtures
- [ ] 3.7 Clean up unused mocks and imports (e.g., EmbeddingsMock in chat.service.spec.ts)
- [ ] 3.8 Fix test file imports (e.g., auto-extraction-flow.e2e-spec.ts module not found error)

## 4. Test Organization and Folder Structure Standardization

### Server App Migration

- [ ] 4.1 Audit all test file locations in apps/server (test/, tests/, src/**/**tests**/, src/**/\*.spec.ts)
- [ ] 4.2 Create new semantic directory structure for server: tests/unit/, tests/e2e/, tests/integration/
- [ ] 4.3 Create subdirectories in tests/unit/ mirroring source structure (auth/, graph/, chat/, etc.)
- [ ] 4.4 Move all unit tests from old tests/ to tests/unit/ with proper subdirectories
- [ ] 4.5 Move all e2e tests from old test/ to tests/e2e/
- [ ] 4.6 Move all integration tests to tests/integration/
- [ ] 4.7 Move all tests from src/modules/_/**tests**/ to tests/unit/_/
- [ ] 4.8 Move all .spec.ts files from src/modules/_/ to tests/unit/_/
- [ ] 4.9 Organize helpers: identify truly shared utilities
- [ ] 4.10 Create tests/helpers/ for shared utilities (if any genuinely shared)
- [ ] 4.11 Create tests/unit/helpers/ for unit-specific utilities
- [ ] 4.12 Create tests/e2e/helpers/ and move createE2EContext, authHeader, etc.
- [ ] 4.13 Create tests/integration/helpers/ if needed
- [ ] 4.14 Update all test file imports after moving files
- [ ] 4.15 Update vitest.config.ts to point to tests/unit/\*_/_.spec.ts
- [ ] 4.16 Update e2e test configuration to point to tests/e2e/\*_/_.e2e-spec.ts
- [ ] 4.17 Update integration test configuration to point to tests/integration/\*_/_.integration.spec.ts
- [ ] 4.18 Delete empty **tests**/ directories after migration
- [ ] 4.19 Delete old test/ directory after migration
- [ ] 4.20 Delete old tests/ directory after migration (once all moved to tests/unit/)
- [ ] 4.21 Verify all unit tests run correctly: nx test server
- [ ] 4.22 Verify all e2e tests run correctly: nx test-e2e server
...
- [ ] 5.1 Document createE2EContext() helper usage and configuration (API e2e)
- [ ] 5.2 Document authHeader() helper usage and scope patterns (API e2e)
- [ ] 5.3 Document Playwright fixtures and auth flow (browser e2e)
- [ ] 5.4 Create reusable mock factories for common dependencies
- [ ] 5.5 Document Vitest configuration (vitest.config.ts)
- [ ] 5.6 Document Playwright configuration (playwright.config.ts)
- [ ] 5.7 Document test scripts in package.json
