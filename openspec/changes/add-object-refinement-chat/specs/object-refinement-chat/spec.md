...
Enable users to refine knowledge graph objects through natural language conversation with LLM assistance. Users can request changes like "make description more compact", "rename to X", or "add parent relationship to Y", and the LLM suggests specific changes with diffs that users can confirm or reject.
