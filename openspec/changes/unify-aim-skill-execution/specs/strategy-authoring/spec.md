...
Skill-executor-generated batches (from `skillexec.Executor.Run` or `RunChunked`) follow
the same staging contract as manually created batches. The executor stages mutations with
`status='staged'` and returns the `batch_id`. All AIM cycle writing steps (assessment,
calibration, adapt-strategy, adapt-foundations) produce batches through the skill executor,
giving them uniform run tracking, token accounting, and schema validation.
...
#### Scenario: Skill executor stages batch with run tracking
- **WHEN** the skill executor runs a skill (e.g. `draft-assessment`) autonomously
- **AND** the LLM returns valid output
- **THEN** one or more `strategy_mutation` rows are created with `status='staged'`
- **AND** a `batch_id` is returned in the `SkillResult`
- **AND** a `skill_runs` row is created with skill name, token counts, and duration
- **AND** the staged content is visible via the draft review UI and `list_pending_batches`
- **AND** the mutations follow the same commit/discard lifecycle as any other batch
