...
- [ ] 6.6 Write tests: auto-commit creates mutation, mutation visible in list, audit entry created
...
- [ ] 8.6 Write integration test: commit a North Star change -> convergence loop runs -> trivial downstream fixes auto-committed -> equilibrium reached
...
- [ ] 9.3 Write tests for both config tools
...
- [ ] 10.4 Write tests for equilibrium status and convergence history tools
...
- [ ] 11.4 Write routing test cases for new intents
...
- [ ] 8b.9 Write tests: convergence reaches equilibrium with changes -> version auto-published with correct metadata
- [ ] 8b.10 Write tests: convergence stopped by damping -> no version published
- [ ] 8b.11 Write tests: convergence no-op -> no version published
- [ ] 8b.12 Write tests: manual publish after auto-publish -> supersedes correctly
- [ ] 8b.13 Write tests: restore_version targeting auto-published version -> restores correctly
...
- [ ] 12.1 Write integration test: convergence loop with circular dependency graph -> verify max depth damping fires
- [ ] 12.2 Write integration test: many small auto-commits -> verify change budget damping fires
- [ ] 12.3 Write integration test: auto-commit that transitively shifts North Star -> verify anchor drift damping fires
- [ ] 12.4 Write integration test: positive feedback loop (each resolution creates 2 new signals) -> verify emergency brake fires
- [ ] 12.5 Write integration test: verify autonomous commits are visible in `list_mutations` with `source='ripple_auto'`
- [ ] 12.6 Write integration test: verify `restore_version` correctly reverts auto-committed mutations
...
- [ ] 13.2 Write integration test: Memory becomes available mid-session -> verify semantic classification activates
...
- [ ] 14.2 Add ripple configuration examples to AGENTS.md (default config, product-led growth config, enterprise config)
