...
7. **Update OpenAPI documentation** (if API changes) - Regenerate spec and update golden contracts after adding/modifying endpoints
8. **For tests: Verify with DevTools MCP first** - Before writing E2E/integration tests, manually verify functionality and gather selectors
9. **Run post-implementation verification** - Execute build → lint → tests (all must pass before marking complete)
...
#### Step 7: Update OpenAPI Documentation (After API Changes)

When implementing changes that add, modify, or remove API endpoints:

**1. Add @Scopes() decorator to new/modified endpoints:**
...
- Requires database to be running (start with `cd docker && docker compose up -d db`)
- Generates updated `openapi.json` with new endpoints and scopes

**3. Update golden scope contract tests:**

- File: `apps/server/tests/unit/openapi/openapi-scope-golden-full.spec.ts`
- Add entries for new endpoints in the `EXPECTED` object
- Format: `'<method> <path>': ['scope1', 'scope2']`
- Keep entries in alphabetical order by method then path
- Example:
  ```typescript
  'get /example': ['resource:read'],
  'post /example': ['resource:write'],
  ```

**4. Update OpenAPI regression hash:**

- File: `apps/server/tests/unit/openapi/openapi-regression.spec.ts`
- Run tests once to get new hash value (will fail with computed hash in error message)
- Update `EXPECTED_HASH` constant with the new hash
- Add changelog comment documenting what changed
- Example:
  ```typescript
  // Updated after adding /example endpoint (November 2025)
  const EXPECTED_HASH = 'new-hash-value-here';
  ```

**5. Update OpenAPI snapshot (for E2E tests):**

```bash
cd apps/server
UPDATE_OPENAPI_SNAPSHOT=1 npx vitest run -c vitest.e2e.config.ts tests/e2e/openapi.snapshot-diff.e2e.spec.ts
```

**When to skip this step:**

- No API endpoints added, modified, or removed
- Only internal implementation changes
- Only frontend/UI changes

**Why this matters:**

- Prevents OpenAPI contract drift
- Ensures security scopes are documented
- Catches unintended API surface changes
- Maintains test reliability

#### Step 8: Manual Testing with DevTools MCP (Before Writing Tests)
...
#### Step 9: Post-Implementation Verification (After Implementation)
