...
- Doesn't work properly in Coolify (path context issues)
...
DB_LOGS_PATH=db_logs              # Use named volume (recommended for Coolify)
POSTGRES_LOG_STATEMENT=none       # Production default (no verbose logging)
```

## Configuration Options

### For Coolify Deployment (Recommended)

The default path is already set for Coolify: `/home/emergent/db_logs`
...
2. ✅ **Works in Coolify**: No path context problems
...
2. **In Coolify UI** (optional - uses default if not set):
