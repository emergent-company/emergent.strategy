...
const assignments = await this.templatePacks.getProjectTemplatePacks(projectId, orgId);
...
for (const job of jobs) {
  await this.db.runWithTenantContext(job.org_id, null, async () => {
    ...
    await this.db.query(
      'UPDATE kb.jobs SET status = $1 WHERE id = $2',
      ['failed', job.id]
    );
  });
}
...
**Last Updated**: November 8, 2025 (post-Session 20)  
**Next Review**: After Session 25 or Phase 3 completion
