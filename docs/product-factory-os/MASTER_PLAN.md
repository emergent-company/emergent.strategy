# **ProductFactoryOS: The Master Plan**
...
**ProductFactoryOS (PFOS)** is a "Local-First Venture Compiler." It shifts the paradigm of company building from "Document Management" to **"Software Engineering."**
...
2. **One Universal Writer:** **OpenCode** writes *all* files (Strategy, Ops, Code), assisted by the **epf-cli** linter.  
...
### **3.2 The Local OS: product-factory-os (The Cockpit)**
...
### **3.4 The Web OS: product-factory-os \--server**
...
* **Interface:** PFOS TUI \+ OpenCode.  
* **Mechanism:**  
  1. User prompts: *"Update Strategy and rewrite the Sales Script."*  
...
**The Universal Rule:** OpenCode (The Agent) writes the Source. PFOS (The Factory) compiles it into Artifacts.

| **Track** | **The Writer (AI)** | **Input (Source in Git)** | **The Compiler (Deterministic)** | **Output (Artifact)** | **Runtime (Target)** |
|-----------|---------------------|---------------------------|----------------------------------|----------------------|----------------------|
| **Strategy** | **OpenCode** | epf/strategy/\*.yaml | pfos | decisions.md | Linear / Dashboard |
| **Product** | **OpenCode** | epf/product/\*.yaml | **OpenCode** (Build Step) | main.go, Dockerfile | Cloud (AWS) |
| **Org/Ops** | **OpenCode** | epf/ops/\*.yaml | pfos | process.json | ClickUp / ERP |
| **Commercial** | **OpenCode** | epf/growth/\*.yaml | pfos | campaign.csv | HubSpot / CRM |

*Note: For the **Product** track, OpenCode acts twice: first writing the spec (product.yaml), then writing the actual application code (main.go) based on that spec.*
...
* **Workspace:** ProductFactory.code-workspace created containing:  
  * epf-cli/ (The Kernel)  
  * product-factory-os/ (The OS)  
  * EPF\_Canonical/ (Reference assets)
...
* **Objective:** Use OpenCode to write the roadmap for PFOS.  
* **Action:** OpenCode writes epf/product.yaml for the PFOS tool itself.

### **Phase 4: Build the OS (product-factory-os)**
