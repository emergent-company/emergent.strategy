...
> **File Purpose**: Framework maintenance protocol and schema-first enrichment workflow  
> **When to use**: Modifying **EPF framework** OR enriching **instance artifacts**  
...
| **Daily operations in product repo** (commands, sync) | → [`.github/copilot-instructions.md`](.github/copilot-instructions.md) |
| **Modifying EPF framework** (schemas, templates, scripts) | → **This file** (you're in the right place) |
| **Enriching instance artifacts** (filling YAML files) | → **This file** (see Schema-First Workflow below) |
...
## 🎭 CRITICAL: Canonical vs Non-Canonical Tracks

**EPF has 4 tracks, but they are NOT all treated the same:**

### ✅ CANONICAL Tracks (Strategy, OrgOps, Commercial)

These tracks have **standardized definitions** usable by any organization:

| Track | Definition Type | Location | Status |
|-------|-----------------|----------|--------|
| **Strategy** | Strategy Definitions (sd-*) | `definitions/strategy/` | CANONICAL - use as-is |
| **OrgOps** | Process Definitions (pd-*) | `definitions/org_ops/` | CANONICAL - use as-is |
| **Commercial** | Commercial Definitions (cd-*) | `definitions/commercial/` | CANONICAL - use as-is |

**When user asks for Strategy/OrgOps/Commercial definitions:**
- → Point them to canonical definitions in `definitions/{track}/`
- → These are ready to use without modification
- → Customize in instance only if org-specific adaptation needed

### ⚠️ NON-CANONICAL Track (Product)

The Product track contains **EXAMPLES ONLY**:

| Track | Definition Type | Location | Status |
|-------|-----------------|----------|--------|
| **Product** | Feature Definitions (fd-*) | `definitions/product/` | EXAMPLES - not canonical |

**Why Product is different:**
- Every product has **unique features** (no two products are alike)
- Features in `definitions/product/` are **quality examples** demonstrating EPF compliance
- They show patterns and validation standards, NOT reusable templates

**When user asks for Product/Feature definitions:**
- → Show examples from `definitions/product/` for reference/learning
- → Point to template at `definitions/product/_template/feature_definition_template.yaml`
- → Create product-specific features in `_instances/{product}/FIRE/feature_definitions/`
- → ❌ NEVER suggest copying Product examples verbatim - they're illustrative, not reusable

### Quick Decision Tree

```
User wants: Strategy/OrgOps/Commercial definitions?
├─ YES → Use canonical definitions from definitions/{track}/
│        (These are standardized and reusable)
└─ NO (Product/Features)?
   ├─ Want to see quality examples? → definitions/product/
   ├─ Want to create features? → Use template + create in instance
   └─ ❌ Never suggest these are canonical templates
```

### ⚠️ Value Models Follow the Same Pattern

| Track | Value Model | Status |
|-------|-------------|--------|
| **Strategy** | `strategy.value_model.yaml` | ✅ CANONICAL - use as-is or customize |
| **OrgOps** | `org_ops.value_model.yaml` | ✅ CANONICAL - use as-is or customize |
| **Commercial** | `commercial.value_model.yaml` | ✅ CANONICAL - use as-is or customize |
| **Product** | `product.value_model.yaml` | ⚠️ PLACEHOLDER - must be customized entirely |

The Product value model is a **placeholder** because each product delivers unique value. Users must create their own Product value model from scratch.
...
## 🔄 CRITICAL: Migration Protocol (Validate-As-You-Go)

**When migrating artifacts between EPF versions, build quality control into each step. FAIL FAST - fix errors before proceeding.**

### Per-Artifact Migration Workflow

For EACH artifact being migrated, execute this sequence:
...
┌─────────────────────────────────────────────────────────────┐
│ STEP 2: UNDERSTAND                                          │
│ Read: artifact, target schema, migration guide, registry    │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│ STEP 3: TRANSFORM                                           │
│ Apply changes per guide. Preserve user content. Add fields. │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│ STEP 4: VALIDATE SCHEMA (MANDATORY)                         │
│ ./scripts/validate-schemas.sh {file}                        │
│ ✓ PASS → Continue    ✗ FAIL → STOP, FIX, RETRY              │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│ STEP 5: CHECK CONTENT                                       │
│ ./scripts/check-content-readiness.sh {file}                 │
│ Grade A-C → Good    Grade D → Note for follow-up            │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│ STEP 6: REPORT & PROCEED TO NEXT ARTIFACT                   │
│ Report: schema=PASS, content=Grade, ready=YES               │
└─────────────────────────────────────────────────────────────┘
```
...
| Rule | Why |
|------|-----|
| **Backup before touching** | Can rollback if migration fails |
...
- `migrations/registry.yaml` - Version-specific changes and AI instructions
- `migrations/guides/*.md` - Detailed migration guides per version
- `MIGRATIONS.md` - Strategy and workflow documentation
- `scripts/generate-migration-plan.sh` - Creates MIGRATION_PLAN.yaml for instance
...
6. **Cross-File ID Traceability & Referential Integrity**
   ...
   - **CRITICAL: Feature Definition → KR references must exist in roadmap**
     ...
     - Run: `grep -r "kr-[a-z]-[0-9]+" definitions/product/ | xargs -I{} grep -q "id: \"{}\""` 
...
8. **Feature Definition Format Compliance**
   ...
   **Traceability Path Validation (CRITICAL FOR EPF INTEGRITY):**
   ...
   a) **Feature → Feature Dependencies** (validate cross-references)
      ...
      - Action: Validate using `./scripts/validate-cross-references.sh definitions/product/`
   ...
   b) **Feature → Value Model Paths** (THE LINCHPIN - validates strategic alignment)
      ...
      - Action: Validate using `./scripts/validate-value-model-references.sh definitions/product/`
   ...
   c) **Feature → Roadmap Assumptions** (validates strategy feedback loops)
      ...
      - Action: Validate using `./scripts/validate-roadmap-references.sh definitions/product/`
...
4. **Update migration infrastructure** (see next section)
5. **Run verification command:**
   ...
   ```
6. **Commit version sync separately** from content changes

**This is not optional. Version drift breaks traceability and causes confusion.**

## 📦 Migration Infrastructure Maintenance (MANDATORY for New Versions)

When releasing a new EPF version, you MUST update the migration infrastructure:

### 1. Update migrations/registry.yaml

Add a new version entry at the TOP of the `versions:` array:

```yaml
# In migrations/registry.yaml
versions:
  - version: "X.Y.Z"  # NEW VERSION
    ...
    breaking_changes:  # For MAJOR versions
      - id: "unique-change-id"
        description: "What changed"
        schema: "affected_schema.json"
        change_type: "restructure|remove|rename"
        old_structure: { field: "old_field", type: "old_type" }
        new_structure: { field: "new_field", type: "new_type" }
        ai_instructions: |
          Step-by-step instructions for AI to perform migration
        migration_guide: "guides/vX.x-to-vY.0.0.md"
    
    new_fields:  # For MINOR versions
      - schema: "affected_schema.json"
        field: "new_field_path"
        type: "string|array|object"
        required: false
        description: "What this field does"
        enrichment_guidance: "How to populate this field"
    
    deprecated_fields: []  # Fields being phased out
    renamed_fields: []     # Fields that changed names
```

### 2. Create Migration Guide (for MAJOR or significant MINOR versions)

If the version has breaking changes or significant new fields:

```bash
# Create guide at migrations/guides/vX.Y.x-to-vA.B.x.md
```

Guide template:
```markdown
# Migration Guide: vX.Y.x → vA.B.x

> **Migration Type**: MAJOR|MINOR
> **Estimated Effort**: X hours per artifact
> **Breaking Changes**: Yes|No

## Overview
[What changed and why]

## Change 1: [Name] {#anchor}
### What Changed
### Before (vX.Y.x)
### After (vA.B.x)
### Migration Steps
...
In `migrations/registry.yaml`, update the `compatibility` section:

```yaml
compatibility:
  ranges:
    - name: "vX.Y+"
      versions: ["X.Y.0", "X.Y.1", ...]
      compatible: true
...
- [ ] `migrations/registry.yaml` has new version entry
- [ ] `registry.yaml` `epf_current_version` updated to new version
- [ ] `registry.yaml` `last_updated` date updated
- [ ] Migration guide created (if breaking changes)
- [ ] Compatibility matrix updated
- [ ] Test: `./scripts/generate-migration-plan.sh` works with new version

### Why This Matters

The migration infrastructure enables:
- **AI-assisted upgrades**: Agents read registry to understand what changed
- **Automated assessment**: Scripts detect version gaps and generate plans
- **Consistent migrations**: Guides ensure all instances migrate the same way
- **Product team autonomy**: Teams can migrate at their own pace with clear guidance

**Skipping this step means product repos cannot properly migrate to the new version.**
