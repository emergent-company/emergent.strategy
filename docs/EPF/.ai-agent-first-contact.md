...
→ Use the **interactive onboarding wizard**: [`wizards/start_epf.agent_prompt.md`](wizards/start_epf.agent_prompt.md)
...
### "Update EPF" / "Migrate EPF artifacts" / "Upgrade to new version" ⚠️ CRITICAL

**This is EPF VERSION MIGRATION - use the documented approach!**

🚨 **DO NOT invent your own migration approach. Follow these steps:**
...
## 🎯 Reality Baseline: Know Where You're Starting From
...
- **Right-size guidance**: Solo founders ≠ 15-person teams
...
### Step 2: 🚨 Create Living Reality Assessment (REQUIRED - 10-15 minutes)
...
> 🛑 **DO NOT SKIP**: Proceeding to North Star or other READY artifacts without the Living Reality Assessment is a critical anti-pattern that results in strategy disconnected from organizational reality.
...
- [ ] **🚨 LIVING REALITY ASSESSMENT**: Does `_instances/{product}/AIM/living_reality_assessment.yaml` exist? **If NO, CREATE IT FIRST** before any READY artifacts!
...
| ❌ WRONG | ✅ CORRECT |
|----------|-----------|
| Generate feature from memory | Read `feature_definition_schema.json` → Read `definitions/product/01-technical/fd-002-*.yaml` → Generate |
...
| Treat `definitions/product/` as canonical templates | Use as examples only – each product has unique features |
...
|---------------------|-----------------|---------------|
| **Onboarding / "I'm new"** | [`start_epf.agent_prompt.md`](wizards/start_epf.agent_prompt.md) ✨ **NEW** | 5-10 min |
...
- **Product track**: `definitions/product/` contains **examples only** – each product writes unique features
