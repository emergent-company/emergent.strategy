...
export async function ensureSchema() {
    // Run the init SQL if tables are missing
  ...
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
    ...
    // Backfill created_at/updated_at if missing on existing installations
  ...
  CREATE TABLE IF NOT EXISTS kb.chunks (
    ...
    // Ensure content_hash column and unique index for deduplication
    ...
    // Delete duplicate documents keeping the earliest created record per content_hash
    await pool.query(`
      WITH ranked AS (
    ...
    // Ensure trigger function and trigger to keep tsv up to date (idempotent)
    await pool.query(`
      DO $do$
      BEGIN
        IF NOT EXISTS (
