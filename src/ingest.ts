...
export async function ingestUrl(url: string) {
    ...
    return ingestText({ text, source_url: url, mime_type: contentType });
}

export async function ingestText({ text, source_url, filename, mime_type }: { text: string; source_url?: string; filename?: string; mime_type?: string; }) {
    // Compute content hash client-side to avoid extra DB work and enable short-circuit
    const contentHash = createHash('sha256').update(text || '').digest('hex');
    const existing = await query<{ id: string }>('SELECT id FROM kb.documents WHERE content_hash = $1', [contentHash]);
    if (existing.rowCount && existing.rowCount > 0) {
        return { documentId: existing.rows[0].id, chunks: 0, alreadyExists: true };
    }
    const { rows: drows } = await query<{ id: string }>(
        `INSERT INTO kb.documents(source_url, filename, mime_type, content, content_hash)
         VALUES ($1,$2,$3,$4,$5)
         RETURNING id`,
        [source_url || null, filename || null, mime_type || 'text/plain', text, contentHash]
    );
