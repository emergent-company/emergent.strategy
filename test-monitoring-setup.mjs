...
async function testDatabaseSetup() {
    ...
    try {
        ...
        if (parseInt(processLogsResult.rows[0].count) === 1) {
            ...
            const expectedColumns = ['id', 'process_id', 'process_type', 'level', 'message', 'metadata', 'timestamp', 'project_id'];
            ...
            if (missingColumns.length === 0) {
                ...
                error(`Missing columns in system_process_logs: ${missingColumns.join(', ')}`);
            }

            // Check row count
            const countResult = await client.query('SELECT COUNT(*) as count FROM kb.system_process_logs');
            info(`  Current rows: ${countResult.rows[0].count}`);
        } else {
            error('Table kb.system_process_logs does not exist');
            warn('  Run migration: npx nx run server-nest:migrate');
        }
        ...
        if (parseInt(llmLogsResult.rows[0].count) === 1) {
            ...
            const expectedLLMColumns = ['id', 'process_id', 'process_type', 'model_name', 'input_tokens', 'output_tokens', 'cost_usd'];
            const actualColumns = columnsResult.rows.map(r => r.column_name);
            const missingColumns = expectedLLMColumns.filter(c => !actualColumns.includes(c));
            ...
            // Check row count
            const countResult = await client.query('SELECT COUNT(*) as count FROM kb.llm_call_logs');
            info(`  Current rows: ${countResult.rows[0].count}`);
        } else {
        ...
        if (indexCount >= 10) {
            ...
            info(`  Indexes: ${indexesResult.rows.map(r => r.indexname).join(', ')}`);
        } else {
...
async function testAPIEndpoints() {
    ...
    try {
        const response = await fetch(`${API_URL}/monitoring/extraction-jobs?limit=5`, { headers });
    ...
    try {
        const listResponse = await fetch(`${API_URL}/monitoring/extraction-jobs?limit=1`, { headers });
        if (listResponse.ok) {
            const listData = await listResponse.json();
            if (listData.items && listData.items.length > 0) {
                const jobId = listData.items[0].id;

                const detailResponse = await fetch(`${API_URL}/monitoring/extraction-jobs/${jobId}`, { headers });
                if (detailResponse.ok) {
                    const detailData = await detailResponse.json();
                    success(`GET /monitoring/extraction-jobs/:id - Status ${detailResponse.status}`);
                    ...
                    error(`GET /monitoring/extraction-jobs/:id - Status ${detailResponse.status}`);
                }
            } else {
...
async function checkModuleRegistration() {
    ...
    const appModulePath = path.resolve('./apps/server-nest/src/modules/app.module.ts');
    if (fs.existsSync(appModulePath)) {
        ...
        if (content.includes('MonitoringModule')) {
            ...
            warn('  Add: import { MonitoringModule } from \'./monitoring/monitoring.module\';');
        }
    } else {
        warn('Could not find app.module.ts');
    }

    const extractionModulePath = path.resolve('./apps/server-nest/src/modules/extraction-jobs/extraction-job.module.ts');
