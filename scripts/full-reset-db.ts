...
async function recreateMinimalSchema(pool: Pool) {
    ...
    await exec(pool, 'CREATE UNIQUE INDEX idx_projects_org_lower_name ON kb.projects(org_id, LOWER(name))');
    ...
    console.log('[full-reset-db] Membership tables (subject_id model)');
    ...
    await exec(pool, 'CREATE UNIQUE INDEX idx_project_membership_unique ON kb.project_memberships(project_id, subject_id)');
