  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        console.log(`   ⚠️  Folder ${folderPath} not found (skipping)`);
        return secrets;
      }
      const errorText = await response.text();
      throw new Error(
    ...
    console.log(`   ✅ Loaded ${secrets.size} secrets from ${folderPath}`);
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : String(error);
    console.error(`   ❌ Error loading ${folderPath}: ${message}`);
  }
...
async function main(): Promise<void> {
  ...
  console.log(`\n📊 Total: ${allSecrets.size} unique secrets\n`);

  if (allSecrets.size === 0) {
    console.log('⚠️  No secrets found. Nothing to write.');
    process.exit(0);
  }
  ...
  if (options.dryRun) {
    console.log('🔍 Dry run - secrets would be written:\n');
    ...
    console.log(`✅ ${existed ? 'Updated' : 'Created'} ${options.output}`);
