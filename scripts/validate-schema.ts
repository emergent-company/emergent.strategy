...
const CRITICAL_TABLES: TableCheck[] = [
    { schema: 'kb', table: 'documents', minColumns: 7, critical: true, description: 'Document storage' },
    { schema: 'kb', table: 'chunks', minColumns: 5, critical: true, description: 'Text chunks with embeddings' },
    { schema: 'kb', table: 'object_extraction_jobs', minColumns: 30, critical: true, description: 'Extraction job queue' },
    { schema: 'kb', table: 'graph_embedding_jobs', minColumns: 10, critical: true, description: 'Embedding job queue' },
    { schema: 'kb', table: 'auth_introspection_cache', minColumns: 3, critical: true, description: 'OAuth token cache' },
    { schema: 'kb', table: 'tags', minColumns: 3, critical: true, description: 'Tag management' },
    { schema: 'kb', table: 'graph_objects', minColumns: 5, critical: true, description: 'Extracted graph objects' },
    { schema: 'kb', table: 'graph_relationships', minColumns: 4, critical: true, description: 'Object relationships' },
];
...
const REQUIRED_FUNCTIONS: FunctionCheck[] = [
    { schema: 'kb', name: 'update_tsv', critical: true, description: 'Full-text search trigger' },
    { schema: 'kb', name: 'refresh_revision_counts', critical: true, description: 'Refresh materialized view' },
];
...
async function validateSchema(client: Client, verbose: boolean): Promise<ValidationResult> {
    ...
    if (applied.size === 0) {
        console.log(`  ${YELLOW}⚠${NC} No migrations tracked (schema_migrations is empty)`);
        result.warnings.push('Migration tracking table is empty or missing');
        result.migrationIssues++;
    } else {
        console.log(`  ${GREEN}✓${NC} ${applied.size} migrations tracked as applied`);
    }

    const pending = available.filter(m => !applied.has(m));
    if (pending.length > 0) {
        console.log(`  ${YELLOW}⚠${NC} ${pending.length} migrations not marked as applied:`);
    ...
    for (const tableCheck of CRITICAL_TABLES) {
        ...
        if (!exists) {
            console.log(`  ${RED}✗${NC} ${tableCheck.schema}.${tableCheck.table} - TABLE MISSING (${tableCheck.description})`);
            result.issues.push(`Missing critical table: ${tableCheck.schema}.${tableCheck.table}`);
            result.tablesMissing++;
            result.passed = false;
        } else if (columnCount < tableCheck.minColumns) {
            console.log(`  ${RED}✗${NC} ${tableCheck.schema}.${tableCheck.table} - INCOMPLETE (${columnCount} columns, expected ${tableCheck.minColumns}+)`);
            result.issues.push(`Table ${tableCheck.schema}.${tableCheck.table} has insufficient columns`);
            result.passed = false;
        } else {
            if (verbose) {
                console.log(`  ${GREEN}✓${NC} ${tableCheck.schema}.${tableCheck.table} (${columnCount} columns) - ${tableCheck.description}`);
            } else {
                console.log(`  ${GREEN}✓${NC} ${tableCheck.schema}.${tableCheck.table} (${columnCount} columns)`);
            }
        }
    }
    ...
    if (verbose || OPTIONAL_TABLES.length > 0) {
        ...
        for (const tableCheck of OPTIONAL_TABLES) {
            ...
            if (!exists) {
                console.log(`  ${YELLOW}⚠${NC} ${tableCheck.schema}.${tableCheck.table} - TABLE MISSING (non-critical)`);
                result.warnings.push(`Optional table missing: ${tableCheck.schema}.${tableCheck.table}`);
            } else {
                console.log(`  ${GREEN}✓${NC} ${tableCheck.schema}.${tableCheck.table} (${columnCount} columns)`);
            }
        }
    ...
    // Check 4: Functions
    console.log(`${BLUE}[${verbose ? '4' : '3'}/4] Checking required functions...${NC}`);
    for (const funcCheck of REQUIRED_FUNCTIONS) {
        result.functionsChecked++;
        const exists = await checkFunctionExists(client, funcCheck.schema, funcCheck.name);

        if (!exists) {
            console.log(`  ${RED}✗${NC} ${funcCheck.schema}.${funcCheck.name}() - FUNCTION MISSING`);
            result.issues.push(`Missing critical function: ${funcCheck.schema}.${funcCheck.name}()`);
            result.functionsMissing++;
            result.passed = false;
        } else {
            console.log(`  ${GREEN}✓${NC} ${funcCheck.schema}.${funcCheck.name}() - ${funcCheck.description}`);
        }
    }
    ...
    for (const funcCheck of OPTIONAL_FUNCTIONS) {
        const exists = await checkFunctionExists(client, funcCheck.schema, funcCheck.name);
        if (!exists) {
            console.log(`  ${YELLOW}⚠${NC} ${funcCheck.schema}.${funcCheck.name}() - FUNCTION MISSING (non-critical)`);
            result.warnings.push(`Optional function missing: ${funcCheck.schema}.${funcCheck.name}()`);
        } else if (verbose) {
            console.log(`  ${GREEN}✓${NC} ${funcCheck.schema}.${funcCheck.name}() - ${funcCheck.description}`);
        }
    }
...
async function fixSchema(client: Client): Promise<void> {
    ...
    const missingTables = initialValidation.issues.filter(i => i.includes('Missing critical table'));
    const missingFunctions = initialValidation.issues.filter(i => i.includes('Missing critical function'));
    
    if (missingTables.length === 0 && missingFunctions.length === 0) {
        console.log(`${GREEN}✓ No missing objects detected${NC}\n`);
    } else {
        console.log(`  Found ${missingTables.length} missing tables and ${missingFunctions.length} missing functions\n`);
    }

    console.log(`${BLUE}[2/3] Extracting and creating missing objects from migrations...${NC}`);
    ...
    for (const issue of missingTables) {
        ...
        if (tableMatch) {
            try {
                ...
                console.log(`  ${RED}✗${NC} Failed to create ${tableName}: ${error.message}`);
            }
        } else {
            console.log(`  ${YELLOW}⚠${NC} Could not find CREATE statement for ${tableName} in migration`);
        }
    }
    ...
    for (const issue of missingFunctions) {
        ...
        if (funcMatch) {
            try {
                ...
                console.log(`  ${RED}✗${NC} Failed to create ${functionName}(): ${error.message}`);
            }
        } else {
            console.log(`  ${YELLOW}⚠${NC} Could not find CREATE statement for ${functionName}() in migration`);
        }
    }
    ...
    if (verifyResult.passed) {
        ...
        console.log(`${YELLOW}Remaining issues may require manual review of migration files${NC}`);
    }
}
...
function printSummary(result: ValidationResult): void {
    ...
    if (result.passed) {
        ...
        if (result.warnings.length > 0) {
            ...
            result.warnings.forEach(warning => console.log(`  ${YELLOW}•${NC} ${warning}`));
        }

        console.log();
        console.log(`${BLUE}Recommended actions:${NC}`);
        console.log(`  1. Run: ${BLUE}npm run db:validate --fix${NC} to auto-repair`);
        console.log(`  2. Or manually run: ${BLUE}npm run migrate${NC}`);
        console.log(`  3. Or review migration files in apps/server-nest/migrations/`);
    }
