...
async function main() {
    console.log('🌱 Starting Emergent Product Framework Template Pack Seeding...');
    ...
    try {
        ...
        if (exists) {
            console.log('⚠️  Emergent Product Framework template pack already exists - skipping creation');
            return;
        }

        const templatePackId = await createEpfTemplatePack(client);
        console.log(`✅ Created Emergent Product Framework template pack with ID: ${templatePackId}`);
