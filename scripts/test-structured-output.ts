...
async function main() {
  ...
  console.log('Testing structured output with simple schema...');
  const start = Date.now();

  try {
    const result = await Promise.race([
      structuredModel.invoke('Extract person names from: John and Mary went to the store.'),
      new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout after 30s')), 30000)),
    ]);
    console.log(`✅ Success in ${Date.now() - start}ms:`, JSON.stringify(result, null, 2));
  } catch (err: any) {
