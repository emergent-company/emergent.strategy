...
const templatePackId = process.env.EXTRACTION_SEED_PACK_ID || DEFAULT_TEMPLATE_PACK_ID;
const templatePackName = process.env.EXTRACTION_SEED_PACK_NAME || 'Extraction Demo Pack';
...
const baseTypes = [
    {
        ...
        schema: {
            ...
            properties: {
                ...
                company: { type: 'string', description: 'Organization the person is associated with' },
                location: { type: 'string', description: 'Primary location for the person' },
            },
        },
        extraction: {
    ...
    },
    {
        ...
        extraction: {
            system: 'Extract organizations mentioned in the text and capture their key attributes.',
    ...
    },
    {
        ...
        schema: {
            ...
            properties: {
                ...
                country: { type: 'string', description: 'Country where the location resides' },
                region: { type: 'string', description: 'Region or state for the location' },
            },
        },
        extraction: {
...
async function ensureOrg(client: PoolClient): Promise<string> {
    const existing = await client.query<{ id: string }>('SELECT id FROM kb.orgs WHERE name = $1 LIMIT 1', [orgName]);
    if (existing.rowCount) {
        console.log(`• Reusing existing org “${orgName}”`);
        return existing.rows[0].id;
    }
    const inserted = await client.query<{ id: string }>('INSERT INTO kb.orgs(name) VALUES($1) RETURNING id', [orgName]);
    console.log(`• Created org “${orgName}”`);
    return inserted.rows[0].id;
}

async function ensureProject(client: PoolClient, orgId: string): Promise<string> {
    const existing = await client.query<{ id: string }>(
        'SELECT id FROM kb.projects WHERE org_id = $1 AND name = $2 LIMIT 1',
        [orgId, projectName]
    );
    if (existing.rowCount) {
    ...
    const inserted = await client.query<{ id: string }>(
        'INSERT INTO kb.projects(org_id, name) VALUES($1, $2) RETURNING id',
        [orgId, projectName]
    );
...
async function upsertTemplatePack(client: PoolClient): Promise<void> {
    ...
    for (const type of baseTypes) {
        ...
        uiConfigs[type.type] = {
            icon: type.type === 'Person' ? 'lucide--user' : type.type === 'Organization' ? 'lucide--building-2' : 'lucide--map-pin',
            color: type.type === 'Person' ? 'primary' : type.type === 'Organization' ? 'accent' : 'info',
        };
    }

    const existing = await client.query('SELECT id FROM kb.graph_template_packs WHERE id = $1', [templatePackId]);
    if (existing.rowCount) {
        await client.query(
...
async function ensureTemplateAssignment(client: PoolClient, tenantId: string, orgId: string, projectId: string): Promise<void> {
    const existing = await client.query('SELECT id FROM kb.project_template_packs WHERE project_id = $1 AND template_pack_id = $2', [projectId, templatePackId]);
    if (existing.rowCount) {
    ...
    await client.query(
        ...
        [tenantId, orgId, projectId, templatePackId, seedUserId]
    );
    console.log('• Assigned template pack to project');
}

async function ensureProjectTypes(client: PoolClient, tenantId: string, orgId: string, projectId: string): Promise<void> {
    for (const type of baseTypes) {
        const existing = await client.query('SELECT id FROM kb.project_object_type_registry WHERE project_id = $1 AND type = $2', [projectId, type.type]);
        if (existing.rowCount) {
            await client.query(
            ...
            await client.query(
                ...
                ) VALUES ($1, $2, $3, $4, 'template', $5, $6::jsonb, '{}'::jsonb, $7::jsonb, true, $8, $9)`,
                [
                    tenantId,
                    orgId,
                    projectId,
...
async function ensureDocument(client: PoolClient, orgId: string, projectId: string): Promise<string> {
    const existing = await client.query<{ id: string }>(
    ...
    if (existing.rowCount) {
        ...
        await client.query('UPDATE kb.documents SET content = $1, mime_type = $2, updated_at = now() WHERE id = $3', [sampleDocument.content, sampleDocument.mimeType, docId]);
        await client.query(
    ...
    const inserted = await client.query<{ id: string }>(
        `INSERT INTO kb.documents (org_id, project_id, filename, mime_type, content)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING id`,
        [orgId, projectId, sampleDocument.filename, sampleDocument.mimeType, sampleDocument.content]
    );
...
async function ensurePendingJob(client: PoolClient, tenantId: string, orgId: string, projectId: string, documentId: string): Promise<void> {
    const existing = await client.query('SELECT id FROM kb.object_extraction_jobs WHERE project_id = $1 AND document_id = $2 AND status = $3 LIMIT 1', [projectId, documentId, 'pending']);
    if (existing.rowCount) {
    ...
    await client.query(
        `INSERT INTO kb.object_extraction_jobs (
            tenant_id, organization_id, project_id, document_id,
            job_type, status, enabled_types, extraction_config,
            created_by, source_type, source_id, source_metadata
        ) VALUES ($1, $2, $3, $4, 'full_extraction', 'pending', $5::text[], $6::jsonb, $7, 'DOCUMENT', $8, '{}'::jsonb)`
        , [
            tenantId,
            orgId,
            projectId,
