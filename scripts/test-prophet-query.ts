...
async function test() {
  try {
    console.log('🔍 Testing query: "prophet who spoke to the people"\n');
    
    const embedding = await generateEmbedding('prophet who spoke to the people');
    ...
    result.rows.forEach((row, i) => {
      console.log(`${i+1}. ${row.key} - distance: ${row.distance.toFixed(4)}`);
      console.log(`   ${row.properties.role || row.properties.title}`);
      if (row.distance <= 0.9) {
        console.log(`   ✅ Within maxDistance threshold (0.9)`);
      } else {
        console.log(`   ❌ Outside maxDistance threshold (0.9)`);
      }
      console.log('');
    });
