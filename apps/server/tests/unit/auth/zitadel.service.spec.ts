...
describe('ZitadelService', () => {
  ...
  describe('getAccessToken', () => {
    ...
    it('should cache new token with safety margin', async () => {
      ...
      expect(cached.expiresAt).toBeLessThan(Date.now() + 3600 * 1000);
    });
  });
  ...
  describe('introspect', () => {
    ...
    it('should cache inactive tokens to prevent DOS', async () => {
      mockCacheService.get.mockResolvedValue(null);
      ...
      expect(mockCacheService.set).toHaveBeenCalled();
      const [token, result, expiresAt] = mockCacheService.set.mock.calls[0];

      expect(token).toBe('test-token');
      expect(result).toEqual(introspectionResult);

      // Should have ~5 min TTL
      const now = Date.now();
      const expectedExp = now + 5 * 60 * 1000;
      expect(expiresAt.getTime()).toBeGreaterThan(now);
      expect(Math.abs(expiresAt.getTime() - expectedExp)).toBeLessThan(2000);
    });
  });
  ...
  describe('grantProjectRole', () => {
    ...
    it('should grant role successfully', async () => {
      ...
      await service.grantProjectRole('user-id', 'project-id', 'project_admin');
  ...
  describe('getUserProjectRoles', () => {
    ...
    it('should return roles if found', async () => {
      ...
      const roles = await service.getUserProjectRoles('user-id', 'project-id');
