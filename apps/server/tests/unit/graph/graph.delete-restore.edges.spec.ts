...
class MockDB {
  ...
  async query(sql: string, params: any[]): Promise<any> {
    ...
    if (
      /FROM kb\.graph_objects WHERE canonical_id=\$1 ORDER BY version DESC LIMIT 1$/i.test(
        sql
      )
    ) {
    ...
    // Insert tombstone or restore object. Real service now appends fts, embedding_updated_at columns.
    if (
      ...
      const row: ObjRow = {
        ...
        properties,
        labels,
        deleted_at: isTombstone ? new Date() : null,
        created_at: new Date(),
      } as any;
    ...
    if (
      /FROM kb\.graph_relationships WHERE canonical_id=\$1 ORDER BY version DESC LIMIT 1$/i.test(
        sql
      )
    ) {
      const rows = this.rels
        .filter((r) => r.canonical_id === params[0])
        .sort((a, b) => b.version - a.version);
      const head = rows[0];
      return { rowCount: head ? 1 : 0, rows: head ? [head] : [] };
    }
    ...
    if (
      /^INSERT INTO kb\.graph_relationships\(project_id, branch_id, type, src_id, dst_id, properties, version, canonical_id, supersedes_id, deleted_at\)/i.test(
        sql
      )
    ) {
      const [
    ...
    if (
      /FROM kb\.graph_relationships WHERE project_id=\$1 AND type=\$2 AND src_id=\$3 AND dst_id=\$4\s+ORDER BY version DESC LIMIT 1$/i.test(
        sql
      )
    ) {
    ...
    // UPDATE object to set deleted_at (soft delete)
    if (
      /^UPDATE kb\.graph_objects SET deleted_at = now\(\) WHERE id = \$1 AND deleted_at IS NULL$/i.test(
        sql
      )
    ) {
      const obj = this.objects.find(
        (o) => o.id === params[0] && o.deleted_at === null
      );
      if (obj) {
        obj.deleted_at = new Date();
        return { rowCount: 1, rows: [obj] };
      }
      return { rowCount: 0, rows: [] };
    }
    // UPDATE relationship to set deleted_at (soft delete)
    if (
      /^UPDATE kb\.graph_relationships SET deleted_at = now\(\) WHERE id = \$1 AND deleted_at IS NULL$/i.test(
        sql
      )
    ) {
      const rel = this.rels.find(
        (r) => r.id === params[0] && r.deleted_at === null
      );
      if (rel) {
...
describe('GraphService delete/restore edge paths', () => {
  ...
  let schema: MockSchemaRegistry;
  let baseObjId: string;
  let relId: string;
  let relCanonical: string;
  let objCanonical: string;
  ...
  it('object: second delete throws already_deleted', async () => {
    ...
    await expect(service.deleteObject(baseObjId)).rejects.toMatchObject({
      response: { message: 'already_deleted' },
    });
  });

  it('object: restore when not deleted throws not_deleted', async () => {
    await expect(service.restoreObject(baseObjId)).rejects.toMatchObject({
      response: { message: 'not_deleted' },
    });
  });
  ...
  it('relationship: second delete throws already_deleted', async () => {
    ...
    expect(first.deleted_at).toBeTruthy();
    await expect(service.deleteRelationship(relId)).rejects.toMatchObject({
      response: { message: 'already_deleted' },
    });
  });

  it('relationship: restore when not deleted throws not_deleted', async () => {
    await expect(service.restoreRelationship(relId)).rejects.toMatchObject({
      response: { message: 'not_deleted' },
    });
  });
});
