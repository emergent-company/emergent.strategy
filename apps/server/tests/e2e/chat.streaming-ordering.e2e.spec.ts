...
import { createConversation, streamConversation, assertDeterministicOrdering } from './utils/chat';
...
describe('Chat Streaming Ordering', () => {
    ...
    it('emits frames in deterministic order (tokens -> citations? -> summary -> done)', async () => {
        const convo = await createConversation(ctx, 'Ordering test', { isPrivate: false, userSuffix: 'chat-sse-order' });
        const stream = await streamConversation(ctx, convo.conversationId, { userSuffix: 'chat-sse-order' });
