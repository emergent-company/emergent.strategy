/**
 * Main NestJS Bootstrap Logic
 *
 * This file contains the actual NestJS application bootstrap code.
 * It is dynamically imported by main.ts AFTER OpenTelemetry SDK is started,
 * ensuring HTTP module patching is in place before NestJS loads.
 *
 * DO NOT import this file statically - always use dynamic import from main.ts.
 */
...
async function bootstrap() {
  ...
  // NOTE: OpenTelemetry SDK was already started in main.ts (the bootstrap entry point)
  ...
  if (!otelPatched) {
    console.warn(
      '[main-bootstrap] WARNING: OpenTelemetry HTTP instrumentation not applied. HTTP spans will not be traced.'
    );
  }
  ...
  // NOTE: HTTP tracing is now handled by HttpInstrumentation in tracing.ts
  // The SDK is started BEFORE NestJS imports to ensure proper patching
  // No manual middleware needed - OpenTelemetry auto-instruments HTTP requests
