...
// registerStrategyContextTools registers read-only tools that provide strategic
// context beyond the core 8 strategy query tools. These are useful for consumers
// of strategy data who want to understand features, value model paths, coverage,
// track definitions, and OKR progress without needing the full authoring toolset.
//
// Called by both registerTools() (full server) and NewStrategyOnlyServer().
func (s *Server) registerStrategyContextTools() {
	...
	s.mcpServer.AddTool(
		mcp.NewTool("epf_list_definitions",
			...
			mcp.WithString("instance_path",
				mcp.Description(instancePathDesc+". If provided, also searches FIRE/definitions/ for synced canonical definitions."),
			),
		),
