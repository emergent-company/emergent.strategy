...
## 🔍 STEP 0.5: Run Strategic Reality Check (Optional but Recommended)

**Before diving into OKR analysis, consider running a Strategic Reality Check to evaluate whether the foundations are still valid.**

The SRC evaluates all READY and FIRE artifacts — not just the roadmap — asking: are beliefs still valid? Are analyses still fresh? Are cross-references intact? Does maturity match claims?

```
epf_aim_generate_src { "instance_path": "..." }
```

This auto-populates mechanical checks. Then use the [Strategic Reality Check wizard](strategic_reality_check.agent_prompt.md) to fill in subjective assessments.

**Why run SRC before Assessment?** The Assessment Report evaluates _execution_ (did we hit our OKRs?). The SRC evaluates _foundations_ (are we building on solid ground?). If foundations are compromised, the Calibration Memo should address both execution outcomes and foundation health.

The SRC's `recalibration_plan` feeds directly into the Calibration Memo:
- Critical SRC findings may override OKR-based decisions
...
**Related:** [strategic_reality_check.agent_prompt.md](strategic_reality_check.agent_prompt.md), [strategic_reality_check_schema.json](../schemas/strategic_reality_check_schema.json)
...
- **Schema**: [living_reality_assessment_schema.json](../schemas/living_reality_assessment_schema.json) - Schema for Living Reality Assessment
- **Schema**: [track_health_assessment_schema.json](../schemas/track_health_assessment_schema.json) - Schema for Track Health Signals
- **Schema**: [strategic_reality_check_schema.json](../schemas/strategic_reality_check_schema.json) - Schema for Strategic Reality Check
- **Template**: [strategic_reality_check.yaml](../templates/AIM/strategic_reality_check.yaml) - Template for cross-artifact health evaluation
- **Wizard**: [strategic_reality_check.agent_prompt.md](strategic_reality_check.agent_prompt.md) - Agent instructions for conducting an SRC
...
## 🛠️ Write-Back MCP Tools

**Instead of manually editing YAML files, use these MCP tools to write AIM artifacts programmatically.** All tools accept structured input and automatically handle formatting, validation, and evolution logging.

### Available Tools

| Tool | Purpose | When to Use |
|------|---------|-------------|
| `epf_aim_generate_src` | Auto-populate Strategic Reality Check | Step 0.5: Before detailed OKR analysis, to evaluate foundation health |
| `epf_aim_write_src` | Write/update SRC from structured input | Step 0.5: After filling in subjective SRC assessments |
| `epf_aim_write_assessment` | Write/update assessment report | Step 5: After analyzing OKR outcomes and assumption evidence |
| `epf_aim_write_calibration` | Write/update calibration memo | Step 6: After team agrees on persevere/pivot/pull-the-plug decision |
| `epf_aim_recalibrate` | Generate changeset from memo + SRC | Step 7: After calibration, to map decisions to READY/FIRE artifact changes |
| `epf_aim_health` | Run AIM-specific diagnostics | Any time: Check for staleness, drift, and evidence gaps |
| `epf_aim_update_lra` | Update Living Reality Assessment | Final Step: After updating track baselines, focus, and gaps |
| `epf_aim_init_cycle` | Bootstrap new cycle artifacts | After calibration: Initialize next cycle's assessment template |
| `epf_aim_archive_cycle` | Archive completed cycle | Before init-cycle: Save current cycle to `cycles/cycle-N/` |

### Recommended Workflow

1. **Step 5 (Assessment):** Use `epf_aim_write_assessment` to write OKR outcomes and assumption validations instead of manually populating `assessment_report.yaml`
2. **Step 6 (Calibration):** Use `epf_aim_write_calibration` to capture the decision, learnings, and next-cycle inputs
3. **Step 7 (Recalibration):** Use `epf_aim_recalibrate` to generate a changeset that maps calibration decisions and SRC findings to specific READY/FIRE artifact changes. Use `--apply` to write LRA updates, or review the dry-run report to guide manual updates.
4. **Final Step (LRA Update):** Use `epf_aim_update_lra` to update track baselines, current focus, and capability gaps — this automatically appends to the evolution log
5. **Cycle Transition:** Use `epf_aim_archive_cycle` to snapshot the completed cycle, then `epf_aim_init_cycle` to set up fresh templates for next cycle

### Benefits Over Manual YAML Editing

- **Validation at write time** — rejects invalid states before they reach the file
- **Automatic evolution logging** — every LRA update is tracked with who, when, and why
- **Merge semantics** — updates specific fields without overwriting the entire file
- **MCP compatibility** — AI agents can call these tools directly via JSON parameters
