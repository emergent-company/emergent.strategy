...
The TTL-based auto-expiration feature provides automatic temporal validity checks for graph objects based on expiration timestamps. Objects can optionally be assigned an `expires_at` timestamp, after which they are automatically excluded from all search and traversal operations.

This feature is part of **Phase 3 - Task 7c** and enables:
