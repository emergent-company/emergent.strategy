...
export class HttpLoggerInterceptor implements NestInterceptor {
    ...
    private readonly httpLogPath: string | null;
    private writeStream: fs.WriteStream | null = null;
    private readonly enabled: boolean;

    constructor() {
        // HTTP logging controlled by HTTP_LOG_PATH environment variable
        // If not set or empty, logging is disabled
        const logPath = process.env.HTTP_LOG_PATH;

        if (!logPath || logPath.trim() === '') {
            this.enabled = false;
            this.httpLogPath = null;
            this.logger.log('HTTP request logging DISABLED (HTTP_LOG_PATH not set)');
            return;
        }

        this.enabled = true;

        // Resolve log path (can be absolute or relative to project root)
        this.httpLogPath = path.isAbsolute(logPath)
            ? logPath
    ...
    intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
        ...
        const realIp = (headers['x-forwarded-for'] as string)?.split(',')[0].trim() ||
    ...
    private logRequest(
        ...
        if (process.env.NODE_ENV === 'development') {
            ...
            console.log(`${statusColor}${method} ${path} ${statusCode}${resetColor} ${duration}ms`);
        }
    }
