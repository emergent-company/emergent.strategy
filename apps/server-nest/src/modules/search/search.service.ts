...
import { calculateStatistics, normalizeScore, fuseScores } from './score-normalization.util';

interface SearchResultItem { id: string; document_id: string; chunk_index: number; text: string; }
interface ScoredResultItem extends SearchResultItem { score?: number; }
...
export class SearchService {
    ...
    async search(
    ...
    ): Promise<{ mode: SearchMode; results: SearchResultItem[]; warning?: string; pathSummaries?: Map<string, any>; totalCandidates?: number }> {
        const q = query.trim();
        ...
        try { qvec = await this.embeddings.embedQuery(q); } catch (e) {
            ...
            return { mode: SearchMode.LEXICAL, results: rows, warning: `Embedding failed; lexical fallback: ${(e as Error).message}`, pathSummaries, totalCandidates: rows.length };
        }
        ...
        if (mode === SearchMode.VECTOR) {
            ...
            return { mode: SearchMode.VECTOR, results: rows, pathSummaries, totalCandidates: rows.length };
        }
        ...
        const candidateMap = new Map<string, { item: SearchResultItem; fusedScore: number }>();

        // Process lexical results
        for (const row of lexicalResults.rows) {
            const normalized = normalizeScore(row.score || 0, lexicalStats);
            const fusedScore = fuseScores(normalized.normalized, 0, lexicalWeight, vectorWeight);
            candidateMap.set(row.id, {
                item: { id: row.id, document_id: row.document_id, chunk_index: row.chunk_index, text: row.text },
                fusedScore,
            });
        }
        ...
        for (const row of vectorResults.rows) {
            ...
            if (existing) {
                ...
                const lexScore = lexicalResults.rows.find(r => r.id === row.id)?.score || 0;
                const lexNorm = normalizeScore(lexScore, lexicalStats);
                const fusedScore = fuseScores(lexNorm.normalized, normalized.normalized, lexicalWeight, vectorWeight);
                existing.fusedScore = fusedScore;
            } else {
                // Item only in vector channel
                const fusedScore = fuseScores(0, normalized.normalized, lexicalWeight, vectorWeight);
                candidateMap.set(row.id, {
                    item: { id: row.id, document_id: row.document_id, chunk_index: row.chunk_index, text: row.text },
                    fusedScore,
                });
        ...
        return { mode: SearchMode.HYBRID, results: sortedResults, pathSummaries, totalCandidates: candidateMap.size };
    }
}
