...
\`\`\`
     ...
     */
    private async routeMethod(request: JsonRpcRequest, req: Request): Promise<any> {
     ...
     */
    private async handleNotification(notification: JsonRpcNotification, req: Request): Promise<void> {
        ...
        return {
            tools: [
                ...
                },
                {
                    ...
                    description: 'Get schema changes since a specific version or date. Returns list of changes with timestamps and descriptions.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            since: {
                                type: 'string',
                                description: 'Version number (e.g., "1.2.0") or ISO date (e.g., "2025-10-15T00:00:00Z")'
                            },
                            limit: {
                                type: 'number',
                                description: 'Maximum number of changes to return (default: 10, max: 100)',
                                minimum: 1,
                                maximum: 100
                            }
                        },
                ...
                {
                    name: 'type_info',
                    description: 'Get information about available object types in the schema. Can return all types or details for a specific type.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            type_name: {
                                type: 'string',
                                description: 'Specific type name to get details for (optional). If omitted, returns all types.'
                            }
                        },
                        required: []
                    }
     ...
     */
    private handleError(id: number | string | null, error: any, req: Request): JsonRpcErrorResponse {
            ...
            switch (status) {
                ...
                default:
                    return this.errorResponse(
                        ...
                        { status, message: typeof response === 'string' ? response : 'Unknown error' }
                    );
            }
        }
