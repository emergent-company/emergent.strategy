import { Controller, Get, Param, ParseUUIDPipe, Query, UseInterceptors, NotFoundException, Res, Post, Body, Req, BadRequestException, Delete, UseGuards } from '@nestjs/common';
...
export class DocumentsController {
    ...
    async list(@Query('limit') limit?: string, @Query('cursor') cursor?: string, @Req() req?: any, @Res({ passthrough: true }) res?: Response) {
        ...
        if (!projectId) {
            throw new BadRequestException({ error: { code: 'bad-request', message: 'x-project-id required' } });
        }
        const { items, nextCursor } = await this.documents.list(n, decoded, { orgId, projectId });
    ...
    @ApiStandardErrors()
    @Scopes('documents:read')
    async get(@Param('id', new ParseUUIDPipe({ version: '4' })) id: string, @Req() req: any) {
        const requestedProject = (req.headers['x-project-id'] as string | undefined) || undefined;
        const requestedOrg = (req.headers['x-org-id'] as string | undefined) || undefined;
        ...
        if (requestedProject && doc.projectId && doc.projectId !== requestedProject) throw new NotFoundException({ error: { code: 'not-found', message: 'Document not found' } });
        if (requestedOrg && doc.orgId && doc.orgId !== requestedOrg) throw new NotFoundException({ error: { code: 'not-found', message: 'Document not found' } });
        return doc;
    }

    @Post()
    @ApiBody({ schema: { properties: { filename: { type: 'string' }, projectId: { type: 'string', format: 'uuid' }, content: { type: 'string' } } } })
    @ApiOkResponse({ description: 'Created document', type: DocumentDto })
    @ApiStandardErrors()
    @Scopes('documents:write')
    async create(@Body() body: CreateDocumentBody, @Req() req: any) {
        const orgId = (req.headers['x-org-id'] as string | undefined) || undefined;
        const headerProjectId = (req.headers['x-project-id'] as string | undefined) || undefined;
        // New strict rule: require header project id; ignore body.projectId fallback
        if (!headerProjectId) throw new BadRequestException({ error: { code: 'bad-request', message: 'x-project-id header required' } });
    ...
    async delete(@Param('id', new ParseUUIDPipe({ version: '4' })) id: string, @Req() req: any) {
        ...
        if (!deleted) throw new NotFoundException({ error: { code: 'not-found', message: 'Document not found' } });
