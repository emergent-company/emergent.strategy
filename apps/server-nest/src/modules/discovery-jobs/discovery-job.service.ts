...
export class DiscoveryJobService {
    ...
    private async extractTypesFromBatch(
        ...
        this.logger.log(`[DISCOVERY] Batch ${batchNumber}: Analyzing ${documentIds.length} documents`);
        ...
        this.logger.debug(`[DISCOVERY] Batch ${batchNumber}: Combined content length: ${combinedContent.length} chars`);
        ...
        this.logger.debug(`[DISCOVERY] Batch ${batchNumber}: Calling LLM with ${documents.length} documents`);
        this.logger.debug(`[DISCOVERY] Batch ${batchNumber}: KB Purpose: ${kbPurpose}`);
        ...
        this.logger.log(`[DISCOVERY] Batch ${batchNumber}: LLM discovered ${discoveredTypes.length} types`);

        if (discoveredTypes.length === 0) {
            this.logger.warn(`[DISCOVERY] Batch ${batchNumber}: No types discovered by LLM!`);
        } else {
            this.logger.debug(`[DISCOVERY] Batch ${batchNumber}: Discovered types: ${discoveredTypes.map(t => t.type_name).join(', ')}`);
        }
        ...
        this.logger.log(`[DISCOVERY] Batch ${batchNumber}: Stored ${discoveredTypes.length} type candidates`);
    }
...
Return JSON array of discovered types.`;
    ...
    private async refineAndMergeTypes(
        ...
        this.logger.log(`[DISCOVERY] Refining ${candidates.length} type candidates`);
        ...
        this.logger.log(`[DISCOVERY] Refined to ${refinedTypes.length} final types`);
     ...
     */
    private async mergeTypeSchemas(candidates: TypeCandidate[]): Promise<DiscoveredType> {
    ...
    private async discoverRelationships(
        ...
        this.logger.log(`[DISCOVERY] Discovering relationships for ${types.length} types`);

        if (types.length < 2) {
            this.logger.log('[DISCOVERY] Need at least 2 types to discover relationships');
            return [];
        }
        ...
        try {
            ...
            this.logger.log(`[DISCOVERY] LLM discovered ${discoveredRelationships.length} relationships`);
            ...
            this.logger.error(`[DISCOVERY] Relationship discovery failed: ${error.message}`, error.stack);
    ...
    private async createTemplatePackFromDiscovery(
        ...
        this.logger.log(`[DISCOVERY] Creating template pack from ${types.length} discovered types`);
    ...
    async finalizeDiscoveryAndCreatePack(
        ...
        this.logger.log(`[FINALIZE] Job ${jobId}, mode: ${mode}, types: ${includedTypes.length}, rels: ${includedRelationships.length}`);
        ...
        if (mode === 'create') {
            ...
            if (!existingPackId) {
                throw new BadRequestException('existingPackId is required for extend mode');
            }
            ...
            if (!existingPack.rows.length) {
                throw new NotFoundException(`Template pack not found: ${existingPackId}`);
            }
