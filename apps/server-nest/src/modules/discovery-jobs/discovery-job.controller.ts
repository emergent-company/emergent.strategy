import {
    ...
    Req, Logger, BadRequestException
} from '@nestjs/common';
...
export class DiscoveryJobController {
    ...
    async startDiscovery(
        ...
        if (!orgId || !headerProjectId) {
            throw new BadRequestException('Missing required headers: X-Org-ID, X-Project-ID');
        }

        // In the current multi-tenancy model, tenant_id = project_id
        const tenantId = headerProjectId;

        if (!config.document_ids || config.document_ids.length === 0) {
            throw new BadRequestException('document_ids array is required and cannot be empty');
        }
    ...
    async finalizeDiscovery(
        ...
        this.logger.log(`[FINALIZE DISCOVERY] Job ${jobId}, mode: ${body.mode}, pack: ${body.packName}`);
