...
export class DiscoveryLLMProvider {
    ...
    private initialize() {
        ...
        if (!location) {
            this.logger.warn('Discovery LLM not configured: VERTEX_AI_LOCATION missing');
            return;
        }
        ...
        this.logger.log(`Initializing Discovery Vertex AI: project=${projectId}, location=${location}, model=${modelName}`);
    ...
    async discoverTypes(params: {
        ...
        this.logger.log(`[DISCOVER_TYPES] Analyzing ${params.documents.length} documents...`);

        const combinedContent = params.documents
            .map(doc => `=== ${doc.filename} ===\n${doc.content}`)
            .join('\n\n');

        this.logger.debug(`[DISCOVER_TYPES] Combined content length: ${combinedContent.length} chars`);
...
`;
                ...
                this.logger.error(`[DISCOVER_TYPES] ❌ FAILED TO WRITE FILE: ${err?.message || String(err)}`);
            ...
            this.logger.debug(`[DISCOVER_TYPES] Response length: ${content.length} chars`);

            // Remove markdown code blocks if present
            // Try multiple patterns to handle different LLM response formats
            let jsonStr = content.trim();

            this.logger.debug(`[DISCOVER_TYPES] Content starts with: ${jsonStr.substring(0, 50)}`);
            this.logger.debug(`[DISCOVER_TYPES] Content ends with: ${jsonStr.substring(jsonStr.length - 50)}`);

            // Pattern 1: ```json ... ``` (with closing backticks)
            const jsonMatch1 = jsonStr.match(/```json\s*([\s\S]*?)\s*```/);
            if (jsonMatch1) {
                this.logger.debug(`[DISCOVER_TYPES] Matched: complete markdown block with closing backticks`);
                jsonStr = jsonMatch1[1];
            } else if (jsonStr.startsWith('```json')) {
                // Pattern 2: ```json ... (WITHOUT closing backticks - LLM forgot them)
                this.logger.debug(`[DISCOVER_TYPES] Matched: markdown start without closing backticks`);
                ...
                if (jsonMatch3) {
                    this.logger.debug(`[DISCOVER_TYPES] Matched: complete generic code block`);
                    jsonStr = jsonMatch3[1];
                } else {
                    // Has opening ``` but no closing
                    this.logger.debug(`[DISCOVER_TYPES] Matched: generic code block start without closing`);
                    jsonStr = jsonStr.substring(3).trim();
                }
            } else {
                this.logger.debug('[DISCOVER_TYPES] No markdown markers found, using content as-is');
            }

            this.logger.debug(`[DISCOVER_TYPES] Extracted JSON (first 200 chars): ${jsonStr.substring(0, 200)}`);

            const parsed = JSON.parse(jsonStr.trim()); this.logger.debug(`[DISCOVER_TYPES] Raw LLM result: ${jsonStr.substring(0, 500)}...`);

            if (!parsed || !parsed.discovered_types) {
                this.logger.warn(`[DISCOVER_TYPES] No discovered_types in LLM response`);
                return [];
            }

            this.logger.log(`[DISCOVER_TYPES] LLM returned ${parsed.discovered_types.length} types`);
            this.logger.debug(`[DISCOVER_TYPES] Discovered types: ${parsed.discovered_types.map((t: any) => t.type_name).join(', ')}`);

            return parsed.discovered_types;
        } catch (error: any) {
            this.logger.error('[DISCOVER_TYPES] Type discovery failed:', error);
            throw new Error(`Failed to discover types: ${error?.message || 'Unknown error'}`);
        }
    }    /**
    ...
    async discoverRelationships(params: {
        ...
        this.logger.log(`[DISCOVER_RELATIONSHIPS] Analyzing relationships between ${params.types.length} types...`);

        const typesList = params.types.map(t => `- ${t.name}: ${t.description}`).join('\n');
        const combinedContent = params.documents
            .map(doc => `=== ${doc.filename} ===\n${doc.content}`)
            .join('\n\n');
...
Focus on the most important relationships that appear in the documents.`;
        ...
        try {
            ...
            if (!parsed || !parsed.discovered_relationships) {
                this.logger.warn(`[DISCOVER_RELATIONSHIPS] No discovered_relationships in LLM response`);
                return [];
            }

            this.logger.log(`[DISCOVER_RELATIONSHIPS] LLM returned ${parsed.discovered_relationships.length} relationships`);

            return parsed.discovered_relationships;
        } catch (error: any) {
            this.logger.error('[DISCOVER_RELATIONSHIPS] Relationship discovery failed:', error);
            throw new Error(`Failed to discover relationships: ${error?.message || 'Unknown error'}`);
        }
    }
}
