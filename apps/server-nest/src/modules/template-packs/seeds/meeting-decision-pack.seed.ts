...
const objectTypeSchemas = {
    Meeting: {
        type: 'object',
        description: 'A specific meeting event with attendees, agenda, and outcomes',
    ...
    Decision: {
        ...
        properties: {
            ...
            decision_type: {
                type: 'string',
                enum: ['strategic', 'tactical', 'technical', 'organizational', 'process', 'product', 'other'],
                description: 'Type of decision',
            },
            status: {
                type: 'string',
                enum: ['proposed', 'under-review', 'approved', 'rejected', 'implemented', 'reversed'],
    ...
    Question: {
        ...
        properties: {
            ...
            question_type: {
                type: 'string',
                enum: ['clarification', 'technical', 'strategic', 'process', 'decision-support', 'blocker', 'other'],
                description: 'Type of question',
            },
            status: {
                type: 'string',
                enum: ['open', 'under-investigation', 'answered', 'deferred', 'no-longer-relevant'],
...
const relationshipTypeSchemas = {
    ...
    ATTENDED_BY: {
        ...
        attributes: {
            ...
            role: {
                type: 'string',
                enum: ['organizer', 'required', 'optional', 'facilitator', 'note-taker', 'presenter'],
            },
            response_date: {
                type: 'string',
                format: 'date-time',
...
const uiConfigs = {
    Meeting: {
        ...
        cardFields: ['title', 'meeting_type', 'scheduled_start', 'agenda', 'status'],
    },
    MeetingSeries: {
    ...
    Decision: {
        ...
        color: '#10b981',
        defaultView: 'card',
        listFields: ['title', 'decision_type', 'status', 'decision_date', 'priority'],
        cardFields: ['title', 'decision_type', 'rationale', 'status', 'impact_scope'],
    },
    ActionItem: {
...
const extractionPrompts = {
    Meeting: {
        system: 'Extract meeting information from text including title, type, date, attendees, agenda, and notes.',
        user: 'Given this text about a meeting, extract structured meeting information.',
    },
    Decision: {
        system: 'Extract decision information including title, rationale, type, and alternatives considered.',
        user: 'Given this text about a decision, extract structured decision information.',
    },
    ActionItem: {
        system: 'Extract action items including what needs to be done, who is responsible, and when it is due.',
        user: 'Given this text, extract any action items mentioned.',
    },
    Question: {
        system: 'Extract questions raised including the question itself, context, and any answers provided.',
...
async function seedMeetingDecisionPack(pool: Pool): Promise<void> {
    ...
    try {
        ...
        if (existing.rows.length > 0) {
            console.log('Meeting & Decision Management pack already exists. Updating...');
        ...
        console.log('- 5 Object Types: Meeting, MeetingSeries, Decision, ActionItem, Question');
