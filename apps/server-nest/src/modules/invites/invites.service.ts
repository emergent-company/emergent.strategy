...
interface InviteRow { id: string; org_id: string; project_id: string | null; email: string; role: string; status: string; token: string; }
...
export class InvitesService {
    ...
    async create(orgId: string, role: string, email: string, projectId?: string | null) {
        ...
        const res = await this.db.query<InviteRow>(
            `INSERT INTO kb.invites(org_id, project_id, email, role, token) VALUES($1,$2,$3,$4,$5) RETURNING id, org_id, project_id, email, role, status, token`,
            [orgId, projectId || null, email.toLowerCase(), role]
        );
        const r = res.rows[0];
        return { id: r.id, orgId: r.org_id, projectId: r.project_id, email: r.email, role: r.role, status: r.status, token: r.token };
    }

    async accept(token: string, userId: string) {
        const res = await this.db.query<InviteRow>(`SELECT id, org_id, project_id, email, role, status, token FROM kb.invites WHERE token = $1`, [token]);
        ...
        try {
            ...
            if (invite.project_id) {
                ...
                await client.query(`INSERT INTO kb.organization_memberships(org_id, subject_id, role) VALUES($1,$2,'org_admin') ON CONFLICT (org_id, subject_id) DO NOTHING`, [invite.org_id, userId]);
            } else {
