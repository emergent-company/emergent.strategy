...
export interface DiffSummary {
    ...
    removed?: string[];
    updated?: Record<string, { from?: any; to?: any; from_hash?: string; to_hash?: string; truncated?: boolean }>;
    paths: string[];
    meta: {
        added: number;
        removed: number;
...
function hashValue(value: any): string {
    ...
    return 'sha256:' + createHash('sha256').update(json).digest('hex').substring(0, 16);
}
...
function valuesEqual(a: any, b: any, opts: Required<DiffOptions>): boolean {
    ...
    // Float tolerance
    if (typeof a === 'number' && typeof b === 'number' && opts.floatTolerance > 0) {
...
function diffObjects(
    ...
    for (const key of sortedKeys) {
        const path = basePath === '' ? `/${escapePointer(key)}` : `${basePath}/${escapePointer(key)}`;
        ...
        if (!(key in oldObj)) {
            ...
            const shouldTreatAsAtomic = (
                oldValType === 'object' && newValType === 'object' &&
            ...
            } else if (oldValType === 'object' && newValType === 'object' && !Array.isArray(oldVal) && !Array.isArray(newVal)) {
...
function diffArrays(
    ...
    for (let i = 0; i < maxLen; i++) {
        ...
        if (i >= newArr.length) {
            ...
            if (oldType === 'object' && newType === 'object' && !Array.isArray(oldVal) && !Array.isArray(newVal)) {
                diffObjects(oldVal, newVal, path, result, opts);
...
export function generateDiff(
    ...
    const added = result.added.size > 0 ? Object.fromEntries(result.added) : undefined;
    const removed = result.removed.size > 0 ? Array.from(result.removed).sort() : undefined;
    const updated = result.updated.size > 0 ? Object.fromEntries(result.updated) : undefined;
 ...
 */
export function computeContentHash(properties: Record<string, any> | null | undefined): Buffer {
    const props = properties || {};
    // Sort keys for deterministic hash
    const sorted = Object.keys(props).sort().reduce((acc, key) => {
        acc[key] = props[key];
        return acc;
 ...
 */
export function isNoOpChange(changeSummary: DiffSummary | null | undefined): boolean {
    if (!changeSummary) return true;
    return changeSummary.meta.added === 0 && changeSummary.meta.removed === 0 && changeSummary.meta.updated === 0;
}

/**
 * Extract all changed paths from change summary
 */
export function extractChangedPaths(changeSummary: DiffSummary | null | undefined): string[] {
