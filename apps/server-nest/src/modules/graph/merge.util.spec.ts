...
function mkRow(kind: 'target' | 'source', id: string, type: string, key: string, canonical: string, hash: string): MergeHeadRow {
    return kind === 'target' ? {
...
describe('pairIndependentHeads', () => {
    ...
    it('does not duplicate when already paired (same canonical id)', () => {
        const paired: MergeHeadRow[] = [{
            canonical_id: 'c1',
            target_id: 't1', target_type: 'doc', target_key: 'A', target_hash: 'h1',
            source_id: 's1', source_type: 'doc', source_key: 'A', source_hash: 'h1'
        }];
