 ...
 */
export function isSensitiveFieldName(fieldName: string, customPatterns?: RegExp[]): boolean {
 ...
 */
export function extractSensitivityMetadata(value: any): SensitivityMetadata | undefined {
    if (!value || typeof value !== 'object') {
        return undefined;
    }

    // Check for sensitivity metadata in the object
    if ('is_sensitive' in value || 'required_scope' in value || 'sensitivity_reason' in value || 'classification' in value) {
        return {
 ...
 */
export function shouldRedactField(metadata: SensitivityMetadata | undefined, userScopes: Set<string>): boolean {
    if (!metadata) {
        return false;
    }
...
export function redactObject<T extends Record<string, any>>(
    ...
    const redactValue = (value: any, fieldPath: string): any => {
        ...
        const metadata = enableMetadata ? extractSensitivityMetadata(value) : undefined;
