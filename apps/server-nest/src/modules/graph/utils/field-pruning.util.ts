import { TraversalNode, TraversalEdge, GraphTraversalResult } from '../graph.types';
 ...
 */
export function pruneNode(node: TraversalNode, strategy: FieldStrategy): TraversalNode {
    ...
    if (node.phaseIndex !== undefined && FIELD_SALIENCE['node.phaseIndex'] >= threshold) {
 ...
 */
export function pruneEdge(edge: TraversalEdge, strategy: FieldStrategy): TraversalEdge {
 ...
 */
export function pruneResultMetadata(result: GraphTraversalResult, strategy: FieldStrategy): GraphTraversalResult {
    if (strategy === FieldStrategy.FULL) {
        return result;
    }
    ...
    if ('total_nodes' in result && FIELD_SALIENCE['result.total_nodes'] >= threshold) {
        pruned.total_nodes = result.total_nodes;
    }

    if ('has_next_page' in result && FIELD_SALIENCE['result.has_next_page'] >= threshold) {
        pruned.has_next_page = result.has_next_page;
    }

    if ('has_previous_page' in result && FIELD_SALIENCE['result.has_previous_page'] >= threshold) {
        pruned.has_previous_page = result.has_previous_page;
    }

    if ('next_cursor' in result && FIELD_SALIENCE['result.next_cursor'] >= threshold) {
        pruned.next_cursor = result.next_cursor;
    }

    if ('previous_cursor' in result && FIELD_SALIENCE['result.previous_cursor'] >= threshold) {
        pruned.previous_cursor = result.previous_cursor;
    }

    if ('approx_position_start' in result && FIELD_SALIENCE['result.approx_position_start'] >= threshold) {
        pruned.approx_position_start = result.approx_position_start;
    }

    if ('approx_position_end' in result && FIELD_SALIENCE['result.approx_position_end'] >= threshold) {
        pruned.approx_position_end = result.approx_position_end;
    }

    if ('page_direction' in result && FIELD_SALIENCE['result.page_direction'] >= threshold) {
 ...
 */
export function pruneGraphResult(result: GraphTraversalResult, strategy: FieldStrategy = FieldStrategy.FULL): GraphTraversalResult {
    if (strategy === FieldStrategy.FULL) {
        return result;
    }
