...
export class RedactionInterceptor implements NestInterceptor {
            ...
            map(data => {
                ...
                try {
                    ...
                    if (this.config.logRedactions && redactedData.redactionCount > 0) {
                        this.logRedactionEvent(request, user, redactedData.redactionCount, redactedData.redactedFields);
                    }
     ...
     */
    private redactResponseData(data: any, config: RedactionConfig): { data: any; redactionCount: number; redactedFields: string[] } {
