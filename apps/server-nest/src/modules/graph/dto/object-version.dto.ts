...
export class ObjectVersionDto {
    ...
    @ApiProperty({ description: 'ID of the previous version (null for v1)', required: false })
    supersedes_id?: string;

    @ApiProperty({ description: 'Canonical ID (same for all versions of this object)' })
    canonical_id!: string;

    @ApiProperty({ description: 'Object type' })
    type!: string;

    @ApiProperty({ description: 'Object key (business identifier)', required: false })
    key?: string;
    ...
    @ApiProperty({ description: 'Content hash for duplicate detection', required: false })
    content_hash?: string;

    @ApiProperty({ description: 'When this version was created' })
    created_at!: string;

    @ApiProperty({ description: 'When this version was soft-deleted', required: false })
    deleted_at?: string | null;

    @ApiProperty({ description: 'User who created this version', required: false })
    created_by?: string;

    @ApiProperty({ description: 'Extraction job that created this version', required: false })
