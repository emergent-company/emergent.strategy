...
export class HistoryResponseDto<T> {
    ...
    @ApiPropertyOptional({ description: 'Present when more pages available; pass as cursor query param' }) next_cursor?: string;
}

export class ObjectHistoryResponseDto extends HistoryResponseDto<VersionedObjectDto> {
    @ApiProperty({ type: [VersionedObjectDto] }) declare items: VersionedObjectDto[];
}

export class RelationshipHistoryResponseDto extends HistoryResponseDto<VersionedRelationshipDto> {
    @ApiProperty({ type: [VersionedRelationshipDto] }) declare items: VersionedRelationshipDto[];
}

export class HistoryQueryDto {
    @ApiPropertyOptional({ minimum: 1, maximum: 200, default: 20 }) limit?: number;
    @ApiPropertyOptional({ description: 'Opaque cursor (version) for descending pagination' }) cursor?: string;
}
