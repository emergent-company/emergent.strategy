...
describe('Graph FTS Search', () => {
    ...
    it('returns ranked matches for query term', async () => {
        ...
        await (service as any).db.query(`DELETE FROM kb.graph_objects WHERE key LIKE $1`, ['a_' + suffix + '%']);
        await (service as any).db.query(`DELETE FROM kb.graph_objects WHERE key LIKE $1`, ['b_' + suffix + '%']);
        await (service as any).db.query(`DELETE FROM kb.graph_objects WHERE key LIKE $1`, ['c_' + suffix + '%']);
        const a = await service.createObject({ ...base, type: 'doc', key: 'a_' + suffix, properties: { title: 'Alpha Node', description: 'Contains unique ' + uniqueToken + ' token' }, labels: ['test'] });
        await service.createObject({ ...base, type: 'doc', key: 'b_' + suffix, properties: { title: 'Beta Node', description: 'Mentions nothing special' }, labels: ['test'] });
        const c = await service.createObject({ ...base, type: 'doc', key: 'c_' + suffix, properties: { title: 'Gamma Node', notes: uniqueToken + ' appears twice: ' + uniqueToken }, labels: ['test'] });
