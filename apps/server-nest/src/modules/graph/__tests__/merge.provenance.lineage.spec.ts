...
class MockDb implements Partial<DatabaseService> {
    ...
    async query(sql: string, params: any[] = []): Promise<any> {
        ...
        if (norm.startsWith('select ancestor_branch_id from kb.branch_lineage where branch_id')) {
            const bid = params[0];
            const rows = Array.from(this.lineage[bid] || []).map(a => ({ ancestor_branch_id: a }));
            return { rows, rowCount: rows.length };
        }
        if (norm.startsWith('with tgt as')) {
            ...
            for (const t of tgtHeads) {
                map.set(t.canonical_id, { canonical_id: t.canonical_id, target_id: t.id, target_hash: t.content_hash, target_change: t.change_summary, target_props: t.properties, target_type: t.type, target_key: t.key });
            }
        ...
        if (norm.startsWith('select distinct on (canonical_id) canonical_id, id, type, key, content_hash')) {
            const branchId = params[0];
            const rows = this.objects.filter(o => o.branch_id === branchId).map(o => ({
        ...
        if (norm.includes('from kb.graph_objects where branch_id = $1') && norm.includes('full outer join')) {
            const tgt = params[0];
            const src = params[1];
        ...
        if (norm.startsWith('insert into kb.merge_provenance')) {
            this.provenance.push({ child_version_id: params[0], parent_version_id: params[1], role: sql.includes("'source'") ? 'source' : sql.includes("'target'") ? 'target' : sql.includes("'base'") ? 'base' : 'unknown' });
        ...
        if (norm.startsWith('update kb.graph_objects set properties')) {
            ...
            if (existing) {
                ...
                const newRow = { ...existing, id: 'obj_' + (this.objects.length + 1), properties: merged, content_hash: Buffer.from(existing.key + '_patched'), change_summary: { paths: Object.keys(params[0]) } };
...
describe('merge lineage + provenance', () => {
    let svc: GraphService; let db: MockDb; let targetObjId: string; let sourceObjId: string;
    beforeEach(() => { ({ service: svc, db, targetObjId, sourceObjId } = serviceWithTwoObjects()); });

    it('classifies fast-forward via lineage ancestor and applies provenance on execute', async () => {
        const summary = await svc.mergeBranchDryRun('b_main', { sourceBranchId: 'b_feature', execute: true });
