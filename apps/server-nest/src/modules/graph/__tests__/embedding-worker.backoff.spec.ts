...
describe('EmbeddingWorkerService Backoff', () => {
    let service: GraphService; let jobs: EmbeddingJobsService; let worker: EmbeddingWorkerService; let objectId: string; let jobId: string;
    ...
    it('requeues job with incremented attempt_count and future scheduled_at', async () => {
        const before = await (service as any).db.query(`SELECT attempt_count, status FROM kb.graph_embedding_jobs WHERE id=$1`, [jobId]);
        ...
        for (let i = 0; i < 5; i++) {
            ...
            const probe = await (service as any).db.query(`SELECT attempt_count, status, scheduled_at, last_error FROM kb.graph_embedding_jobs WHERE id=$1`, [jobId]);
            if (probe.rows[0].attempt_count > 0) { incremented = true; break; }
        }
        if (!incremented) {
            const dump = await (service as any).db.query(`SELECT id, attempt_count, status, scheduled_at, last_error FROM kb.graph_embedding_jobs WHERE id=$1`, [jobId]);
            throw new Error('attempt_count_never_incremented: ' + JSON.stringify(dump.rows[0]));
        }
        const after = await (service as any).db.query(`SELECT attempt_count, status, scheduled_at FROM kb.graph_embedding_jobs WHERE id=$1`, [jobId]);
