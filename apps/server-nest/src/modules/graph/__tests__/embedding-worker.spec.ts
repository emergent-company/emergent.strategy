...
describe('EmbeddingWorkerService', () => {
    let graph: GraphService; let jobs: EmbeddingJobsService; let db: DatabaseService; let worker: EmbeddingWorkerService; let objectId: string;
    ...
    beforeAll(async () => {
        ...
        const pendingJob = await db.query(`SELECT id FROM kb.graph_embedding_jobs WHERE object_id=$1 AND status='pending'`, [objectId]);
    ...
    it('processes a pending embedding job and populates embedding column', async () => {
        ...
        let success = false; let lastProbe: any = null;
        for (let i = 0; i < 5; i++) {
            await worker.processBatch();
            const probe = await db.query<{ embedding: any }>(`SELECT embedding FROM kb.graph_objects WHERE id=$1`, [objectId]);
            lastProbe = probe;
            if (probe.rowCount === 1 && probe.rows[0].embedding) { success = true; break; }
        }
        if (!success) {
            const jobState = await db.query(`SELECT id, status, attempt_count, last_error FROM kb.graph_embedding_jobs WHERE object_id=$1`, [objectId]);
            throw new Error('embedding_not_populated ' + JSON.stringify({ jobState: jobState.rows, lastProbe: lastProbe.rows }));
        }
