...
export class BranchService {
    ...
    async ensureBranchLineage(branchId: string): Promise<void> {
        ...
        try {
            ...
            const branchRes = await client.query<{ id: string; parent_branch_id: string | null }>(`SELECT id, parent_branch_id FROM kb.branches WHERE id=$1`, [branchId]);
            if (!branchRes.rowCount) { await client.query('ROLLBACK'); return; }
            const parentId = branchRes.rows[0].parent_branch_id;
            await client.query(`INSERT INTO kb.branch_lineage(branch_id, ancestor_branch_id, depth) VALUES ($1,$1,0) ON CONFLICT DO NOTHING`, [branchId]);
            if (parentId) {
                await client.query(`INSERT INTO kb.branch_lineage(branch_id, ancestor_branch_id, depth) VALUES ($1,$1,0) ON CONFLICT DO NOTHING`, [parentId]);
                await client.query(`INSERT INTO kb.branch_lineage(branch_id, ancestor_branch_id, depth)
                    SELECT $1, ancestor_branch_id, depth + 1 FROM kb.branch_lineage WHERE branch_id=$2
                    ON CONFLICT (branch_id, ancestor_branch_id) DO NOTHING`, [branchId, parentId]);
                await client.query(`INSERT INTO kb.branch_lineage(branch_id, ancestor_branch_id, depth) VALUES ($1,$2,1) ON CONFLICT DO NOTHING`, [branchId, parentId]);
            }
    ...
    async create(dto: CreateBranchDto): Promise<BranchRow> {
        ...
        if (parent_branch_id) {
            const parent = await this.db.query<{ id: string }>(`SELECT id FROM kb.branches WHERE id=$1`, [parent_branch_id]);
            if (!parent.rowCount) throw new NotFoundException('parent_branch_not_found');
        }
        ...
        try {
         ...
         RETURNING id, organization_id, project_id, name, parent_branch_id, created_at`,
            ...
            try {
                await client.query(`INSERT INTO kb.branch_lineage(branch_id, ancestor_branch_id, depth) VALUES ($1,$1,0) ON CONFLICT DO NOTHING`, [row.id]);
            } catch { /* ignore if table missing (older schema) */ }
            // If parent exists, copy its ancestor chain + itself with incremented depth
            if (parent_branch_id) {
                try {
                    // Ensure parent self row exists (defensive)
                    await client.query(`INSERT INTO kb.branch_lineage(branch_id, ancestor_branch_id, depth) VALUES ($1,$1,0) ON CONFLICT DO NOTHING`, [parent_branch_id]);
                    ...
                    await client.query(`INSERT INTO kb.branch_lineage(branch_id, ancestor_branch_id, depth)
                        ...
                        ON CONFLICT (branch_id, ancestor_branch_id) DO NOTHING`, [row.id, parent_branch_id]);
                    // Also add direct parent (depth 1) if not already captured
                    await client.query(`INSERT INTO kb.branch_lineage(branch_id, ancestor_branch_id, depth)
                        VALUES ($1,$2,1) ON CONFLICT (branch_id, ancestor_branch_id) DO NOTHING`, [row.id, parent_branch_id]);
                } catch { /* ignore lineage errors to avoid blocking branch creation */ }
            }
            await client.query('COMMIT');
            return row;
        } catch (e) {
