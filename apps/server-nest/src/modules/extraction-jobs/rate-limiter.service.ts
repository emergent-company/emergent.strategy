...
export class RateLimiterService {
    ...
    async tryConsume(estimatedTokens: number = 1000): Promise<boolean> {
        ...
        if (this.llmTokens < estimatedTokens) {
            this.logger.warn(
                `Rate limit exceeded: tokens per minute (need ${estimatedTokens}, have ${Math.floor(this.llmTokens)})`
            );
            return false;
        }
        ...
        this.logger.debug(
            ...
            `Remaining: ${Math.floor(this.requestTokens)} requests, ${Math.floor(this.llmTokens)} tokens`
        );
    ...
    private getTimeUntilRefill(): number {
        ...
        const requestTimeUntilRefill = Math.max(
            0,
            60000 - requestTimeSinceRefill
        );

        // Calculate time until next token refill
        const llmTimeSinceRefill = now - this.lastLlmTokenRefill;
        const llmTimeUntilRefill = Math.max(
            0,
            60000 - llmTimeSinceRefill
        );
