...
export class ExtractionWorkerService implements OnModuleInit, OnModuleDestroy {
                                ...
                                if (duplicateStrategy === 'skip') {
                                    ...
                                    try {
                                        ...
                                        if (existingResult.rowCount === 0) {
                                            throw new Error('Object not found after duplicate detection');
                                        }
                                        ...
                                        await this.extractionLogger.logStep({
                                            ...
                                            outputData: {
                                                ...
                                                properties_added: Object.keys(entity.properties || {}).length,
                                                duplicate_strategy: duplicateStrategy,
                                            },
                                            durationMs: Date.now() - objectCreationStartTime,
                                            metadata: {
                                        ...
                                        this.logger.debug(
                                            ...
                                            `(v${existingObject.version} → v${updatedObject.version}, confidence: ${finalConfidence.toFixed(3)})`
                                        );
                                        outcome = 'merged';
                                    } catch (mergeError) {
                                        const mergeErr = mergeError instanceof Error ? mergeError : new Error(String(mergeError));
                                        this.logger.error(
    ...
    private async loadExtractionConfig(job: ExtractionJobDto): Promise<{
        ...
        for (const row of result.rows) {
            ...
            for (const [key, value] of Object.entries(extractionPrompts)) {
                if (typeof value === 'string') {
                    ...
                    this.logger.debug(`[loadExtractionConfig] Added extraction prompt key: ${key} from ${packName}`);
                }
            }
            ...
            for (const [typeName, schema] of Object.entries(objectSchemas)) {
                ...
                if (mergedObjectSchemas[typeName]) {
                    ...
                    this.logger.debug(`[loadExtractionConfig] Merging schema for type: ${typeName} (from ${packName})`);
                    ...
                    this.logger.debug(`[loadExtractionConfig] Adding schema for type: ${typeName} (from ${packName})`);
                    mergedObjectSchemas[typeName] = {
                        ...(schema as Record<string, any>),
        ...
        this.logger.log(`Using schema-based extraction with ${Object.keys(mergedObjectSchemas).length} object type(s)`);
