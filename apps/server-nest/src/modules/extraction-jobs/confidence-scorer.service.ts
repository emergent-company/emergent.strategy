...
export class ConfidenceScorerService {
    ...
    private readonly WEIGHTS = {
        ...
        evidenceQuality: 0.20,    // Description and property richness
        propertyQuality: 0.20,    // Property value quality
    };
     ...
     */
    calculateConfidence(entity: ExtractedEntity, allowedTypes?: string[]): number {
        try {
            ...
            this.logger.debug(
                ...
                `Evidence: ${evidenceScore.toFixed(2)}, Property: ${propertyScore.toFixed(2)})`
            );

            return finalScore;

        } catch (error) {
            this.logger.warn(`Failed to calculate confidence for ${entity.name}`, error);
     ...
     * @returns Score 0.0-1.0
     */
    private scoreSchemaCompleteness(entity: ExtractedEntity, allowedTypes?: string[]): number {
        let score = 1.0;
     ...
     */
    getConfidenceBreakdown(entity: ExtractedEntity, allowedTypes?: string[]): {
        total: number;
        components: {
