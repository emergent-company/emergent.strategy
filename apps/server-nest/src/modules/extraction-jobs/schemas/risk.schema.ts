...
export const RiskSchema = BaseExtractedEntitySchema.extend({
    ...
    risk_type: z.enum([
        ...
        'resource',
        'other'
    ]).optional().describe('Type of risk'),
    ...
    severity: z.enum(['low', 'medium', 'high', 'critical']).optional().describe('Severity if realized'),

    probability: z.enum(['low', 'medium', 'high']).optional().describe('Probability of occurrence'),

    impact: z.string().optional().describe('Potential impact if realized'),

    mitigation_strategy: z.string().optional().describe('How to mitigate or prevent this risk'),
