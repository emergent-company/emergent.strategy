...
export const FeatureSchema = BaseExtractedEntitySchema.extend({
    ...
    feature_type: z.enum([
        ...
        'technical',
        'other'
    ]).optional().describe('Type of feature'),
    ...
    status: z.enum([
        ...
        'deprecated',
        'cancelled'
    ]).optional().describe('Current status'),

    priority: z.enum(['low', 'medium', 'high', 'critical']).optional().describe('Priority level'),

    complexity: z.enum(['trivial', 'low', 'medium', 'high', 'very-high']).optional().describe('Estimated complexity'),

    user_story: z.string().optional().describe('User story format (As a... I want... So that...)'),

    acceptance_criteria: z.string().optional().describe('Criteria for feature completion'),

    estimated_effort: z.string().optional().describe('Estimated effort (hours, days, story points)'),
