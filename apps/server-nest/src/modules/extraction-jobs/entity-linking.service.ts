...
export class EntityLinkingService {
    ...
    async findSimilarObject(
        ...
        if (strategy === 'vector_similarity') {
            ...
            if (keyMatch) {
                this.logger.debug(`Vector strategy: found exact key match for ${entity.name}`);
                return keyMatch;
            }

            // Fall back to semantic similarity search
            if (this.embeddings.isEnabled()) {
                const vectorMatch = await this.findByVectorSimilarity(entity, projectId);
                if (vectorMatch) {
                    this.logger.debug(`Vector strategy: found semantic match for ${entity.name}`);
                    return vectorMatch;
                }
            } else {
                this.logger.warn('Vector similarity requested but embeddings service is disabled');
            }
    ...
    private async findByKeyMatch(
        ...
        if (entity.business_key) {
            const exactMatch = await this.findByExactKey(projectId, typeName, entity.business_key);
            if (exactMatch) {
                this.logger.debug(`Found exact key match for ${entity.name}: ${exactMatch}`);
                return exactMatch;
            }
        }
        ...
        if (normalizedName) {
            const nameMatch = await this.findByNormalizedName(projectId, typeName, normalizedName);
            if (nameMatch) {
                this.logger.debug(`Found normalized name match for ${entity.name}: ${nameMatch}`);
                return nameMatch;
            }
        }
        ...
        if (extractedKey) {
            const propertyMatch = await this.findByExactKey(projectId, typeName, extractedKey);
            if (propertyMatch) {
                this.logger.debug(`Found property-based key match for ${entity.name}: ${propertyMatch}`);
    ...
    private async findByVectorSimilarity(
        ...
        try {
            ...
            // Note: embedding_v1 uses 768-dimensional vectors (Gemini text-embedding-004)
            const result = await this.db.query<{ id: string; similarity: number }>(
                `SELECT id, 
            ...
            this.logger.error(`Vector similarity search failed: ${errorMessage}`, errorStack);
            return null;
        }
    }
    ...
    private generateEntityText(entity: ExtractedEntity): string {
        ...
        if (entity.properties) {
            const keyProps = ['id', 'identifier', 'code', 'reference', 'name', 'title'];
    ...
    /**
     * Find object by exact key match
     * 
     * @param projectId - Project scope
    ...
    /**
     * Find object by normalized name match
     * 
    ...
    async decideMergeAction(
        ...
        projectId: string,
        strategy: 'key_match' | 'vector_similarity' | 'always_new'
    ): Promise<{ action: 'skip' | 'merge' | 'create'; existingObjectId?: string }> {
        if (strategy === 'always_new') {
            return { action: 'create' };
        }

        const existingObjectId = await this.findSimilarObject(entity, projectId, strategy);
        ...
        try {
            ...
            this.logger.warn(`Object ${existingObjectId} not found (may have been deleted)`);
            return { action: 'create' };
        }

        const overlap = this.calculatePropertyOverlap(entity, existingObject.properties || {});

        // If high overlap (>90%), skip creation (already exists)
        if (overlap > 0.9) {
            this.logger.debug(
                `Skipping entity ${entity.name}: high overlap (${(overlap * 100).toFixed(0)}%) ` +
                `with existing object ${existingObjectId}`
            );
        ...
        this.logger.debug(
            `Merging entity ${entity.name}: partial overlap (${(overlap * 100).toFixed(0)}%) ` +
            `with existing object ${existingObjectId}`
        );
