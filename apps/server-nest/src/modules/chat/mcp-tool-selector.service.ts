...
export class McpToolSelectorService {
    ...
    async selectTool(
        ...
        try {
            this.logger.debug(`Selecting MCP tool for message: "${userMessage.substring(0, 50)}..."`);
    ...
    private buildToolSelectionPrompt(
        ...
        const entityTypesList = entityTypes.length > 0
            ? entityTypes.map(t => `  - **${t.name}** (${t.count} instances): ${t.description}`).join('\n')
...
**Respond with ONLY the JSON object, no other text.**`;
    ...
    private parseToolSelection(content: string): ToolSelectionResult {
        try {
            ...
            if (typeof parsed.shouldUseMcp !== 'boolean' ||
