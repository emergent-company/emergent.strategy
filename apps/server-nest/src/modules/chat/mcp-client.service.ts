...
export class McpClientService {
    ...
    async initialize(config: McpClientConfig): Promise<void> {
        ...
        try {
            ...
            this.logger.debug(
                `MCP client initialized. Server: ${response.serverInfo?.name || 'unknown'}, ` +
            ...
            this.logger.error(`Failed to initialize MCP client: ${err.message}`, err.stack);
            throw error;
        }
    }
    ...
    async callTool(
        ...
        this.logger.debug(`Calling tool: ${toolName} with args: ${JSON.stringify(toolArguments)}`);
    ...
    private async sendRequest(method: string, params?: any): Promise<any> {
        ...
        this.logger.debug(`Sending JSON-RPC request: ${method} (id: ${request.id})`);
