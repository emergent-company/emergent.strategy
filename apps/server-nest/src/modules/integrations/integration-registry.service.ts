...
export class IntegrationRegistryService implements OnModuleInit {
    ...
    register(integration: BaseIntegration): void {
        ...
        if (this.integrations.has(name)) {
            this.logger.warn(`Integration '${name}' is already registered. Overwriting.`);
        }

        this.integrations.set(name, integration);
        this.logger.log(`Registered integration: ${integration.getDisplayName()} (${name})`);
    }
    ...
    async cleanup(): Promise<void> {
        ...
        for (const [name, integration] of this.integrations.entries()) {
            try {
                ...
                this.logger.error(`Failed to cleanup integration ${name}: ${err.message}`);
            }
        }
