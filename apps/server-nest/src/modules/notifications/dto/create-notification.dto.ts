...
export class CreateNotificationDto {
    ...
    @ApiProperty({ enum: NotificationImportance, default: NotificationImportance.OTHER })
    ...
    @ApiPropertyOptional({ description: 'Notification type (e.g., extraction_complete, extraction_failed)' })
    @IsString()
    @IsOptional()
    type?: string;
    ...
    @ApiPropertyOptional({ description: 'Related resource type (e.g., extraction_job, document)' })
    @IsString()
    @IsOptional()
    ...
    @ApiPropertyOptional({ description: 'Whether notification has been read', default: false })
    @IsBoolean()
    @IsOptional()
    read?: boolean;

    @ApiPropertyOptional({ description: 'Whether notification has been dismissed', default: false })
    @IsBoolean()
    @IsOptional()
    dismissed?: boolean;
    ...
    @ApiPropertyOptional({
        ...
        items: {
            ...
            properties: {
                label: { type: 'string' },
                url: { type: 'string' },
                style: { type: 'string', enum: ['primary', 'secondary', 'warning', 'danger'] }
            }
        }
    })
    @IsArray()
...
export class SnoozeNotificationDto {
    @ApiProperty({ description: 'ISO 8601 datetime string for snooze until time' })
    @IsDateString()
    until!: string;
}
