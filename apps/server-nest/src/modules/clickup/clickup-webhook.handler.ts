...
export class ClickUpWebhookHandler {
    ...
    async handleWebhook(
        ...
        try {
            ...
            this.logger.log(`Received webhook: ${event.event} for integration ${integrationId}`);

            // Route to appropriate handler based on event type
            if (event.event.startsWith('task')) {
                return await this.handleTaskEvent(integrationId, projectId, orgId, event);
            } else if (event.event.startsWith('list')) {
                return await this.handleListEvent(integrationId, projectId, orgId, event);
            } else if (event.event.startsWith('folder')) {
                return await this.handleFolderEvent(integrationId, projectId, orgId, event);
            } else if (event.event.startsWith('space')) {
                return await this.handleSpaceEvent(integrationId, projectId, orgId, event);
            } else {
    ...
    private async handleFolderEvent(
        ...
        this.logger.log(`Processing folder event: ${event.event} for folder ${folderId}`);
    ...
    private async handleSpaceEvent(
        ...
        this.logger.log(`Processing space event: ${event.event} for space ${spaceId}`);
     ...
     */
    private async updateLastWebhookProcessed(integrationId: string, entityId: string): Promise<void> {
        try {
            await this.db.query(
