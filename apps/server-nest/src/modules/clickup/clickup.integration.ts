...
import { BaseIntegration, IntegrationCapabilities, ImportConfig, ImportResult, WebhookPayload } from '../integrations/base-integration';
import { IntegrationDto, IntegrationSettings } from '../integrations/dto/integration.dto';
...
export class ClickUpIntegration extends BaseIntegration {
    ...
    protected async onConfigure(): Promise<void> {
        ...
        this.logger.log(`Configured ClickUp integration for workspace ${this.clickupSettings.workspace_id}`);
    }
    ...
    protected async onValidateConfiguration(): Promise<boolean> {
        ...
        try {
            ...
            const workspace = response.teams.find(t => t.id === this.clickupSettings!.workspace_id);
            if (!workspace) {
                throw new Error(
                    `Workspace ${this.clickupSettings.workspace_id} not found. ` +
                    `Available workspaces: ${response.teams.map(t => `${t.name} (${t.id})`).join(', ')}`
                );
            }

            this.logger.log(`Successfully connected to ClickUp workspace: ${workspace.name}`);
            return true;
        } catch (error) {
     ...
     */
    protected async onRunFullImport(config?: ImportConfig): Promise<ImportResult> {
        if (!this.integration || !this.clickupSettings) {
            throw new Error('Integration not configured');
        }

        this.logger.log(`Starting full import from ClickUp workspace ${this.clickupSettings.workspace_id}`);
     ...
     */
    protected async verifyWebhookSignature(payload: WebhookPayload): Promise<{ valid: boolean; error?: string }> {
