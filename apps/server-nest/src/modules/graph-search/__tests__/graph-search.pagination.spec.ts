...
import { GraphSearchRepository } from '../../graph-search/graph-search.repository';
import { EmbeddingService } from '../../graph-search/embedding.service';

class StubEmbedding extends EmbeddingService {
    async embedQuery(q: string): Promise<number[]> { return Array.from({ length: 8 }, (_, i) => (q.length + i) / 10); }
}

describe('GraphSearchService pagination', () => {
    let service: GraphSearchService;
    beforeEach(() => {
        service = new GraphSearchService(new GraphSearchRepository(), new StubEmbedding() as any);
    });

    it('returns first page with nextCursor and hasNext when more results exist', async () => {
        const res = await service.search({ query: 'abcdef', pagination: { limit: 2 } } as any, { debug: false, scopes: [] });
    ...
    it('paginates forward using cursor without overlap', async () => {
        const first = await service.search({ query: 'abcdef', pagination: { limit: 2 } } as any, { debug: false, scopes: [] });
        const second = await service.search({ query: 'abcdef', pagination: { limit: 2, cursor: first.meta.nextCursor! } } as any, { debug: false, scopes: [] });
    ...
    it('cursor past end yields boundary page (may be empty) and hasPrev=true but hasNext=false (index-based)', async () => {
        const first = await service.search({ query: 'abcdef', pagination: { limit: 5 } } as any, { debug: false, scopes: [] });
        const last = first.items[first.items.length - 1];
        const beyondCursor = Buffer.from(JSON.stringify({ s: Number(last.score.toFixed(6)), id: last.object_id + 'zzz' })).toString('base64url');
        const boundary = await service.search({ query: 'abcdef', pagination: { limit: 5, cursor: beyondCursor } } as any, { debug: false, scopes: [] });
    ...
    it('backward from first page returns initial window and hasPrev=false (AT-GSP-15)', async () => {
        const backwardFirst = await service.search({ query: 'abcdef', pagination: { limit: 3, direction: 'backward' } } as any, { debug: false, scopes: [] });
    ...
    it('unknown id backward cursor falls back gracefully (AT-GSP-17)', async () => {
        ...
        const unknownCursor = Buffer.from(JSON.stringify({ s: approxScore, id: fakeId })).toString('base64url');
        const page = await service.search({ query: 'abcdef', pagination: { limit: 2, cursor: unknownCursor, direction: 'backward' } } as any, { debug: false, scopes: [] });
    ...
    it('mixed direction (forward chain) produces strictly contiguous, non-overlapping forward windows (AT-GSP-16 simplified)', async () => {
        const forward1 = await service.search({ query: 'abcdef', pagination: { limit: 3 } } as any, { debug: false, scopes: [] });
        if (!forward1.meta.nextCursor) return; // insufficient data
        const forward2 = await service.search({ query: 'abcdef', pagination: { limit: 3, cursor: forward1.meta.nextCursor } } as any, { debug: false, scopes: [] });
    ...
    it('exposes approx position range for pages (AT-GSP-19)', async () => {
        const page1 = await service.search({ query: 'abcdef', pagination: { limit: 3 } } as any, { debug: false, scopes: [] });
        ...
        if (page1.meta.nextCursor) {
            const page2 = await service.search({ query: 'abcdef', pagination: { limit: 3, cursor: page1.meta.nextCursor } } as any, { debug: false, scopes: [] });
