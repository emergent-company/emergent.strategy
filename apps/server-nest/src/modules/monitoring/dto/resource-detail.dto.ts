...
export class LogEntryDto {
    ...
    @ApiProperty({ example: 'info', enum: ['debug', 'info', 'warn', 'error', 'fatal'] })
    level!: string;

    @ApiProperty({ example: 'Job processing started' })
    message!: string;

    @ApiProperty({ required: false, example: { step: 'extraction', entity_count: 5 } })
...
export class LLMCallDto {
    ...
    @ApiProperty({ example: 'success', enum: ['success', 'error', 'timeout', 'pending'] })
    status!: string;
...
export class ResourceDetailResponseDto {
    ...
    @ApiProperty({ required: false, example: { totalCost: 0.0125, avgDuration: 5000 } })
