...
--
CREATE FUNCTION kb.delete_old_cleared_notifications() RETURNS void LANGUAGE plpgsql AS $$ BEGIN
DELETE FROM
    kb.notifications
WHERE
...
--
CREATE FUNCTION kb.get_object_revision_count(p_object_id uuid) RETURNS integer LANGUAGE plpgsql STABLE AS $$ DECLARE v_canonical_id UUID;
...
--
CREATE FUNCTION kb.get_project_active_types(p_project_id uuid) RETURNS TABLE(
    type text,
    source text,
    json_schema jsonb,
    ui_config jsonb,
    extraction_config jsonb
) LANGUAGE plpgsql STABLE SECURITY DEFINER AS $$ BEGIN RETURN QUERY
SELECT
    ptr.type,
    ptr.source,
    ptr.json_schema,
...
--
CREATE FUNCTION kb.refresh_revision_counts() RETURNS integer LANGUAGE plpgsql SECURITY DEFINER AS $$ DECLARE refresh_start TIMESTAMPTZ;
...
--
CREATE FUNCTION kb.sync_document_org() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF NEW.project_id IS NOT NULL THEN NEW.org_id := (
    SELECT
        org_id
    FROM
        kb.projects
...
--
CREATE FUNCTION kb.touch_updated_at() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN NEW.updated_at := now();
...
--
CREATE FUNCTION kb.wakeup_snoozed_notifications() RETURNS void LANGUAGE plpgsql AS $$ BEGIN
UPDATE
    kb.notifications
SET
    snoozed_until = NULL
...
CREATE TABLE kb.clickup_import_logs (
    ...
    metadata jsonb,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT chk_clickup_import_logs_operation CHECK (
        (
            operation_type = ANY (
                ARRAY ['discovery'::text, 'fetch_spaces'::text, 'fetch_docs'::text, 'fetch_pages'::text, 'store_document'::text, 'create_extraction'::text, 'api_call'::text, 'error'::text]
            )
        )
    ),
    CONSTRAINT chk_clickup_import_logs_status CHECK (
        (
            status = ANY (
                ARRAY ['pending'::text, 'success'::text, 'error'::text, 'warning'::text, 'info'::text]
            )
        )
    )
);
...
CREATE TABLE kb.graph_objects (
    ...
    CONSTRAINT graph_objects_extraction_confidence_check CHECK (
        (
            (extraction_confidence IS NULL)
            OR (
                (extraction_confidence >= (0.0) :: double precision)
                AND (extraction_confidence <= (1.0) :: double precision)
            )
        )
    )
);
...
--
-- Name: COLUMN notifications.user_id; Type: COMMENT; Schema: kb; Owner: -
--
COMMENT ON COLUMN kb.notifications.user_id IS 'Zitadel user ID of the user who receives this notification';
...
CREATE TABLE kb.object_extraction_jobs (
    ...
    CONSTRAINT object_extraction_jobs_job_type_check CHECK (
        (
            job_type = ANY (
                ARRAY ['full_extraction'::text, 'type_discovery'::text, 'reprocessing'::text]
            )
        )
    ),
    ...
    CONSTRAINT object_extraction_jobs_status_check CHECK (
        (
            status = ANY (
                ARRAY ['pending'::text, 'running'::text, 'processing'::text, 'completed'::text, 'requires_review'::text, 'failed'::text, 'cancelled'::text]
            )
        )
    )
);
...
ALTER TABLE
    ONLY kb.object_extraction_jobs FORCE ROW LEVEL SECURITY;
...
CREATE TABLE kb.object_type_suggestions (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT object_type_suggestions_confidence_check CHECK (
        (
    ...
    CONSTRAINT object_type_suggestions_source_check CHECK (
        (
            source = ANY (
                ARRAY ['pattern_analysis'::text, 'user_feedback'::text, 'import'::text]
            )
        )
    ),
    CONSTRAINT object_type_suggestions_status_check CHECK (
        (
            status = ANY (
                ARRAY ['pending'::text, 'accepted'::text, 'rejected'::text, 'merged'::text]
            )
        )
    )
);
...
ALTER TABLE
    ONLY kb.object_type_suggestions FORCE ROW LEVEL SECURITY;
...
CREATE TABLE kb.project_object_type_registry (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT project_object_type_registry_discovery_confidence_check CHECK (
        (
            (discovery_confidence IS NULL)
            OR (
                (discovery_confidence >= (0.0) :: double precision)
                AND (discovery_confidence <= (1.0) :: double precision)
            )
        )
    ),
...
ALTER TABLE
    ONLY kb.project_object_type_registry FORCE ROW LEVEL SECURITY;
...
ALTER TABLE
    ONLY kb.project_template_packs FORCE ROW LEVEL SECURITY;
...
CREATE TABLE kb.system_process_logs (
    ...
    organization_id text,
    project_id uuid,
    CONSTRAINT chk_system_process_logs_level CHECK (
        (
            level = ANY (
...
--
ALTER TABLE
    ONLY kb.clickup_import_logs
ADD
    CONSTRAINT clickup_import_logs_pkey PRIMARY KEY (id);
...
--
ALTER TABLE
    ONLY kb.graph_embedding_coverage
ADD
    CONSTRAINT graph_embedding_coverage_pkey PRIMARY KEY (model_version, organization_id, project_id);
...
ADD
    CONSTRAINT object_extraction_jobs_pkey PRIMARY KEY (id);

--
-- Name: object_extraction_logs object_extraction_logs_pkey; Type: CONSTRAINT; Schema: kb; Owner: -
--
ALTER TABLE
    ONLY kb.object_extraction_logs
ADD
    CONSTRAINT object_extraction_logs_pkey PRIMARY KEY (id);
...
--
ALTER TABLE
    ONLY kb.object_type_suggestions
ADD
    CONSTRAINT object_type_suggestions_pkey PRIMARY KEY (id);
...
ALTER TABLE
    ONLY kb.project_object_type_registry
ADD
    CONSTRAINT project_object_type_registry_project_id_type_key UNIQUE (project_id, type);
...
--
CREATE INDEX idx_clickup_import_logs_errors ON kb.clickup_import_logs USING btree (import_session_id, status)
WHERE
    (status = 'error' :: text);
...
--
CREATE INDEX idx_extraction_jobs_created_by ON kb.extraction_jobs USING btree (created_by, created_at DESC)
WHERE
...
--
CREATE INDEX idx_extraction_jobs_document ON kb.object_extraction_jobs USING btree (document_id)
WHERE
...
--
CREATE INDEX idx_extraction_jobs_progress ON kb.object_extraction_jobs USING btree (status, processed_items, total_items)
WHERE
    (
        status = ANY (ARRAY ['running'::text, 'pending'::text])
    );
...
--
CREATE INDEX idx_extraction_jobs_source ON kb.extraction_jobs USING btree (source_type, source_id)
WHERE
...
--
CREATE INDEX idx_extraction_jobs_status_created ON kb.object_extraction_jobs USING btree (status, created_at)
WHERE
    (
...
--
CREATE INDEX idx_extraction_logs_errors ON kb.object_extraction_logs USING btree (extraction_job_id)
WHERE
    (status = 'error' :: text);
...
--
CREATE INDEX idx_graph_objects_confidence ON kb.graph_objects USING btree (extraction_confidence DESC)
WHERE
...
--
CREATE INDEX idx_graph_objects_extraction_job ON kb.graph_objects USING btree (extraction_job_id)
WHERE
...
--
CREATE INDEX idx_graph_objects_needs_review ON kb.graph_objects USING btree (project_id, needs_review)
WHERE
    (needs_review = true);
...
--
CREATE INDEX idx_graph_objects_status ON kb.graph_objects USING btree (status)
WHERE
    (status IS NOT NULL);
...
--
CREATE INDEX idx_graph_rel_valid_to ON kb.graph_relationships USING btree (valid_to)
WHERE
    (valid_to IS NULL);
...
--
CREATE INDEX idx_notifications_active ON kb.notifications USING btree (user_id, importance, read_at, cleared_at)
WHERE
    (
        (read_at IS NULL)
...
--
CREATE INDEX idx_notifications_category ON kb.notifications USING btree (user_id, category);

--
-- Name: idx_notifications_cleared; Type: INDEX; Schema: kb; Owner: -
--
CREATE INDEX idx_notifications_cleared ON kb.notifications USING btree (user_id, cleared_at)
WHERE
    (cleared_at IS NOT NULL);
...
--
CREATE INDEX idx_notifications_important ON kb.notifications USING btree (user_id, importance)
WHERE
    (cleared_at IS NULL);
...
--
CREATE INDEX idx_notifications_snoozed ON kb.notifications USING btree (snoozed_until)
WHERE
    (snoozed_until IS NOT NULL);
...
--
-- Name: idx_notifications_user_simple; Type: INDEX; Schema: kb; Owner: -
--
CREATE INDEX idx_notifications_user_simple ON kb.notifications USING btree (user_id);
...
--
CREATE INDEX idx_notifications_unread ON kb.notifications USING btree (user_id)
WHERE
    (read_at IS NULL);
...
--
CREATE INDEX idx_notifications_user ON kb.notifications USING btree (user_id, created_at DESC);
...
--
CREATE INDEX idx_project_type_registry_template ON kb.project_object_type_registry USING btree (template_pack_id)
WHERE
    (template_pack_id IS NOT NULL);
...
--
CREATE INDEX idx_system_process_logs_org ON kb.system_process_logs USING btree (organization_id)
WHERE
    (organization_id IS NOT NULL);
...
--
CREATE INDEX idx_system_process_logs_process ON kb.system_process_logs USING btree (process_id)
WHERE
...
--
CREATE INDEX idx_template_packs_published ON kb.graph_template_packs USING btree (published_at DESC)
WHERE
...
--
CREATE INDEX idx_type_suggestions_confidence ON kb.object_type_suggestions USING btree (confidence DESC)
WHERE
    (status = 'pending' :: text);
...
--
CREATE TRIGGER trg_documents_sync_org BEFORE
INSERT
    OR
UPDATE
    ON kb.documents FOR EACH ROW EXECUTE FUNCTION kb.sync_document_org();

--
-- Name: documents trg_documents_touch; Type: TRIGGER; Schema: kb; Owner: -
--
CREATE TRIGGER trg_documents_touch BEFORE
UPDATE
    ON kb.documents FOR EACH ROW EXECUTE FUNCTION kb.touch_updated_at();

--
-- Name: graph_objects trg_graph_objects_touch; Type: TRIGGER; Schema: kb; Owner: -
--
CREATE TRIGGER trg_graph_objects_touch BEFORE
UPDATE
    ON kb.graph_objects FOR EACH ROW EXECUTE FUNCTION kb.touch_updated_at();

--
-- Name: settings trg_settings_touch; Type: TRIGGER; Schema: kb; Owner: -
--
CREATE TRIGGER trg_settings_touch BEFORE
UPDATE
    ON kb.settings FOR EACH ROW EXECUTE FUNCTION kb.touch_updated_at();
...
ALTER TABLE
    ONLY kb.extraction_jobs
ADD
    CONSTRAINT fk_extraction_jobs_org FOREIGN KEY (organization_id) REFERENCES kb.orgs(id) ON DELETE CASCADE;
...
ALTER TABLE
    ONLY kb.graph_embedding_jobs
ADD
    CONSTRAINT fk_graph_embedding_jobs_object FOREIGN KEY (object_id) REFERENCES kb.graph_objects(id) ON DELETE CASCADE;
...
ALTER TABLE
    ONLY kb.graph_objects
ADD
    CONSTRAINT graph_objects_extraction_job_id_fkey FOREIGN KEY (extraction_job_id) REFERENCES kb.object_extraction_jobs(id) ON DELETE
SET
    NULL;
...
ALTER TABLE
    ONLY kb.graph_objects
ADD
    CONSTRAINT graph_objects_org_id_fkey FOREIGN KEY (organization_id) REFERENCES kb.orgs(id) ON DELETE CASCADE;
...
ALTER TABLE
    ONLY kb.graph_objects
ADD
    CONSTRAINT graph_objects_project_id_fkey FOREIGN KEY (project_id) REFERENCES kb.projects(id) ON DELETE CASCADE;
...
ALTER TABLE
    ONLY kb.graph_objects
ADD
    CONSTRAINT graph_objects_supersedes_id_fkey FOREIGN KEY (supersedes_id) REFERENCES kb.graph_objects(id) ON DELETE
SET
    NULL;
...
ALTER TABLE
    ONLY kb.graph_relationships
ADD
    CONSTRAINT graph_relationships_dst_id_fkey FOREIGN KEY (dst_id) REFERENCES kb.graph_objects(id) ON DELETE CASCADE;
...
ALTER TABLE
    ONLY kb.graph_relationships
ADD
    CONSTRAINT graph_relationships_org_id_fkey FOREIGN KEY (organization_id) REFERENCES kb.orgs(id) ON DELETE CASCADE;
...
ALTER TABLE
    ONLY kb.graph_relationships
ADD
    CONSTRAINT graph_relationships_project_id_fkey FOREIGN KEY (project_id) REFERENCES kb.projects(id) ON DELETE CASCADE;
...
ALTER TABLE
    ONLY kb.graph_relationships
ADD
    CONSTRAINT graph_relationships_src_id_fkey FOREIGN KEY (src_id) REFERENCES kb.graph_objects(id) ON DELETE CASCADE;
...
--
-- Name: notifications notifications_user_id_fkey; Type: FK CONSTRAINT; Schema: kb; Owner: -
--
ALTER TABLE
    ONLY kb.notifications
ADD
...
ALTER TABLE
    ONLY kb.object_extraction_jobs
ADD
    CONSTRAINT object_extraction_jobs_chunk_id_fkey FOREIGN KEY (chunk_id) REFERENCES kb.chunks(id) ON DELETE CASCADE;
...
ALTER TABLE
    ONLY kb.object_extraction_jobs
ADD
    CONSTRAINT object_extraction_jobs_document_id_fkey FOREIGN KEY (document_id) REFERENCES kb.documents(id) ON DELETE CASCADE;
...
ALTER TABLE
    ONLY kb.object_extraction_jobs
ADD
    CONSTRAINT object_extraction_jobs_project_id_fkey FOREIGN KEY (project_id) REFERENCES kb.projects(id) ON DELETE CASCADE;
...
ALTER TABLE
    ONLY kb.object_extraction_jobs
ADD
    CONSTRAINT object_extraction_jobs_reprocessing_of_fkey FOREIGN KEY (reprocessing_of) REFERENCES kb.object_extraction_jobs(id);
...
ALTER TABLE
    ONLY kb.object_extraction_logs
ADD
    CONSTRAINT object_extraction_logs_extraction_job_id_fkey FOREIGN KEY (extraction_job_id) REFERENCES kb.object_extraction_jobs(id) ON DELETE CASCADE;
...
ALTER TABLE
    ONLY kb.object_type_suggestions
ADD
    CONSTRAINT object_type_suggestions_project_id_fkey FOREIGN KEY (project_id) REFERENCES kb.projects(id) ON DELETE CASCADE;
...
ALTER TABLE
    ONLY kb.tags
ADD
    CONSTRAINT tags_product_version_id_fkey FOREIGN KEY (product_version_id) REFERENCES kb.product_versions(id) ON DELETE CASCADE;
