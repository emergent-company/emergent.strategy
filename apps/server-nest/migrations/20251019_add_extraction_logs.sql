...
CREATE TABLE IF NOT EXISTS kb.object_extraction_logs (
    ...
    step_index INTEGER NOT NULL, -- Order of operations within the job
    operation_type TEXT NOT NULL, -- e.g., 'llm_call', 'chunk_processing', 'object_creation', 'relationship_creation', 'error'
    
    -- Operation details
    operation_name TEXT, -- e.g., 'extract_entities', 'create_graph_object', 'link_entities'
    ...
    input_data JSONB, -- Prompt, chunk text, entity data, etc.
    output_data JSONB, -- LLM response, created IDs, etc.
    ...
    tokens_used INTEGER, -- If applicable (LLM calls)
    
    -- Metadata
    metadata JSONB, -- Additional context (model name, chunk index, etc.)
...
CREATE INDEX IF NOT EXISTS idx_extraction_logs_job_id 
    ON kb.object_extraction_logs(extraction_job_id);
...
CREATE INDEX IF NOT EXISTS idx_extraction_logs_operation 
    ON kb.object_extraction_logs(extraction_job_id, operation_type);

-- Index for error logs
CREATE INDEX IF NOT EXISTS idx_extraction_logs_errors 
    ON kb.object_extraction_logs(extraction_job_id) 
    WHERE status = 'error';
