...
ALTER TABLE core.user_profiles ADD CONSTRAINT user_profiles_zitadel_user_id_key UNIQUE (zitadel_user_id);
...
WHERE om.subject_id = up.zitadel_user_id;
...
ALTER TABLE kb.organization_memberships DROP CONSTRAINT organization_memberships_subject_id_fkey;
ALTER TABLE kb.organization_memberships DROP COLUMN subject_id;
...
SET user_id = up.id
FROM core.user_profiles up
WHERE pm.subject_id = up.zitadel_user_id;

-- Step 13: Make user_id NOT NULL and drop subject_id
ALTER TABLE kb.project_memberships ALTER COLUMN user_id SET NOT NULL;
ALTER TABLE kb.project_memberships DROP CONSTRAINT project_memberships_subject_id_fkey;
ALTER TABLE kb.project_memberships DROP COLUMN subject_id;
...
SET user_id = up.id
FROM core.user_profiles up
WHERE ue.subject_id = up.zitadel_user_id;

ALTER TABLE core.user_emails ALTER COLUMN user_id SET NOT NULL;
ALTER TABLE core.user_emails DROP CONSTRAINT user_emails_subject_id_fkey;
ALTER TABLE core.user_emails DROP COLUMN subject_id;
...
SET user_id = up.id
FROM core.user_profiles up
WHERE unp.subject_id = up.zitadel_user_id;

ALTER TABLE kb.user_notification_preferences ALTER COLUMN user_id SET NOT NULL;
ALTER TABLE kb.user_notification_preferences DROP CONSTRAINT user_notification_preferences_subject_id_fkey;
ALTER TABLE kb.user_notification_preferences DROP COLUMN subject_id;
...
ALTER TABLE kb.notifications RENAME COLUMN subject_id TO user_id;
ALTER TABLE kb.notifications DROP CONSTRAINT notifications_subject_id_fkey;

-- Update data type if needed and add FK
-- First, add temp column, populate, then swap
ALTER TABLE kb.notifications ADD COLUMN user_id_temp UUID;

UPDATE kb.notifications n
SET user_id_temp = up.id
FROM core.user_profiles up
WHERE n.user_id = up.zitadel_user_id;

ALTER TABLE kb.notifications DROP COLUMN user_id;
ALTER TABLE kb.notifications RENAME COLUMN user_id_temp TO user_id;
...
ALTER TABLE kb.chat_conversations RENAME COLUMN owner_subject_id TO owner_user_id;
ALTER TABLE kb.chat_conversations DROP CONSTRAINT chat_conversations_owner_subject_id_fkey;

-- Add temp column, populate, then swap
ALTER TABLE kb.chat_conversations ADD COLUMN owner_user_id_temp UUID;

UPDATE kb.chat_conversations cc
SET owner_user_id_temp = up.id
FROM core.user_profiles up
WHERE cc.owner_user_id = up.zitadel_user_id;

ALTER TABLE kb.chat_conversations DROP COLUMN owner_user_id;
ALTER TABLE kb.chat_conversations RENAME COLUMN owner_user_id_temp TO owner_user_id;
...
BEGIN
    IF EXISTS (
        ...
        SET user_id = up.id
        FROM core.user_profiles up
        WHERE oej.subject_id = up.zitadel_user_id;
        
        ALTER TABLE kb.object_extraction_jobs DROP COLUMN subject_id;
