...
ALTER TABLE kb.branches 
    RENAME COLUMN org_id TO organization_id;

ALTER TABLE kb.branches 
    ADD CONSTRAINT fk_branches_organization 
    FOREIGN KEY (organization_id) REFERENCES kb.orgs(id) ON DELETE CASCADE;
...
ALTER TABLE kb.chat_conversations 
    ADD CONSTRAINT fk_chat_conversations_organization 
    FOREIGN KEY (organization_id) REFERENCES kb.orgs(id) ON DELETE CASCADE;
...
ALTER TABLE kb.graph_objects 
    ADD CONSTRAINT fk_graph_objects_organization 
    FOREIGN KEY (organization_id) REFERENCES kb.orgs(id) ON DELETE CASCADE;

-- invites: Already uses org_id (UUID) - add FK
ALTER TABLE kb.invites 
    ADD CONSTRAINT fk_invites_organization 
    FOREIGN KEY (org_id) REFERENCES kb.orgs(id) ON DELETE CASCADE;
...
ALTER TABLE kb.object_extraction_jobs 
    ADD CONSTRAINT fk_extraction_jobs_organization 
    FOREIGN KEY (organization_id) REFERENCES kb.orgs(id) ON DELETE CASCADE;

-- product_versions: Rename org_id to organization_id, add FK
ALTER TABLE kb.product_versions 
    RENAME COLUMN org_id TO organization_id;

ALTER TABLE kb.product_versions 
    ADD CONSTRAINT fk_product_versions_organization 
    FOREIGN KEY (organization_id) REFERENCES kb.orgs(id) ON DELETE CASCADE;
...
ALTER TABLE kb.tags 
    DROP CONSTRAINT IF EXISTS fk_tags_organization;
...
ALTER TABLE kb.llm_call_logs 
    ADD CONSTRAINT fk_llm_call_logs_organization 
    FOREIGN KEY (organization_id) REFERENCES kb.orgs(id) ON DELETE SET NULL;

ALTER TABLE kb.llm_call_logs 
    DROP COLUMN org_id;

-- mcp_tool_calls: Convert org_id TEXT to organization_id UUID
ALTER TABLE kb.mcp_tool_calls 
    ADD COLUMN organization_id UUID;

UPDATE kb.mcp_tool_calls 
SET organization_id = org_id::uuid 
WHERE org_id IS NOT NULL 
  AND org_id ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$';

ALTER TABLE kb.mcp_tool_calls 
    ADD CONSTRAINT fk_mcp_tool_calls_organization 
    FOREIGN KEY (organization_id) REFERENCES kb.orgs(id) ON DELETE SET NULL;
...
UPDATE kb.system_process_logs 
SET organization_id = org_id::uuid 
WHERE org_id IS NOT NULL 
  AND org_id ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$';

ALTER TABLE kb.system_process_logs 
    ADD CONSTRAINT fk_system_process_logs_organization 
    FOREIGN KEY (organization_id) REFERENCES kb.orgs(id) ON DELETE SET NULL;
...
CREATE INDEX idx_chat_conversations_organization_proj 
    ON kb.chat_conversations(organization_id, project_id, updated_at DESC);

CREATE INDEX idx_documents_organization 
    ON kb.documents(organization_id);

CREATE INDEX idx_integrations_organization 
    ON kb.integrations(organization_id);

CREATE INDEX idx_llm_call_logs_organization_timestamp 
    ON kb.llm_call_logs(organization_id, started_at DESC) 
    WHERE organization_id IS NOT NULL;

CREATE INDEX idx_mcp_tool_calls_organization 
    ON kb.mcp_tool_calls(organization_id, "timestamp");

CREATE INDEX idx_tags_organization_id 
    ON kb.tags(organization_id);
...
CREATE POLICY tags_isolation ON kb.tags 
    USING (organization_id::text = current_setting('app.current_organization_id', TRUE));

CREATE POLICY tags_read ON kb.tags FOR SELECT 
    USING (organization_id::text = current_setting('app.current_organization_id', TRUE));

CREATE POLICY tags_insert ON kb.tags FOR INSERT 
    WITH CHECK (organization_id::text = current_setting('app.current_organization_id', TRUE));

CREATE POLICY tags_update ON kb.tags FOR UPDATE 
    USING (organization_id::text = current_setting('app.current_organization_id', TRUE));

CREATE POLICY tags_delete ON kb.tags FOR DELETE 
    USING (organization_id::text = current_setting('app.current_organization_id', TRUE));
...
ALTER TABLE kb.branches 
    ADD CONSTRAINT fk_branches_project 
    FOREIGN KEY (project_id) REFERENCES kb.projects(id) ON DELETE CASCADE;

ALTER TABLE kb.chat_conversations 
    ADD CONSTRAINT fk_chat_conversations_project 
    FOREIGN KEY (project_id) REFERENCES kb.projects(id) ON DELETE CASCADE;

ALTER TABLE kb.chunks 
    ADD CONSTRAINT fk_chunks_project 
    FOREIGN KEY (project_id) REFERENCES kb.projects(id) ON DELETE CASCADE;
...
ALTER TABLE kb.graph_objects 
    ADD CONSTRAINT fk_graph_objects_project 
    FOREIGN KEY (project_id) REFERENCES kb.projects(id) ON DELETE CASCADE;

ALTER TABLE kb.integrations 
    ADD CONSTRAINT fk_integrations_project 
    FOREIGN KEY (project_id) REFERENCES kb.projects(id) ON DELETE CASCADE;

ALTER TABLE kb.invites 
    ADD CONSTRAINT fk_invites_project 
    FOREIGN KEY (project_id) REFERENCES kb.projects(id) ON DELETE CASCADE;

ALTER TABLE kb.llm_call_logs 
    ADD CONSTRAINT fk_llm_call_logs_project 
    FOREIGN KEY (project_id) REFERENCES kb.projects(id) ON DELETE SET NULL;

ALTER TABLE kb.mcp_tool_calls 
    ADD CONSTRAINT fk_mcp_tool_calls_project 
    FOREIGN KEY (project_id) REFERENCES kb.projects(id) ON DELETE SET NULL;

ALTER TABLE kb.product_versions 
    ADD CONSTRAINT fk_product_versions_project 
    FOREIGN KEY (project_id) REFERENCES kb.projects(id) ON DELETE CASCADE;

ALTER TABLE kb.system_process_logs 
    ADD CONSTRAINT fk_system_process_logs_project 
    FOREIGN KEY (project_id) REFERENCES kb.projects(id) ON DELETE SET NULL;
...
COMMENT ON COLUMN kb.integrations.organization_id IS 
    'Organization this integration belongs to (FK to orgs.id)';

COMMENT ON COLUMN kb.llm_call_logs.organization_id IS 
    'Organization context for this LLM call (FK to orgs.id)';

COMMENT ON COLUMN kb.mcp_tool_calls.organization_id IS 
    'Organization context for this tool call (FK to orgs.id)';

COMMENT ON COLUMN kb.system_process_logs.organization_id IS 
    'Organization context for this log entry (FK to orgs.id)';

COMMENT ON COLUMN kb.tags.organization_id IS 
    'Organization this tag belongs to (FK to orgs.id, for RLS)';
