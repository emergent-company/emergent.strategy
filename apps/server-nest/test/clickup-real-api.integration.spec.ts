...
describe('ClickUp Real API Integration Tests', () => {
  ...
  beforeAll(async () => {
    if (skipTests) {
      console.log('⚠️  Skipping ClickUp real API tests - credentials not provided');
    ...
    rateLimiter = module.get<ClickUpRateLimiterService>(ClickUpRateLimiterService);
  ...
  describe('3. Task Fetching - Sample Data', () => {
    it('should fetch tasks from first available list', async () => {
      ...
      if (!listId) {
        console.log('⚠️  No lists found in workspace - skipping task fetch test');
        return;
      }
      ...
      if (tasks.tasks.length > 0) {
        ...
        console.log(`   Tags: ${firstTask.tags?.map(t => t.name).join(', ') || 'none'}`);
      }
    }, 60000);
  });
  ...
  describe('4. Data Mapping - Entity Transformation', () => {
    it('should map ClickUp task to internal format', async () => {
      ...
      const tasks = await apiClient.getTasks(apiToken, firstList.id, { page: 0 });
      ...
      const mappedTask = dataMapper.mapTask(clickUpTask, firstList.id, workspaceId);
      ...
      console.log(`   Metadata keys: ${Object.keys(mappedTask.metadata).join(', ')}`);
  ...
  describe('6. Import Service - Structure Building', () => {
    it('should build complete workspace structure', async () => {
      ...
      const structure = await importService.getWorkspaceStructure(apiToken, workspaceId);
      ...
      structure.workspace.spaces.forEach(space => {
        ...
        totalLists += (space.folders?.reduce((acc, f) => acc + (f.lists?.length || 0), 0) || 0)
