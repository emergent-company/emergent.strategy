...
describe('GraphSearchService fusion', () => {
    ...
    it('caps limit at 50 and reports effective + requested limits', async () => {
        const req: GraphSearchRequestDto = { query: 'hello world', limit: 500 } as any;
        const res = await service.search(req, { debug: false, scopes: [] });
    ...
    it('supports backward pagination returning prior window', async () => {
        // First page
        const first = await service.search({ query: 'hello world', limit: 1 } as any, { debug: false, scopes: [] });
        expect(first.items.length).toBe(1);
        const forwardCursor = first.meta.nextCursor;
        if (!forwardCursor) return; // not enough data to continue
        // Second page forward
        const second = await service.search({ query: 'hello world', limit: 1, pagination: { cursor: forwardCursor } } as any, { debug: false, scopes: [] });
        if (second.items.length === 0) return; // no second page
        const secondFirst = second.items[0];
        // Now go backward from second page start
        const backward = await service.search({ query: 'hello world', limit: 1, pagination: { cursor: secondFirst.cursor, direction: 'backward' } } as any, { debug: false, scopes: [] });
    ...
    it('tolerates backward cursor when score precision drifts (id-only match)', async () => {
        ...
        const raw = Buffer.from(cursorObj.cursor as string, 'base64url').toString('utf8');
        let decoded: any;
        ...
        const driftedCursor = Buffer.from(JSON.stringify(decoded)).toString('base64url');
        const backward = await service.search({ query: 'hello world', limit: 1, pagination: { cursor: driftedCursor, direction: 'backward' } } as any, { debug: false, scopes: [] });
        // Should not fall back to forward semantics including the cursor item itself
        if (backward.items.length) {
            expect(backward.items.some(i => i.object_id === cursorObj.object_id)).toBe(false);
    ...
    it('AT-GSP-10: malformed cursor gracefully treated as initial page', async () => {
        const res = await service.search({ query: 'hello world', pagination: { cursor: '!!not-base64!!' }, limit: 2 } as any, { debug: false, scopes: [] });
    ...
    it('AT-GSP-13: prevCursor continuity matches last item cursor of previous page', async () => {
        // Page 1
        const page1 = await service.search({ query: 'hello world', limit: 2 } as any, { debug: false, scopes: [] });
        ...
        // Page 2 forward
        const page2 = await service.search({ query: 'hello world', pagination: { cursor: nextCursor }, limit: 2 } as any, { debug: false, scopes: [] });
