...
describeWithDb('Auth scopes', () => {
    ...
    it('denies access with missing scope (403)', async () => {
        ...
        const res = await httpGetAuth<{ error: { code: string } }>(current.baseUrl, '/auth/me', 'no-scope');
        if (res.status !== 403) {
    ...
    it('allows access with required scope', async () => {
        const current = ctx;
        if (!current) throw new Error('Test app was not bootstrapped');
        const res = await httpGetAuth<{ sub: string }>(current.baseUrl, '/auth/me', 'with-scope');
        if (res.status !== 200) {
    ...
    it('rejects missing Authorization header (401)', async () => {
        const current = ctx;
        if (!current) throw new Error('Test app was not bootstrapped');
        const res = await httpGet<{ error: { code: string } }>(current.baseUrl, '/auth/me');
        if (res.status !== 401) {
