...
describe('GraphService extended coverage', () => {
    ...
    beforeEach(() => {
        ...
        schema = {
            ...
            getRelationshipMultiplicity: vi.fn(async () => ({ src: 'many', dst: 'many' })),
        } as any;
    ...
    function scriptedQuery(sql: string, params: any[]) {
        ...
        if (sql.startsWith('SELECT id, project_id, deleted_at, branch_id FROM kb.graph_objects WHERE id = ANY')) {
        ...
        if (sql.startsWith('SELECT * FROM kb.graph_objects WHERE canonical_id=')) {
            ...
            const matching = Array.from(objects.values()).filter(o => o.canonical_id === params[0]).sort((a, b) => b.version - a.version);
        ...
        if (sql.startsWith('SELECT canonical_id FROM kb.graph_objects WHERE id=')) {
            const row = objects.get(params[0]);
            return { rowCount: row ? 1 : 0, rows: row ? [{ canonical_id: row.canonical_id }] : [] };
        }
        ...
        if (sql.startsWith('SELECT id, org_id, project_id, canonical_id') && sql.includes('FROM kb.graph_objects WHERE canonical_id=')) {
            ...
            let rows = Array.from(objects.values()).filter(o => o.canonical_id === canonicalId).sort((a, b) => b.version - a.version);
            if (cursorIdx !== null) rows = rows.filter(r => r.version < params[cursorIdx]);
        ...
        if (sql.startsWith('SELECT * FROM kb.graph_relationships') && sql.includes('ORDER BY version DESC LIMIT 1')) {
            ...
            const matches = Array.from(rels.values()).filter(r => r.project_id === projectId && (branchId === null ? true : true) && r.type === type && r.src_id === src && r.dst_id === dst).sort((a, b) => b.version - a.version);
            return { rowCount: matches.length ? 1 : 0, rows: matches.slice(0, 1) };
        }

        // Relationship canonical history lookup
        if (sql.startsWith('SELECT canonical_id FROM kb.graph_relationships WHERE id=')) {
            const row = rels.get(params[0]);
            return { rowCount: row ? 1 : 0, rows: row ? [{ canonical_id: row.canonical_id }] : [] };
        }

        // Relationship head version fast path
        if (sql.startsWith('SELECT version as v FROM kb.graph_relationships WHERE canonical_id=')) {
            const canonical = params[0];
            const head = Array.from(rels.values()).filter(r => r.canonical_id === canonical).sort((a, b) => b.version - a.version)[0];
            return { rowCount: head ? 1 : 0, rows: head ? [{ v: head.version }] : [] };
        }
        ...
        if (sql.startsWith('SELECT id, org_id, project_id, type, src_id, dst_id')) {
            ...
            const limit = params[params.length - 1];
            const cursorIdx = sql.includes('version < $2') ? 1 : null;
            let rows = Array.from(rels.values()).filter(r => r.canonical_id === canonicalId).sort((a, b) => b.version - a.version);
            if (cursorIdx !== null) rows = rows.filter(r => r.version < params[cursorIdx]);
            return { rowCount: rows.length, rows: rows.slice(0, limit) };
        }
        ...
        if (sql.includes('FROM kb.graph_objects') || sql.includes('FROM kb.graph_relationships r')) {
            return { rowCount: 0, rows: [] };
        }
    ...
    it('createRelationship no-op diff returns head without insert', async () => {
        ...
        const initial = mkRel({ properties: { a: 1 }, src_id: src.id, dst_id: dst.id });
        rels.set(initial.id, initial);
        const res = await svc.createRelationship({ type: initial.type, src_id: initial.src_id, dst_id: initial.dst_id, properties: { a: 1 } }, initial.org_id, initial.project_id);
        expect(res.id).toBe(initial.id);
        // ensure no second INSERT fired (only BEGIN, advisory lock, select head, ROLLBACK recorded)
        const insertCount = queries.filter(q => q.sql.startsWith('INSERT INTO kb.graph_relationships')).length;
    ...
    it('listHistory paginates with limit+1 producing next_cursor', async () => {
        ...
        for (let v = 2; v <= 5; v++) {
            objects.set(crypto.randomUUID(), mkObj({ canonical_id: base.canonical_id, version: v }));
        }
    ...
    it('traverse forward & backward pagination with filters and truncation by nodes', async () => {
        ...
        const r1 = mkRel({ src_id: a.id, dst_id: b.id }); rels.set(r1.id, r1);
        const r2 = mkRel({ src_id: b.id, dst_id: c.id }); rels.set(r2.id, r2);
        const r3 = mkRel({ src_id: c.id, dst_id: d.id }); rels.set(r3.id, r3);

        const forward = await svc.traverse({ root_ids: [a.id], limit: 2, page_direction: 'forward' });
        expect(forward.nodes.length).toBe(2);
        expect(forward.next_cursor).toBeTruthy();
        const back = await svc.traverse({ root_ids: [a.id], limit: 2, page_direction: 'backward', cursor: forward.next_cursor! });
        expect(back.nodes.length).toBeGreaterThan(0);
    });

    it('traverse sets telemetry and truncates by max_edges', async () => {
        const a = mkObj({}); const b = mkObj({}); const c = mkObj({});
        [a, b, c].forEach(o => objects.set(o.id, o));
        // Fully connect to exceed edge cap 1
        const r1 = mkRel({ src_id: a.id, dst_id: b.id }); rels.set(r1.id, r1);
        const r2 = mkRel({ src_id: b.id, dst_id: c.id }); rels.set(r2.id, r2);
        const r3 = mkRel({ src_id: a.id, dst_id: c.id }); rels.set(r3.id, r3);
        const res = await svc.traverse({ root_ids: [a.id], max_edges: 1, limit: 10 });
        // @ts-ignore
    ...
    it('expand projection include/exclude and include_relationship_properties', async () => {
        ...
        const r = mkRel({ src_id: a.id, dst_id: b.id, properties: { weight: 5 } }); rels.set(r.id, r);
        const res = await svc.expand({ root_ids: [a.id], projection: { include_object_properties: ['keep'] }, include_relationship_properties: true });
