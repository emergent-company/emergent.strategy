...
interface ObjRow { id: string; type: string; key: string | null; labels: string[]; deleted_at: Date | null; }
interface RelRow { id: string; type: string; src_id: string; dst_id: string; deleted_at: Date | null; version: number; canonical_id: string; }
...
class MockDb extends DatabaseService {
    ...
    private rels: RelRow[] = [];
    constructor() { // @ts-ignore super expects config
        super({} as any);
        const letters = ['a', 'b', 'c', 'd', 'e'];
        letters.forEach(l => this.objects.push({ id: makeId(l), type: 'node', key: l, labels: [], deleted_at: null }));
        // linear edges a->b->c->d->e
        for (let i = 0; i < letters.length - 1; i++) {
            const src = makeId(letters[i]);
            const dst = makeId(letters[i + 1]);
            const canon = `rel-${i}`;
            this.rels.push({ id: `rel-${i}-v1`, type: 'link', src_id: src, dst_id: dst, deleted_at: null, version: 1, canonical_id: canon });
        }
    }
    async getClient() {
    ...
    async query(sql: string, params: any[]) {
        ...
        if (/FROM kb\.graph_relationships/.test(sql) && /DISTINCT ON/.test(sql)) {
            ...
            for (const r of rels) { byCanon[r.canonical_id] = byCanon[r.canonical_id] || []; byCanon[r.canonical_id].push(r); }
            const heads: RelRow[] = Object.values(byCanon).map(v => v.sort((a, b) => b.version - a.version)[0]);
...
describe('GraphService traverse backward pagination', () => {
    ...
    it('paginates backward correctly and sets previous/next cursors appropriately', async () => {
        ...
        const first = await service.traverse({ root_ids: [makeId('a')], limit: 3, max_depth: 10, page_direction: 'forward' });
        ...
        const second = await service.traverse({ root_ids: [makeId('a')], limit: 3, max_depth: 10, cursor: (cursorC || undefined), page_direction: 'forward' });
        ...
        const backward = await service.traverse({ root_ids: [makeId('a')], limit: 2, max_depth: 10, cursor: (tailCursor || undefined), page_direction: 'backward' });
        ...
        const backwardToStart = await service.traverse({ root_ids: [makeId('a')], limit: 5, max_depth: 10, cursor: (backward.previous_cursor || undefined), page_direction: 'backward' });
