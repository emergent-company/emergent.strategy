...
describe('Graph Vector Controller Endpoints', () => {
    ...
    let orgId: string; let projectId: string; let branchId: string | null = null;

    beforeAll(async () => {
        process.env.DB_AUTOINIT = 'true';
        const mod = await Test.createTestingModule({ imports: [DatabaseModule, AppConfigModule, GraphModule] }).compile();
    ...
    it('POST /graph/objects/vector-search returns ordered neighbors (numeric distances sorted ascending)', async () => {
        ...
        const isNonDecreasing = distances.every((d: number, i: number, arr: number[]) => i === 0 || d >= arr[i - 1]);
    ...
    it('GET /graph/objects/:id/similar supports type filter', async () => {
        ...
        for (const row of res.body) {
            const q = await db.query(`SELECT type FROM kb.graph_objects WHERE id=$1`, [row.id]);
            if (q.rowCount) {
    ...
    it('GET /graph/objects/:id/similar supports labelsAll filter', async () => {
        ...
        const id = randomUUID();
        const literal = '[' + Array(768).fill(0).map((_, i) => (i === 2 ? 0.003 : 0)).join(',') + ']';
        await db.setTenantContext(orgId, projectId);
        await db.query(
        ...
        for (const row of res.body) {
            const q = await db.query(`SELECT labels FROM kb.graph_objects WHERE id=$1`, [row.id]);
            if (q.rowCount) {
                const labels = q.rows[0].labels as string[];
                expect(labels).toEqual(expect.arrayContaining(['sim-alpha', 'sim-beta']));
            }
        }
    });
    ...
    it('POST /graph/objects/vector-search labelsAll filter requires all labels', async () => {
        ...
        const literal = '[' + Array(768).fill(0).map((_, i) => (i === 0 ? 0.001 : 0)).join(',') + ']';
        await db.setTenantContext(orgId, projectId);
        await db.query(
            `INSERT INTO kb.graph_objects(id, organization_id, project_id, branch_id, type, key, labels, embedding_vec, canonical_id, version)
             VALUES($1,$2,$3,$4,$5,$6,$7,$8::vector,$1,1)`,
        ...
        const resExclude = await request(app.getHttpServer())
            .post('/graph/objects/vector-search')
            .send({ vector: baseVec(), limit: 50, labelsAll: ['lab-one', 'missing-label'] })
            .expect(200);
    ...
    it('POST /graph/objects/vector-search labelsAny returns objects having any label', async () => {
        if (!db.isOnline()) return; // skip
        const id = randomUUID();
        const literal = '[' + Array(768).fill(0).map((_, i) => (i === 1 ? 0.002 : 0)).join(',') + ']';
        await db.setTenantContext(orgId, projectId);
        await db.query(
            `INSERT INTO kb.graph_objects(id, organization_id, project_id, branch_id, type, key, labels, embedding_vec, canonical_id, version)
             VALUES($1,$2,$3,$4,$5,$6,$7,$8::vector,$1,1)`,
    ...
    it('POST /graph/objects/vector-search supports maxDistance alias (precedence over minScore)', async () => {
        ...
        const nearLiteral = '[' + Array(768).fill(0).map((_, i) => (i === 0 ? 0.05 : 0)).join(',') + ']';
        await db.setTenantContext(orgId, projectId);
        await db.query(
        ...
        const res = await request(app.getHttpServer())
            .post('/graph/objects/vector-search')
            .send({ vector: baseVec(), limit: 10, minScore: 0.0000001, maxDistance: 0.15 })
            .expect(200);
    ...
    it('GET /graph/objects/:id/similar supports maxDistance alias', async () => {
        if (!db.isOnline()) return; // skip
        const res = await request(app.getHttpServer())
            .get(`/graph/objects/${a}/similar?limit=5&minScore=0.0000001&maxDistance=0.2`)
            .expect(200);
    ...
    it('POST /graph/objects/vector-search supports combined labelsAll + labelsAny', async () => {
        ...
        const literal = '[' + Array(768).fill(0).map((_, i) => (i === 3 ? 0.004 : 0)).join(',') + ']';
        await db.setTenantContext(orgId, projectId);
        await db.query(
            `INSERT INTO kb.graph_objects(id, organization_id, project_id, branch_id, type, key, labels, embedding_vec, canonical_id, version)
             VALUES($1,$2,$3,$4,$5,$6,$7,$8::vector,$1,1)`,
