...
describe('GraphVectorSearchService', () => {
    ...
    it('applies labelsAll containment and labelsAny overlap filters', async () => {
        ...
        const idAll = uuid();
        await db.query(`INSERT INTO kb.graph_objects(id, org_id, project_id, type, key, labels, properties, embedding_vec, canonical_id, version) VALUES ($1,$2,$3,'LabType','lab-all','$4','{}',$5::vector,$1,1)`, [idAll, ids.orgId, ids.projectId, ['alpha', 'beta', 'shared'], baseLiteral]);
        const idAny = uuid();
        await db.query(`INSERT INTO kb.graph_objects(id, org_id, project_id, type, key, labels, properties, embedding_vec, canonical_id, version) VALUES ($1,$2,$3,'LabType','lab-any','$4','{}',$5::vector,$1,1)`, [idAny, ids.orgId, ids.projectId, ['gamma', 'shared'], baseLiteral]);
    ...
    it('applies orgId + projectId filters together', async () => {
        ...
        const projA = (await db.query<{ id: string }>(`INSERT INTO kb.projects(org_id,name) VALUES($1,$2) RETURNING id`, [orgA, 'proj-filter-a-' + uuid().slice(0, 8)])).rows[0].id;
        const orgB = (await db.query<{ id: string }>(`INSERT INTO kb.orgs(name) VALUES($1) RETURNING id`, ['org-filter-b-' + uuid().slice(0, 8)])).rows[0].id;
        const projB = (await db.query<{ id: string }>(`INSERT INTO kb.projects(org_id,name) VALUES($1,$2) RETURNING id`, [orgB, 'proj-filter-b-' + uuid().slice(0, 8)])).rows[0].id;
        const vecLiteral = '[' + baseVec().join(',') + ']';
        const idA = uuid();
        await db.query(`INSERT INTO kb.graph_objects(id, org_id, project_id, type, key, properties, embedding_vec, canonical_id, version) VALUES ($1,$2,$3,'OrgProjType','a-key','{}',$4::vector,$1,1)`, [idA, orgA, projA, vecLiteral]);
        const idB = uuid();
        await db.query(`INSERT INTO kb.graph_objects(id, org_id, project_id, type, key, properties, embedding_vec, canonical_id, version) VALUES ($1,$2,$3,'OrgProjType','b-key','{}',$4::vector,$1,1)`, [idB, orgB, projB, vecLiteral]);
