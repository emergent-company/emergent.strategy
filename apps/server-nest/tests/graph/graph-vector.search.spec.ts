...
describe('GraphVectorSearchService', () => {
    ...
    it('applies labelsAll containment and labelsAny overlap filters', async () => {
        ...
        const idAll = uuid();
        await db.query(`INSERT INTO kb.graph_objects(id, organization_id, project_id, type, key, labels, properties, embedding_vec, canonical_id, version) VALUES ($1,$2,$3,'LabType','lab-all','$4','{}',$5::vector,$1,1)`, [idAll, ids.orgId, ids.projectId, ['alpha', 'beta', 'shared'], baseLiteral]);
        const idAny = uuid();
        await db.query(`INSERT INTO kb.graph_objects(id, organization_id, project_id, type, key, labels, properties, embedding_vec, canonical_id, version) VALUES ($1,$2,$3,'LabType','lab-any','$4','{}',$5::vector,$1,1)`, [idAny, ids.orgId, ids.projectId, ['gamma', 'shared'], baseLiteral]);
    ...
    it('applies distance threshold via minScore/maxDistance alias', async () => {
        ...
        const filtered = await svc.searchByVector(baseVec(), { limit: 10, minScore: 0.2 }); // legacy param meaning max allowed distance
        ...
        const strict = await svc.searchByVector(baseVec(), { limit: 10, maxDistance: 0.05, minScore: 0.5 }); // maxDistance must override minScore
    ...
    it('treats maxDistance as precedence over minScore when both supplied', async () => {
        ...
        const v2 = await insertObject(db, (() => { const v = baseVec(); v[0] = 0.06; return v; })(), 'AliasPrecMid');
        const all = await svc.searchByVector(baseVec(), { limit: 10 });
        ...
        const filtered = await svc.searchByVector(baseVec(), { limit: 10, minScore: 0.5, maxDistance: 0.05 });
    ...
    it('applies orgId + projectId filters together', async () => {
        if (!db.isOnline()) return;
        const orgA = (await db.query<{ id: string }>(`INSERT INTO kb.orgs(name) VALUES($1) RETURNING id`, ['org-filter-a-' + uuid().slice(0, 8)])).rows[0].id;
        const projA = (await db.query<{ id: string }>(`INSERT INTO kb.projects(organization_id,name) VALUES($1,$2) RETURNING id`, [orgA, 'proj-filter-a-' + uuid().slice(0, 8)])).rows[0].id;
        const orgB = (await db.query<{ id: string }>(`INSERT INTO kb.orgs(name) VALUES($1) RETURNING id`, ['org-filter-b-' + uuid().slice(0, 8)])).rows[0].id;
        const projB = (await db.query<{ id: string }>(`INSERT INTO kb.projects(organization_id,name) VALUES($1,$2) RETURNING id`, [orgB, 'proj-filter-b-' + uuid().slice(0, 8)])).rows[0].id;
        const vecLiteral = '[' + baseVec().join(',') + ']';
        const idA = uuid();
        await db.query(`INSERT INTO kb.graph_objects(id, organization_id, project_id, type, key, properties, embedding_vec, canonical_id, version) VALUES ($1,$2,$3,'OrgProjType','a-key','{}',$4::vector,$1,1)`, [idA, orgA, projA, vecLiteral]);
        const idB = uuid();
        await db.query(`INSERT INTO kb.graph_objects(id, organization_id, project_id, type, key, properties, embedding_vec, canonical_id, version) VALUES ($1,$2,$3,'OrgProjType','b-key','{}',$4::vector,$1,1)`, [idB, orgB, projB, vecLiteral]);
        const filtered = await svc.searchByVector(baseVec(), { limit: 10, orgId: orgA, projectId: projA });
