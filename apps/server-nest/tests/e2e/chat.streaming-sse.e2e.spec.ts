...
describe('Chat Streaming SSE E2E', () => {
    ...
    it('streams deterministic token frames then DONE', async () => {
        ...
        const createRes = await fetch(`${ctx.baseUrl}/chat/conversations`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', ...authHeader('all', 'chat-sse'), 'x-org-id': ctx.orgId, 'x-project-id': ctx.projectId },
            body: JSON.stringify({ message: 'Hello streaming test', isPrivate: false })
        });
