...
describe('LangChain Extraction - Real Meeting (E2E)', () => {
    ...
    beforeAll(async () => {
        ...
        if (!process.env.GOOGLE_API_KEY) {
            throw new Error('GOOGLE_API_KEY environment variable not set. Cannot run E2E test with real model.');
        }
    ...
    it('should extract structured entities from unstructured meeting transcript using LangChain', async () => {
        ...
        console.log(`   ✓ Loaded ${meetingContent.length} characters from meeting transcript`);

        console.log('\n📤 Step 2: Creating document...');
        const documentResult = await db.query<{ id: string }>(
            `INSERT INTO kb.documents (id, org_id, project_id, source_type, filename, mime_type, full_text, created_by)
             VALUES (gen_random_uuid(), $1, $2, 'upload', $3, 'text/markdown', $4, $5)
             RETURNING id`,
            [TEST_ORG_ID, TEST_PROJECT_ID, 'meeting_1.md', meetingContent, TEST_USER_ID]
        );
        ...
        console.log('   (This may take 10-30 seconds depending on API response time)');
        ...
        // Fetch updated job
        const completedJob = await extractionJobService.getJobById(job.id, TEST_PROJECT_ID, TEST_ORG_ID);
        ...
        console.log('\n📈 Extraction Summary:');
        console.log(`   • Total entities extracted: ${completedJob.successful_items}`);
        ...
        if (completedJob.discovered_types && completedJob.discovered_types.length > 0) {
            console.log('\n   Discovered types:');
        ...
        if (decisions.length > 0) {
            ...
            decisions.slice(0, 2).forEach((decision, idx) => {
                ...
                console.log(`      Confidence: ${decision.confidence?.toFixed(2) || 'N/A'}`);
                if (decision.properties.rationale) {
                    console.log(`      Rationale: ${decision.properties.rationale.substring(0, 100)}...`);
                }
            });
        }
        ...
        if (tasks.length > 0) {
            ...
            tasks.slice(0, 2).forEach((task, idx) => {
                ...
                console.log(`      Confidence: ${task.confidence?.toFixed(2) || 'N/A'}`);
            });
        }
        ...
        const lowConfidence = objects.filter(o => o.confidence && o.confidence < 0.5);
        console.log(`   • Objects with confidence < 0.5: ${lowConfidence.length}`);

        const highConfidence = objects.filter(o => o.confidence && o.confidence >= 0.8);
        console.log(`   • Objects with confidence >= 0.8: ${highConfidence.length}`);
        ...
        console.log(`\nSummary:`);
        console.log(`• Processed meeting transcript: ${(meetingContent.length / 1024).toFixed(1)} KB`);
        console.log(`• Total entities extracted: ${objects.length}`);
        console.log(`• Processing time: ${duration}ms`);
        console.log(`• LLM Provider: ${llmFactory.getProviderName()}`);
        console.log(`• Average confidence: ${(objects.reduce((sum, o) => sum + (o.confidence || 0), 0) / objects.length).toFixed(2)}`);
    ...
    it('should extract expected key decisions from meeting about spec and AI', async () => {
        ...
        const specDecision = decisions.find(d =>
        ...
        const aiDecision = decisions.find(d =>
    ...
    async function cleanupTestData() {
        ...
        await db.query('DELETE FROM kb.objects WHERE project_id = $1', [TEST_PROJECT_ID]);
        await db.query('DELETE FROM kb.object_extraction_jobs WHERE project_id = $1', [TEST_PROJECT_ID]);
        await db.query('DELETE FROM kb.documents WHERE project_id = $1', [TEST_PROJECT_ID]);
