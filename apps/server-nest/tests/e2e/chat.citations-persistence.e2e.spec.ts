...
interface ConversationResponse { id: string; messages: { id: string; role: string; content: string; citations?: any[] }[]; }
let ctx: E2EContext;
...
describe('Chat Citations Persistence', () => {
    ...
    it('streams then fetches conversation; assistant message persisted with optional citations', async () => {
        ...
        const createRes = await fetch(`${ctx.baseUrl}/chat/conversations`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', ...authHeader('all', 'chat-cite-persist'), 'x-org-id': ctx.orgId, 'x-project-id': ctx.projectId },
            body: JSON.stringify({ message: 'Tell me about mountains', isPrivate: true })
        });
        ...
        const streamRes = await fetch(`${ctx.baseUrl}/chat/${conversationId}/stream`, {
            headers: { ...authHeader('all', 'chat-cite-persist'), 'x-org-id': ctx.orgId, 'x-project-id': ctx.projectId }
        });
        expect(streamRes.status).toBe(200);
        const body = await streamRes.text();
        const events = body.split(/\n\n/).filter(e => e.trim().length > 0);
        // Optional: ensure token frames include index/total fields
        const tokenFrames = events.filter(e => /"message"\s*:\s*"token-0"/.test(e));
        ...
        if (assistantMsg) {
            ...
            if (citationFrame) {
                ...
                if (Array.isArray(assistantMsg.citations)) expect(assistantMsg.citations.length).toBeGreaterThan(0);
            } else {
