    ...
    async query(text: string, params?: any[]) {
        ...
        if (!script) {
            ...
            const fallbackScript = this.scripts.find(s => {
                ...
                return keyParts.length > 0 && keyParts.every(part => lowerText.includes(part));
            });
...
class FakeDb extends DatabaseService {
    constructor(private isDbOnline: boolean = true, private clientFactory?: () => FakeClient) {
        super({} as any);
    }
    isOnline() { return this.isDbOnline; }
    setOnline(v: boolean) { this.isDbOnline = v; }
...
describe('OrgsService', () => {
    it('list() offline returns in-memory array copy (initially empty)', async () => {
        ...
        const svc = new OrgsService(orgRepo as any, membershipRepo as any, dataSource as any, db as any, cfg as any);
    ...
    it('get() online table missing returns null (fallback)', async () => {
        ...
        const membershipRepo = createMockRepository();
        const dataSource = new FakeDataSource([
            { text: /SELECT id, name, created_at, updated_at FROM kb\.orgs WHERE/, throw: pgError('42P01') }
        ]);
        const db = new FakeDb(true);
        const cfg = createMockConfig();
        const svc = new OrgsService(orgRepo as any, membershipRepo as any, dataSource as any, db as any, cfg as any);
