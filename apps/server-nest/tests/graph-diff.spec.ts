...
describe('diffProperties', () => {
    it('detects added, removed, and updated fields with JSON Pointer paths (including array growth)', () => {
        const prev = { title: 'Old', count: 1, nested: { a: 1, b: 2 }, tags: ['x', 'y'] };
        const next = { title: 'New', count: 1, nested: { a: 1, c: 3 }, tags: ['x', 'y', 'z'], extra: true };
        ...
        expect(diff.paths).toEqual(expect.arrayContaining(['/title', '/nested/c', '/nested/b', '/tags/2', '/extra']));
    });
    ...
    it('elides updated block when exceeding max size', () => {
        const prev: Record<string, number> = {}; const next: Record<string, number> = {};
        for (let i = 0; i < 200; i++) { prev['k' + i] = i; next['k' + i] = i + 1; }
