        ...
        createQueryBuilder: vi.fn(() => {
            ...
            if (shouldFailFirst && callCount === 1) {
                return {
                    ...
                    getMany: vi.fn().mockRejectedValue(Object.assign(
                        new Error('column c.embedding does not exist'),
                        { code: '42703' }
                    ))
                };
            }
...
const mockDb = { isOnline: () => true } as any;
...
describe('ChunksService list resilience', () => {
    it('falls back when embedding column missing', async () => {
        ...
        const service = new ChunksService(repo as any, mockDb);
