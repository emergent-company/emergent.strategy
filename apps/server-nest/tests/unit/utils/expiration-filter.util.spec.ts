...
describe('expiration-filter.util', () => {
  describe('buildExpirationFilterClause', () => {
    ...
    it('should handle different table aliases', () => {
      expect(buildExpirationFilterClause('obj')).toBe(
        '(obj.expires_at IS NULL OR obj.expires_at > now())'
      );
      expect(buildExpirationFilterClause('t')).toBe(
        '(t.expires_at IS NULL OR t.expires_at > now())'
      );
      expect(buildExpirationFilterClause('h')).toBe(
