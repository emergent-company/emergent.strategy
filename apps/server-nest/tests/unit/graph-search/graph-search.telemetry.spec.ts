...
class StubEmbedding extends EmbeddingService {
    async embedQuery(q: string): Promise<number[]> { return Array.from({ length: 4 }, (_, i) => (q.length + i) / 10); }
}
...
describe('GraphSearchService telemetry (AT-GSP-12)', () => {
    ...
    it('emits pagination telemetry event with required fields', async () => {
        const res = await service.search({ query: 'some query', pagination: { limit: 3 } } as any, { debug: false, scopes: [] });
