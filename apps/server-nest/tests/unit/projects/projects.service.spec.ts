...
class FakeDataSource {
  ...
  async query(text: string, params?: any[]) {
    ...
    const h = this.handlers.find((h) =>
      typeof h.match === 'string' ? text.includes(h.match) : h.match.test(text)
    );
    if (!h) return [];
...
describe('ProjectsService', () => {
  ...
  it('list returns rows mapped when no orgId', async () => {
    ...
    expect(res).toEqual([
      { id: uuid(1), name: 'P1', orgId: uuid(9), kb_purpose: undefined },
    ]);
  });
  ...
  it('create rejects blank name', async () => {
    ...
    await expect(svc.create('  ')).rejects.toMatchObject({
      response: { error: { code: 'validation-failed' } },
    });
  });
  ...
  it('create rejects missing orgId', async () => {
    ...
    await expect(svc.create('Proj')).rejects.toMatchObject({
      response: { error: { code: 'org-required' } },
    });
  });
  ...
  it('create rejects org not found', async () => {
    ...
    const orgRepo = createMockRepository();
    const dataSource = new FakeDataSource([]);
    (dataSource as any).createQueryRunner = vi
      .fn()
    ...
    await expect(svc.create('Proj', uuid(1))).rejects.toMatchObject({
    ...
    const queryRunner = {
      ...
      manager: {
        ...
        save: vi
          ...
          .mockRejectedValue(
            new Error(
              'insert or update on table "kb.projects" violates foreign key constraint "projects_organization_id_fkey"'
            )
          ),
      },
    };
