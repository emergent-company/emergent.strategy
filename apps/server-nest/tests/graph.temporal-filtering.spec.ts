...
describe('Graph Traversal - Temporal Filtering', () => {
    ...
    describe('Temporal filter SQL clause generation', () => {
        it('should generate correct SQL for valid_from mode', async () => {
            ...
            const { buildTemporalFilterClause } = await import('../src/modules/graph/temporal-filter.util');
    ...
    describe('Integration with traversal', () => {
        it('should accept temporalFilter parameter in traverse method', async () => {
            // Create simple graph
            await service.createObject({ type: 'Document', key: 'doc-1', labels: [] } as any);
        ...
        it('should accept temporalFilter with phased traversal', async () => {
            await service.createObject({ type: 'Requirement', key: 'req-1', labels: [], project_id: 'proj' } as any);
            await service.createObject({ type: 'Dependency', key: 'dep-1', labels: [], project_id: 'proj' } as any);
            await service.createRelationship({ type: 'DEPENDS_ON', src_id: 'o_1', dst_id: 'o_2' }, 'org', 'proj');
        ...
        it('should work with predicate filters', async () => {
            await service.createObject({ type: 'Task', key: 'task-1', labels: [], properties: { status: 'active' } } as any);
        ...
        it('should work with path enumeration', async () => {
            await service.createObject({ type: 'Concept', key: 'A', labels: [], project_id: 'proj' } as any);
            await service.createObject({ type: 'Concept', key: 'B', labels: [], project_id: 'proj' } as any);
            await service.createRelationship({ type: 'RELATES_TO', src_id: 'o_1', dst_id: 'o_2' }, 'org', 'proj');
