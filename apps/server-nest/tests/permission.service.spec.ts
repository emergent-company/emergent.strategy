...
function createMockRepo(fixtures: any[]) { // eslint-disable-line @typescript-eslint/no-explicit-any
    return {
        find: vi.fn(async (opts?: any) => { // eslint-disable-line @typescript-eslint/no-explicit-any
...
describe('PermissionService', () => {
    it('grants org_admin full mapped scopes including chat:admin/documents:delete', async () => {
        const orgRepo = createMockRepo([{ user_id: 'user-1', organization_id: 'o1', role: 'org_admin' }]);
        const projectRepo = createMockRepo([]);
        const db = new MockDb();
        const svc = new PermissionService(orgRepo as any, projectRepo as any, db as any);
    ...
    it('project_admin inherits project + documents full but not project creation/delete', async () => {
        ...
        const projectRepo = createMockRepo([{ user_id: 'user-2', project_id: 'p1', role: 'project_admin' }]);
        const db = new MockDb();
        const svc = new PermissionService(orgRepo as any, projectRepo as any, db as any);
    ...
    it('project_user has read-only docs & chat:use', async () => {
        const orgRepo = createMockRepo([]);
        const projectRepo = createMockRepo([{ user_id: 'user-3', project_id: 'p1', role: 'project_user' }]);
        const db = new MockDb();
        const svc = new PermissionService(orgRepo as any, projectRepo as any, db as any);
    ...
    it('offline DB fallback returns minimal org:read only', async () => {
        ...
        const svc = new PermissionService(orgRepo as any, projectRepo as any, offline as any);
