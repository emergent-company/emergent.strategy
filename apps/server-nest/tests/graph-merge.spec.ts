...
describe('Graph Merge Dry-Run (MVP)', () => {
  ...
  beforeAll(async () => {
    ...
    targetBranch = await createBranchOrThrow(`target-merge-test-${suffix}`, orgId, projectId);
    sourceBranch = await createBranchOrThrow(`source-merge-test-${suffix}`, orgId, projectId);
  ...
  test('AT-MERGE-4: truncation when limit lower than enumeration size', async () => {
    ...
    const isolatedSource = await createBranchOrThrow(`trunc-source-${Date.now()}`, orgId, projectId);
  ...
  test('AT-MERGE-5: deterministic ordering (conflict > fast_forward > added > unchanged if present)', async () => {
    ...
    const orderSource = await createBranchOrThrow(`order-source-${Date.now()}`, orgId, projectId);
    ...
    await createObject(orderTarget, 'Doc', 'ord-ff', { title: 'OnlyTargetProp' });
    await createObject(orderSource, 'Doc', 'ord-ff', { body: 'OnlySourceProp' });
    ...
    console.log('AT-MERGE-5 raw statuses', statuses, 'debug_priorities', objs.map((o: any) => o.debug_priority));

    // Priority enforced by service sort (ascending by numeric priority): conflict(0) < fast_forward(1) < added(2) < unchanged(3)
    const priority: Record<string, number> = { conflict: 0, fast_forward: 1, added: 2, unchanged: 3 };
    ...
    if (firstIdx.conflict !== undefined && firstIdx.fast_forward !== undefined) {
    ...
    expect(statuses.filter((s: string) => s === 'conflict').length).toBeGreaterThanOrEqual(1);
