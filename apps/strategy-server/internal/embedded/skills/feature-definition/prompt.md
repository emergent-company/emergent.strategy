...
### Step 0: Granularity Check (5 minutes)

**BEFORE creating a feature definition, verify the scope is correct.**

This prevents the common mistake of defining features that are too coarse (doing too much) or too granular (artificial fragmentation).

**Quick Granularity Test:**

| Question | Yes → Proceed | No → Action |
|----------|---------------|-------------|
| Can you describe it in one sentence without "and" connecting unrelated functions? | ✅ | SPLIT: Each unrelated function → separate FD |
| Would a user recognize this as "one thing they can accomplish"? | ✅ | TOO GRANULAR: Combine with related feature |
| Can you cover use cases with 4 personas? | ✅ | SPLIT: >4 personas needed suggests multiple jobs |
| Will it have 2-15 capabilities? | ✅ | <2: Combine / >15: Split by job |
| Will capabilities mature within 1-2 stages of each other? | ✅ | CONSIDER SPLIT: Future caps → future FD |

**Examples of Right-Sized Features:**

| ✅ Good Scope | ❌ Too Granular | ❌ Too Coarse |
|---------------|-----------------|---------------|
| Document Management & Version Control | Upload Button | Complete Platform |
| Team Collaboration & Communication | Send Message Action | Everything Users Do |
| Knowledge Graph Engine | Display Graph Node | All AI Features |

**If unsure:** Err toward slightly larger features (easier to split later). Check the EPF corpus for similar scoping patterns.

**For comprehensive guidance:** See [FEATURE_DEFINITION_GRANULARITY_GUIDE.md](../docs/guides/FEATURE_DEFINITION_GRANULARITY_GUIDE.md)
