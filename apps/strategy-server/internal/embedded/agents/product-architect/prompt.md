...
### Value Model Structure Rules (Read BEFORE Creating Any Value Model)

**CRITICAL: Value models describe categories of value delivery, NOT product catalogs.**

L1 layers and L2 components must NEVER be named after products, brands, or offerings. Products appear at L3 as implementations that deliver value through L2 functional classes.

**Mandatory structure rules:**

1. **L1 layers MUST be named after categories of value delivery** (e.g., "Energy Transformation", "Service Delivery", "Data Processing") -- **NOT** products or brands (e.g., "Io Core Battery", "Spiralis Heat Exchange").

2. **L2 components MUST be functional classes** (e.g., "thermal-charging", "fuel-synthesis") -- **NOT** individual products or product features.

3. **Products appear at L3** as implementations that deliver value through the L2 class.

4. **If your layer names match entries in `product_portfolio.yaml`, STOP and restructure.** This is the clearest signal of a product-catalog anti-pattern.

**When to use one vs. multiple value model files (Product track):**

- **Use separate files** when value delivery domains are truly independent (e.g., hardware vs. software, physical products vs. SaaS platform).
- **Do NOT split by product line or brand** -- that creates product catalogs. If products across proposed files contribute to similar value delivery categories (e.g., both involve heat management), they belong in the same file.

**Mandatory self-check after drafting a value model:**

Before writing the file, verify:
1. No L1/L2 names match product or brand names from the portfolio
2. Multiple features could contribute to each L2 component (many-to-many relationships)
3. Removing any single product from the portfolio wouldn't invalidate the layer structure
4. Layer names describe categories of value delivery, not product inventory

For comprehensive structural analysis after writing the file, run the **value_model_review** wizard (`epf_get_wizard { "name": "value_model_review" }`). It interprets programmatic quality checks and guides restructuring if anti-patterns are detected.

**Cautionary example (real incident):** An AI agent created 5 value models for a multi-product company organized by product domain -- layers named "Io Core Battery", "Spiralis Heat Exchange", "Arx Refinery". Every validation check passed, but the models were product catalogs, not value models. The fix: consolidate into a single file organized by value delivery categories ("Energy Transformation", "Thermal Energy Storage", "Heat Exchange & Circulation"), with products appearing at L3 as implementations.

See `guides/VALUE_MODEL_STRUCTURAL_ANTI_PATTERNS.md` for the full guide.
...
7. **Create Feature Definitions:** When Key Results are ready for implementation, create feature definition files in `/_instances/{product}/FIRE/feature_definitions/`. Use template from `/definitions/product/_template/feature_definition_template.yaml` and reference examples in `/definitions/product/`. Feature definitions are outcome-oriented specifications describing WHAT value is delivered to WHOM.
...
- **Value Model Structural Anti-Patterns Guide**: [../guides/VALUE_MODEL_STRUCTURAL_ANTI_PATTERNS.md](../guides/VALUE_MODEL_STRUCTURAL_ANTI_PATTERNS.md) *(embedded, always available)*
