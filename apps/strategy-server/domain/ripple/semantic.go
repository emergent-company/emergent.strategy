...
type SemanticChangeResult struct {
	...
	Score       float64             `json:"score"`       // similarity between old and new (0-1, higher = more similar)
	Description string              `json:"description"`
}

// ClassifyChange estimates how semantically different the new content is from the old.
// Uses Memory search: search for the old content, see if new content's key appears
// in top results. High rank = trivial change. Low/no rank = major change.
func (a *SemanticAnalyzer) ClassifyChange(ctx context.Context, artifactKey string, oldPayload, newPayload json.RawMessage) (*SemanticChangeResult, error) {
	...
	// Search Memory using a truncated version of the old content as query.
	query := truncateForSearch(oldText, 500)
	results, err := a.mem.Search(ctx, memory.SearchRequest{
		Query: query,
		Limit: 20,
	})
	if err != nil {
		slog.WarnContext(ctx, "semantic: search failed for change classification", "key", artifactKey, "error", err)
		return &SemanticChangeResult{
			Class:       ChangeClassMinor,
			Score:       0.5,
			Description: "Memory search unavailable — defaulting to minor.",
		}, nil
	}
	...
	if !found {
		// Artifact not in Memory graph yet — can't classify.
		return &SemanticChangeResult{
			Class:       ChangeClassMinor,
			Score:       0.5,
			Description: "Artifact not yet in semantic graph — defaulting to minor.",
		}, nil
	}

	// The self-score tells us how well the OLD content matches the CURRENT
	// state in Memory. If Memory was recently synced, the current state IS
	// the old content, so self-score should be high. After the new content
	// is ingested, a future classification would compare against the new state.
	//
	// For now, use a simpler heuristic: compare the text lengths and structural
	...
	case ratio > 0.95:
		class = ChangeClassTrivial
		desc = "Very minor text changes — likely typo fixes or formatting."
	case ratio > 0.80:
		class = ChangeClassMinor
		desc = "Moderate text changes — clarifications or detail additions."
	case ratio > 0.60:
		class = ChangeClassSignificant
		desc = "Significant content changes — scope or emphasis shift likely."
	default:
		class = ChangeClassMajor
		desc = "Major content rewrite — direction change or pivot likely."
	}

	// Boost severity for high-impact artifact types.
	_ = selfScore // available for future refinement
