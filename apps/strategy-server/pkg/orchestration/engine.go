...
// engine through. It exists so a second implementation — an ADK-backed engine —
// can be substituted without touching any of the ~30 call sites across
// internal/handler and internal/mcpserver.
//
// This is a pure extraction: every method already existed on *Engine, and
// *Engine satisfies this interface unchanged. Deliberately excluded are
// Subscribe, Unsubscribe, and Publish — the SSE fanout has no external caller
// (the run panel polls GetRun on a timer instead, see
// handler_aim_orchestrator.go), so a second implementation is not obligated to
// replicate ADK-side of that mechanism.
