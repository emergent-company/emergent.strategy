...
test.describe('Onboarding - first login organization & project creation', () => {
    test('creates org then project and reveals app shell badges', async ({ page, consoleErrors, pageErrors }) => {
        ...
        let createdProject: { id: string; name: string; orgId: string } | null = null;
        ...
        await test.step('Prepare auth + clear config + network stubs (empty lists)', async () => {
            ...
            await page.route('**/orgs', async (route) => {
                ...
                if (method === 'GET') {
                    return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(createdOrg ? [createdOrg] : []) });
                }
                if (method === 'POST') {
                    const id = '11111111-1111-4111-8111-111111111111';
                    createdOrg = { id, name: ORG_NAME };
                    return route.fulfill({ status: 201, contentType: 'application/json', body: JSON.stringify(createdOrg) });
                }
            ...
            await page.route('**/projects*', async (route) => {
                const method = route.request().method();
                if (method === 'GET') {
                    return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(createdProject ? [createdProject] : []) });
                }
                if (method === 'POST') {
                    if (!createdOrg) return route.fulfill({ status: 400, contentType: 'application/json', body: JSON.stringify({ error: 'org required' }) });
        ...
        await test.step('See create organization form and submit', async () => {
            ...
            for (const fn of orgInputStrategies) {
                const loc = fn();
                if (await loc.count()) {
                    try { await loc.fill(ORG_NAME); filled = true; break; } catch { /* try next */ }
                }
            }
            if (!filled) throw new Error('Could not locate organization name input');
            await page.getByRole('button', { name: /create organization/i }).click();
            await expect(page.getByRole('heading', { name: /create first project/i })).toBeVisible();
        });
        ...
        await test.step('Fill first project form and submit', async () => {
            ...
            for (const fn of projInputStrategies) {
                const loc = fn();
                if (await loc.count()) {
                    try { await loc.fill(PROJECT_NAME); filled = true; break; } catch { /* try next */ }
                }
            }
        ...
        await test.step('Badges reflect active org & project (gate passed)', async () => {
            ...
            const projectBadge = page.getByText(new RegExp(`Project: ${PROJECT_NAME}`, 'i'));
        ...
        await test.step('Documents area (child content) visible after onboarding', async () => {
            ...
            for (const fn of candidates) {
                const loc = fn();
                try { await loc.waitFor({ state: 'visible', timeout: 5_000 }); found = true; break; } catch { /* try next */ }
            }
            if (!found) throw new Error('Documents content not visible after onboarding');
        });
