...
test.describe.skip('Chat - lifecycle', () => {
    ...
    test.beforeEach(async ({ page }) => {
        ...
        conversationId = await stubChatBackend(page);

        // Log all console output
        page.on('console', msg => {
            const type = msg.type();
            const text = msg.text();
            if (type === 'error' || type === 'warning' || text.includes('chat') || text.includes('[use-chat')) {
    ...
    test('manual send from empty state then rename + delete conversation', async ({ page, consoleErrors, pageErrors }) => {
        ...
        await test.step('Wait for streamed assistant response', async () => {
            ...
            const assistantBubble = page.locator('.chat.chat-start .chat-bubble-primary, .chat.chat-start .chat-bubble');
            await assistantBubble.first().waitFor({ state: 'visible', timeout: 10_000 });

            // Check the actual content - log it even if assertion fails
            const content = await assistantBubble.first().textContent();
            console.log('[test-check] Assistant bubble content:', JSON.stringify(content));

            // Verify localStorage to see if conversation was updated
            const lsData = await page.evaluate(() => {
                const keys = ['spec-server.chat.conversations.22222222-2222-4222-8222-222222222222.33333333-3333-4333-8333-333333333333'];
                const result: any = {};
                for (const k of keys) {
            ...
            console.log('[test-check] LocalStorage conversations:', JSON.stringify(lsData, null, 2));

            // Now assert (will fail but we'll have the debug info)
            await expect(assistantBubble.first()).toContainText('Pong', { timeout: 5_000 });
