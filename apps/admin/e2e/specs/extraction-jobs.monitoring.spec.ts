...
test.describe('Extraction Jobs - Monitoring Endpoint', () => {
    test('should authenticate via OIDC, load page correctly, and not return 500 errors', async ({ page }) => {
        await test.step('Authenticate via Zitadel OIDC login', async () => {
            ...
            await page.goto('http://localhost:5176/admin/extraction-jobs');
            ...
            console.log('Password Next button clicked - checking for password change requirement...');
            ...
            if (currentUrl.includes('password') && (currentUrl.includes('change') || currentUrl.includes('set'))) {
                ...
                throw new Error('Zitadel requires password change - cannot complete automated login');
            }

            console.log('No password change required - proceeding with OAuth flow...');
        ...
        await test.step('Verify authentication state in localStorage', async () => {
            ...
            // Wait for auth state to be saved to localStorage (may take a moment during page load)
            // NOTE: The actual key is 'spec-server-auth', not '__nexus_auth_v1__'
            let authState = null;
            let attempts = 0;
            ...
            while (!authState && attempts < maxAttempts) {
                ...
                if (!authState) {
                    console.log(`Attempt ${attempts}/${maxAttempts}: Auth state not yet in localStorage, waiting...`);
                    await page.waitForTimeout(500);
                }
            }

            expect(authState, 'Auth state MUST exist in localStorage after OIDC login').toBeTruthy();
            expect(authState?.accessToken, 'Access token MUST be present after OIDC login').toBeTruthy();

            const tokenPreview = authState?.accessToken?.substring(0, 30);
            console.log('✅ Authentication verified - token:', tokenPreview + '...');
            console.log('✅ Token expires:', authState?.expiresAt ? new Date(authState.expiresAt).toISOString() : 'never');
            console.log('✅ User sub:', authState?.user?.sub);
        });

        const apiRequests: Array<{ url: string; status: number; method: string; body?: any }> = [];
        ...
        await test.step('Setup network monitoring', async () => {
            ...
            page.on('response', async (response) => {
                ...
                if (url.includes('/api/') || url.includes('localhost:5176')) {
                    ...
                    const statusEmoji = status >= 200 && status < 300 ? 'OK' : status >= 400 ? 'ERR' : 'WARN';
        ...
        await test.step('Navigate to extraction jobs page', async () => {
            ...
            const isOnLoginPage = currentUrl.includes('/login') || currentUrl.includes('localhost:8200');
            if (isOnLoginPage) {
                console.error('Redirected to login page:', currentUrl);
                throw new Error('Unexpected redirect to login - authentication may have failed');
            }
        ...
        await test.step('Verify page rendered with correct elements', async () => {
            ...
            const pageContainer = page.locator('[data-testid="page-extraction-jobs"]');
            await expect(pageContainer, 'Page container with data-testid="page-extraction-jobs" MUST be visible').toBeVisible({ timeout: 15000 });
            console.log('Page container found with correct data-testid');

            const pageTitle = page.getByRole('heading', { name: /extraction jobs/i, level: 1 });
            await expect(pageTitle, 'Page title "Extraction Jobs" MUST be visible').toBeVisible({ timeout: 5000 });
        ...
        await test.step('Capture screenshot for verification', async () => {
            ...
            await page.screenshot({ path: 'test-results/extraction-jobs-monitoring-verified.png', fullPage: true });
            console.log('Screenshot saved: test-results/extraction-jobs-monitoring-verified.png');
        });
        ...
        await test.step('Verify no 500 errors from monitoring endpoint', async () => {
            if (errorRequests.length > 0) {
                ...
                throw new Error(`Found ${errorRequests.length} 500 error(s). See logs above for details.`);
            }
        ...
        await test.step('Report any client errors (4xx)', async () => {
            const clientErrors = apiRequests.filter(r => r.status >= 400 && r.status < 500);
            ...
            if (clientErrors.length > 0) {
                ...
                console.log('Note: Client errors are informational and do not fail the test');
            } else {
