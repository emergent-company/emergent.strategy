...
test.describe('SetupGuard - behavior verification', () => {
    test('allows access when user has org and project', async ({ page, cleanupComplete }) => {
        ...
        await test.step('Create test org and project', async () => {
            ...
            const projectId = await createTestProject(page, orgId, 'Setup Guard Test Project');
    ...
    test('redirects to setup when user has no org/project', async ({ page, cleanupComplete }) => {
        console.log('[TEST] Cleanup complete:', cleanupComplete);
        ...
        await test.step('Verify redirect to setup', async () => {
            ...
            const setupForm = page.getByTestId('setup-org-form').or(page.getByTestId('setup-project-form'));
