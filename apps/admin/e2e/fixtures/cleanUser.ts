...
async function loginTestUser(page: import('@playwright/test').Page): Promise<void> {
    ...
    console.log('[FIXTURE] cleanUser: Auth data:', JSON.stringify(authData, null, 2));
}
...
export const test = consoleGateTest.extend<{ cleanupComplete: boolean }>({
    cleanupComplete: async ({ page }, use) => {
        ...
        try {
            ...
            console.warn('[FIXTURE] cleanUser: Pre-test cleanup failed (may be first run):', err.message);
        ...
        try {
            ...
            const err = error as Error;
            console.error('[FIXTURE] cleanUser: Post-test cleanup failed:', err.message);
