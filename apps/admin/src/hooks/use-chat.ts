...
function dedupeConversations(list: Conversation[]): Conversation[] {
    ...
    for (const c of list) {
        ...
        if (!ex) {
            ...
            const pickMoreMsgs = (ex.messages?.length || 0) < (c.messages?.length || 0);
    ...
    for (const [, group] of buckets) {
        ...
        const target = servers.sort((a, b) => (b.createdAt || "").localeCompare(a.createdAt || ""))[0];
        ...
        for (const t of temps) {
            ...
            if (Number.isFinite(tTime) && Number.isFinite(targetTime) && Math.abs(tTime - targetTime) < 60_000) {
    ...
    const temps = arr.filter((c) => /^c_/.test(c.id) && (c.messages?.length || 0) === 0);
    if (temps.length > 1) {
        const keep = temps.sort((a, b) => (b.createdAt || "").localeCompare(a.createdAt || ""))[0];
        arr = arr.filter((c) => !(/^c_/.test(c.id) && (c.messages?.length || 0) === 0 && c.id !== keep.id));
    }
...
export function useChat(opts: UseChatOptions = {}) {
    ...
    const refreshConversationsFromServer = useCallback(async () => {
        try {
            ...
            if (streamingRef.current && activeIdRef.current && /^c_/.test(activeIdRef.current)) {
                const now = Date.now();
    ...
    const createConversation = useCallback((title?: string, init?: { isPrivate?: boolean; ownerUserId?: string }): Conversation => {
        ...
        if (active && (active.messages?.length || 0) === 0) {
            const reused: Conversation = { ...active, title: title || active.title || "New Conversation", isPrivate: init?.isPrivate ?? !!active.isPrivate };
    ...
    const appendMessage = useCallback((conversationId: string, role: Role, content: string): Message => {
    ...
    const updateMessage = useCallback((conversationId: string, messageId: string, patch: Partial<Message>) => {
        setConversationsPersisted((prev) => {
            ...
            const newConv: Conversation = {
                ...conv,
                updatedAt: nowIso(),
                messages: [...conv.messages.slice(0, mi), msg, ...conv.messages.slice(mi + 1)],
            };
    ...
    const send = useCallback(
        async (input: { message: string; conversationId?: string; topK?: number; documentIds?: string[]; isPrivate?: boolean }) => {
            ...
            const latestConvs = conversationsRef.current;
            let conv = latestConvs.find((c) => c.id === input.conversationId) || latestConvs.find((c) => c.id === activeIdRef.current!) || activeConversation || null;
            if (conv && conv.messages.length === 0 && typeof input.isPrivate === 'boolean') {
            ...
            const history = (conversationsRef.current.find((c) => c.id === workingConvId)?.messages || []).slice(-8).map((m) => ({ role: m.role, content: m.content }));
                ...
                while (true) {
                    ...
                    for (const chunk of parts) {
                        ...
                        if (!serverConvId && evt.type === "meta" && evt.conversationId) {
                            ...
                            if (/^c_/.test(currentConvId) && serverConvId && currentConvId !== serverConvId) {
                                setConversationsPersisted((prev) => {
                                    ...
                                    const existingIdx = prev.findIndex((c) => c.id === serverConvId);
                                    if (existingIdx >= 0) {
                                        ...
                                        const merged: Conversation = {
                                            ...
                                            updatedAt: nowIso(),
                                            isPrivate: typeof target.isPrivate === 'boolean' ? target.isPrivate : temp.isPrivate,
                                            messages: (target.messages?.length || 0) >= (temp.messages?.length || 0) ? target.messages : temp.messages,
                                        };
                                        const next = [...prev];
                                    ...
                                    const updated: Conversation = { ...prev[tempIdx], id: serverConvId! };
                                    const next = [...prev];
                        ...
                        if (evt.type === "token" && evt.token) {
                            ...
                            setConversationsPersisted((prev) => {
                                ...
                                const mi = convPrev.messages.findIndex((m) => m.id === assistant.id);
                                if (mi < 0) return prev;
                                const msg = { ...convPrev.messages[mi], content: convPrev.messages[mi].content + evt.token } as Message;
                                const newConv: Conversation = {
                                    ...convPrev,
                                    updatedAt: nowIso(),
                                    messages: [...convPrev.messages.slice(0, mi), msg, ...convPrev.messages.slice(mi + 1)],
                                };
                                return [...prev.slice(0, ci), newConv, ...prev.slice(ci + 1)];
                            });
    ...
    return {
        conversations,
        sharedConversations: useMemo(() => conversations.filter((c) => !c.isPrivate), [conversations]),
        privateConversations: useMemo(() => conversations.filter((c) => c.isPrivate), [conversations]),
        activeConversation,
        setActive,
        createConversation,
        deleteConversation,
