...
export const Footer: React.FC<FooterProps> = ({ className, yearOverride, statusMessage }) => {
    const year = yearOverride ?? new Date().getFullYear()
    return (
        <div
            role="contentinfo"
            className={`flex flex-wrap justify-between items-center gap-3 px-6 py-3 w-full ${className ?? ''}`}
        ...
        >
            <div className="flex items-center gap-2.5 bg-base-100 hover:bg-base-200 shadow-xs px-2.5 py-1 border border-base-300 rounded-full transition-colors cursor-pointer">
                <span className="status status-success" aria-label="System Status: OK" />
                <p className="text-sm text-base-content/80">
                    {statusMessage || 'System running smoothly'}
                </p>
            </div>
            <span className="text-sm text-base-content/80">© {year} Nexus. All rights reserved</span>
        </div>
    )
}
