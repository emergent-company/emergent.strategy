...
import type { DiscoveryJob, TypeCandidate, Relationship } from './DiscoveryWizard';
...
export function Step5_Complete({
    ...
    const handleSavePack = async () => {
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : 'Failed to save template pack');
        } finally {
            setSaving(false);
        }
    };
    ...
    return (
        <div className="flex flex-col justify-center items-center space-y-6 py-8 text-center">
            {/* Success Icon */}
            <div className={`flex justify-center items-center rounded-full w-24 h-24 ${saved ? 'bg-success/10' : 'bg-info/10'
                }`}>
                <Icon
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <span className="font-medium">
                                    {packConfig.mode === 'create'
                                        ? 'Create New Pack'
                                        : `Extend Pack (${packConfig.existingPackId?.slice(0, 8)}...)`}
                                </span>
                            </div>
                            {saved && savedPackId && (
                                <div className="flex justify-between items-center">
                ...
                <div className="w-full max-w-md">
                    <details className="collapse collapse-arrow bg-base-100 border border-base-300 rounded-lg">
                        <summary className="collapse-title font-medium text-sm">
                            View Discovered Types ({includedTypeCount} included / {totalTypeCount} total)
                        </summary>
                            ...
                            <ul className="space-y-2 text-sm">
                                {jobData.discovered_types.map((type, idx) => {
                                    ...
                                    return (
                                        <li
                                            key={idx}
                                            className={`flex justify-between items-center ${isIncluded ? '' : 'opacity-40 line-through'
                                                }`}
                                        >
                ...
                {saved && (
                    <div className="bg-success/10 p-4 border border-success/30 rounded-lg">
                        ...
                        <p className="mt-2 text-sm text-base-content/70">
                            Your template pack is now available in your project's template library.
                        </p>
                    </div>
                )}
                <button
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div className="text-sm text-left">
                            ...
                            <ul className="space-y-1 mt-2 pl-4 text-base-content/70 list-disc">
                                ...
                                <li>The pack will be saved to: <strong>{packConfig?.mode === 'create' ? 'New Pack' : 'Existing Pack'}</strong></li>
                            </ul>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-success/5 p-4 border border-success/30 rounded-lg w-full max-w-md">
                    <div className="flex items-start gap-3">
                        ...
                        <div className="text-sm text-left">
                            ...
                            <ul className="space-y-1 mt-2 pl-4 text-base-content/70 list-disc">
                                <li>Install the pack in your project settings to use the types</li>
                                <li>Start creating instances of your discovered entity types</li>
                                <li>Run additional discoveries to expand your knowledge graph</li>
                            </ul>
                        </div>
                    </div>
                </div>
