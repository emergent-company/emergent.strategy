...
export function Step2_Analyzing({ jobId, onComplete, onCancel }: Step2Props) {
    ...
    useEffect(() => {
        const pollJob = async () => {
            try {
                const jobData = await fetchJson<DiscoveryJob>(`${apiBase}/api/discovery-jobs/${jobId}`);
                setJob(jobData);
                setError(null);
    ...
    if (!job) {
        return (
            <div className="flex flex-col justify-center items-center py-12">
                ...
                <p className="mt-4 text-sm text-base-content/70">Loading job status...</p>
            </div>
        );
    }

    const progress = job.progress || { current_step: 0, total_steps: 1, message: 'Initializing...' };
        ...
        <div className="space-y-8 py-6">
            ...
            <div className="flex flex-col justify-center items-center text-center">
                <div className="flex justify-center items-center bg-primary/10 mb-4 rounded-full w-20 h-20">
                    <Icon icon={getStatusIcon()} className="size-10 text-primary animate-pulse" />
                </div>
            ...
            <div className="gap-4 grid grid-cols-2">
                <div className="p-4 border border-base-300 rounded-lg text-center">
                    ...
                    <div className="mt-1 text-sm text-base-content/70">Types Discovered</div>
                </div>
                <div className="p-4 border border-base-300 rounded-lg text-center">
                    <div className="font-bold text-secondary text-3xl">
                        {job.discovered_relationships?.length || 0}
                    </div>
                    <div className="mt-1 text-sm text-base-content/70">Relationships Found</div>
                </div>
            </div>
