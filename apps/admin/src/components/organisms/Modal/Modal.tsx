...
export interface ModalProps {
    ...
    onOpenChange: (open: boolean, reason: 'backdrop' | 'close-button' | 'escape') => void
...
export const Modal: React.FC<ModalProps> = ({
    ...
    useEffect(() => {
        ...
        if (open && !dialog.open) {
            if (canShow) {
                try { (dialog as any).showModal() } catch { dialog.setAttribute('open', '') }
            } else {
                dialog.setAttribute('open', '')
            }
        } else if (!open && dialog.open) {
            if (canClose) {
                try { (dialog as any).close() } catch { dialog.removeAttribute('open') }
            } else {
    ...
    const handleCloseButton = useCallback(() => onOpenChange(false, 'close-button'), [onOpenChange])
    ...
    return createPortal(
        <dialog
            ...
            aria-label={ariaLabel}
        >
            <div className={['modal-box', sizeClassName, className].filter(Boolean).join(' ')}>
                ...
                {description && (
                    <p id={descId} className='mt-[-0.5rem] mb-4 text-sm text-base-content/70'>
                        {description}
                    </p>
                )}
                ...
                {actions && actions.length > 0 && (
                    <div className='modal-action'>
                        {actions.map((a, idx) => {
                            const btnClasses = [
                                'btn',
                                a.variant && a.variant !== 'outline' && a.variant !== 'ghost' ? `btn-${a.variant}` : '',
                            ...
                            return (
                                ...
                                >
                                    {a.icon && <span className={`iconify ${a.icon} size-4`} aria-hidden='true' />}
                                    {a.label}
                                </button>
                            )
                        })}
            ...
            <form method='dialog' className='modal-backdrop' onClick={handleBackdropClick} aria-label='Close modal backdrop'>
