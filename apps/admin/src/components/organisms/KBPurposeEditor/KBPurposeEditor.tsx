...
export const KBPurposeEditor: React.FC<KBPurposeEditorProps> = ({ projectId }) => {
    ...
    useEffect(() => {
        const loadPurpose = async () => {
            ...
            setLoading(true);
            try {
                const project = await fetchJson<{ kb_purpose?: string }>(`/api/projects/${projectId}`);
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex justify-between items-start">
                <div>
                    ...
                    <p className="mt-1 text-sm text-base-content/70">
                        Describe what this knowledge base is about. This helps the AI discover relevant entity types.
                    </p>
                </div>
                <div className="flex items-center gap-2">
            ...
            {/* Editor / Preview */}
            <div className="gap-4 grid grid-cols-1" style={{ gridTemplateColumns: showPreview ? '1fr 1fr' : '1fr' }}>
                {/* Markdown Editor */}
                <div className="space-y-2">
                    <label className="label">
                        <span className="label-text">Purpose (Markdown supported)</span>
                        <span className={`label-text-alt ${isValid ? 'text-success' : 'text-warning'}`}>
                            {charCount} / 1000 chars
                        </span>
                    </label>
                    <textarea
                        className={`textarea textarea-bordered w-full h-64 font-mono text-sm ${!isValid && charCount > 0 ? 'textarea-warning' : ''
                            }`}
                        placeholder={`Example:
...
The AI should discover entities related to customer management, support operations, and product feedback.`}
                    ...
                    <div className="text-xs text-base-content/60">
                        💡 Tip: Be specific about domains, entities, and relationships you expect. Use bullet points for clarity.
                    </div>
                </div>
            ...
            <div className="space-y-2 bg-base-200 p-4 rounded-lg">
                ...
                <ul className="space-y-1 text-sm text-base-content/70 list-disc list-inside">
                    <li>The AI analyzes your documents alongside this purpose description</li>
                    <li>Types and relationships are discovered based on recurring patterns that match your purpose</li>
                    <li>More specific purpose = more accurate entity discovery</li>
                    <li>You can review and edit all discovered types before installation</li>
                </ul>
            </div>
        </div>
    );
