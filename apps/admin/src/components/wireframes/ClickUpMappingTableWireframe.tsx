...
export const ClickUpMappingTableWireframe: React.FC<ClickUpMappingTableWireframeProps> = ({ state = 'default', rows = 5 }) => {
    if (state === 'no-space') {
        return (
            <div className="flex flex-col justify-center items-center gap-4 bg-base-100 p-12 border border-base-300 rounded text-center">
                ...
                <div className="space-y-2 text-sm text-base-content/70">
                    ...
                    <p className="text-base-content/50">Connect a space to configure mapping.</p>
                </div>
                ...
                <table className="table table-sm">
                    ...
                    <tbody>
                        {Array.from({ length: rows }).map((_, i) => (
                            <tr key={i} className={state === 'syncing' ? 'animate-pulse opacity-70' : ''}>
                                <td><div className="bg-base-300 rounded w-32 h-4" /></td>
