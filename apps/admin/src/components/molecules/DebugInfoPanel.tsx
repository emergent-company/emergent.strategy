...
export function DebugInfoPanel({ debugInfo }: DebugInfoPanelProps) {
    ...
    const timeline = Array.isArray(data.timeline) ? (data.timeline as TimelineEvent[]) : [];
    ...
    const formatStep = (step: string) => step
        .split(/[_-]/)
        .filter(Boolean)
                ...
                <div>
                    ...
                    <div className="gap-3 grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6">
                        {Object.entries(entityOutcomes).map(([key, value]) => (
                            <div key={key} className="bg-base-200 p-3 rounded-box text-center">
                                <div className="text-xs text-base-content/60 uppercase">{formatStep(key)}</div>
                                <div className="font-semibold text-base-content text-lg">{value}</div>
                            </div>
                        ))}
                    </div>
                </div>
