...
import { useNotificationStats, useNotifications, useNotificationMutations } from '@/hooks/useNotifications';
...
export const NotificationBell: React.FC<NotificationBellProps> = ({
    ...
    const { data: notifications, isLoading, refetch: refetchNotifications } = useNotifications('all', ALL_NOTIFICATIONS_FILTER);
    ...
    const handleActionClick = useCallback((e: React.MouseEvent, action: NotificationAction, notification: Notification) => {
        e.stopPropagation();
                    ...
                    <div className="flex-1 overflow-y-auto">
                        {isLoading ? (
                            ...
                            <div className="flex flex-col justify-center items-center px-4 py-12">
                                <Icon icon="lucide--bell-off" className="mb-3 size-12 text-base-content/30" />
                            ...
                            <div className="divide-y divide-base-300/50">
                                {recentNotifications.map((notification) => {
                                    ...
                                    const hasDismissed = notification.dismissed || notification.clearedAt;

                                    return (
                                        <div
                                            key={notification.id}
                                            className={`p-4 hover:bg-base-200/50 cursor-pointer transition-colors ${isUnread ? 'bg-base-200/20' : ''
                                            ...
                                            <div className="flex justify-between items-start gap-2 mb-2">
                                                <div className="flex flex-1 items-start gap-2 min-w-0">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className={`text-sm font-medium mb-1 ${isUnread ? 'text-base-content' : 'text-base-content/80'}`}>
                                                            {notification.severity && (
                                                                <Icon
                                                                    icon={
                                                                        notification.severity === 'error' ? 'lucide--circle-x' :
                                                                            notification.severity === 'warning' ? 'lucide--triangle-alert' :
                                                                                notification.severity === 'success' ? 'lucide--circle-check' :
                                                                                    'lucide--info'
                                                                    }
                                                                    className={`inline-block size-4 mr-1 ${getSeverityColor(notification.severity)}`}
                                                                />
                                                            )}
                                                            {notification.title}
                                                        </div>
                                            ...
                                            {notification.actions && notification.actions.length > 0 && (
                                                <div className="flex flex-wrap gap-2 mt-3">
                                                    {notification.actions.map((action, idx) => (
                                                        <button
                                                            key={idx}
                                                            className={`btn btn-xs ${getActionStyle(action.style)}`}
                                                            onClick={(e) => handleActionClick(e, action, notification)}
                                                        >
                                                            {action.label}
                                                        </button>
                                                    ))}
