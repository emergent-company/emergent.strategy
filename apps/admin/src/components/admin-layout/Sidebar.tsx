...
export const Sidebar = ({ children }: ISidebarProps) => {
    ...
    // Collect items from children <SidebarSection /> components
    const arrayChildren = useMemo(() => (Array.isArray(children) ? children : [children]), [children]);
    const primaryItems: ISidebarMenuItem[] = useMemo(() => {
            ...
            if (section && section.type === SidebarSection) {
                ...
                sectionChildren.forEach((itemNode: any) => {
                    if (itemNode && itemNode.type === SidebarMenuItem) {
                        const { activated: _a, onToggleActivated: _b, ...rest } = itemNode.props || {};
    ...
    return (
        <>
            ...
            <div id="layout-sidebar" className="flex flex-col sidebar-menu" data-theme={calculatedSidebarTheme}>
                ...
                {/* Optional: Project dropdown can now be composed by callers as a child before sections */}
