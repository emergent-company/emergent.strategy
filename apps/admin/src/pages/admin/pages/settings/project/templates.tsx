...
export default function ProjectTemplatesSettingsPage() {
  ...
  const loadCompiledTypes = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to load compiled types'
      );
    }
  };
  ...
  const handleInstall = async (packId: string) => {
    ...
    try {
      await fetchJson(
        `${apiBase}/api/template-packs/projects/${config.activeProjectId}/assign`,
        {
          method: 'POST',
          body: {
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to install template pack'
      );
    } finally {
      setInstalling(null);
    }
  ...
  const handleUninstall = async (assignmentId: string) => {
    ...
    if (
      !window.confirm(
        "Are you sure you want to uninstall this template pack? Existing objects will not be deleted but won't appear in type filters."
      )
    ) {
      return;
    }
    ...
    try {
      await fetchJson(
        `${apiBase}/api/template-packs/projects/${config.activeProjectId}/assignments/${assignmentId}`,
        {
          method: 'DELETE',
        }
      );
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to uninstall template pack'
      );
    }
  };
  ...
  const handleToggleActive = async (
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to update template pack'
      );
    }
  };
  ...
  const handleDelete = async (packId: string, packName: string) => {
    if (
      !window.confirm(
        `Are you sure you want to permanently delete "${packName}"? This action cannot be undone.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to delete template pack'
      );
    }
  };
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="mt-1 text-base-content/70">
            Template packs define the types of structured objects you can create
              ...
              <div className="bg-base-200 card">
                <div className="py-12 text-center card-body">
                  ...
                  <p className="text-base-content/70">
                    No template packs installed yet
                  </p>
                  <p className="text-sm text-base-content/60">
                    Browse available packs below to get started
                  </p>
                </div>
              </div>
            ) : (
              <div className="space-y-3">
                {installedPacks.map((pack, index) => (
                  <div
                    ...
                    className="bg-base-100 border border-base-300 card"
                  >
                      ...
                      <div className="flex justify-between items-start gap-4">
                        ...
                        <div className="flex gap-2">
                          ...
                          >
                            <Icon
                              icon={
                                pack.active ? 'lucide--pause' : 'lucide--play'
                              }
                              className="size-4"
                            />
                ...
                <div className="space-y-3">
                  {availablePacks
                    ...
                    .map((pack) => (
                      <div
                        key={pack.id}
                        className="bg-base-100 border border-base-300 hover:border-info/30 transition-colors card"
                      >
                        <div className="card-body">
                          <div className="flex items-start gap-3">
                            <Icon
                              icon="lucide--package"
                              className="mt-1 size-6 text-info"
                            />
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2 mt-3 text-sm text-base-content/60">
                                <span>
                                  {pack.object_types?.length || 0} object types
                                </span>
                                <span>•</span>
                                <span>
                                  {pack.relationship_count || 0} relationships
                                </span>
                                {pack.author && (
                                  <>
                                    <span>•</span>
                ...
                <div className="space-y-3">
                  {availablePacks
                    ...
                    .map((pack) => (
                      <div
                        key={pack.id}
                        className="bg-base-100 border border-base-300 hover:border-primary/30 transition-colors card"
                      >
                        <div className="card-body">
                          <div className="flex items-start gap-3">
                            <Icon
                ...
                <div className="bg-base-200 card">
                  <div className="py-8 text-center card-body">
                    <Icon
                      ...
                      className="opacity-50 mx-auto mb-2 size-10"
                    />
                    <p className="text-base-content/70">
                      All available template packs are installed
                    </p>
                  </div>
                </div>
              )}
        ...
        <dialog className="modal modal-open" data-testid="compiled-types-modal">
          <div className="w-11/12 max-w-6xl modal-box">
            ...
            <p className="mb-4 text-sm text-base-content/70">
              All object types from installed template packs, merged and ready
              for extraction
            </p>
            ...
            {Object.keys(compiledTypes).length === 0 ? (
              <div className="bg-base-200 py-8 text-center card">
                ...
                />
                <p className="text-base-content/70">
                  No object types installed
                </p>
              </div>
            ) : (
              <>
                <div className="divider">
                  Object Types ({Object.keys(compiledTypes).length})
                </div>
