...
export default function IntegrationsPage() {
  ...
  const [configuredInstance, setConfiguredInstance] =
  ...
  const handleConfigure = (integration: AvailableIntegration) => {
    ...
    const existing = configuredIntegrations.find(
  ...
  return (
    <OrgAndProjectGate>
      <PageContainer maxWidth="7xl" testId="page-integrations">
        {/* Header */}
        <div className="mb-6">
          <h1 className="font-bold text-2xl">Integrations</h1>
          <p className="mt-1 text-base-content/70">
            Connect external tools and services to sync data with your knowledge
            base
          </p>
        </div>
            ...
            {availableIntegrations.map((integration) => {
              ...
              return (
                <IntegrationCard
                  ...
                  onToggle={
                    configured ? () => handleToggle(configured) : undefined
                  }
                  onDelete={
                    configured ? () => handleDelete(configured) : undefined
                  }
                  onSync={
                    configured ? () => handleSync(integration) : undefined
                  }
                  data-testid={
                    integration.name === 'clickup'
                      ? 'integration-card-clickup'
                      : undefined
                  }
                />
              );
