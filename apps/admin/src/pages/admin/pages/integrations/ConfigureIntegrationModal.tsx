...
import type { AvailableIntegration, Integration, IntegrationsClient } from "@/api/integrations";
...
export function ConfigureIntegrationModal({
    ...
    useEffect(() => {
        if (configuredInstance) {
            ...
            Object.entries(integration.optionalSettings || {}).forEach(([key, value]) => {
                defaults[key] = value;
            });
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (missingRequired.length > 0) {
                throw new Error(`Missing required settings: ${missingRequired.join(', ')}`);
            }
    ...
    const allSettings = [
        ...
        ...Object.keys(integration.optionalSettings || {}).map(key => ({ key, required: false })),
    ];
            ...
            <div className="max-w-2xl modal-box">
                ...
                <div className="flex justify-between items-center mb-6">
                    <h3 className="font-bold text-lg">
                        {configuredInstance ? 'Configure' : 'Connect'} {integration.displayName}
                    </h3>
                    <button
                ...
                {/* Test Result */}
                {testResult && (
                    <div className={`alert ${testResult.success ? 'alert-success' : 'alert-warning'} mb-4`}>
                        <Icon
                ...
                <div className="space-y-4">
                    {allSettings.map(({ key, required }) => {
                        ...
                        const isBoolean = typeof integration.optionalSettings?.[key] === 'boolean';
                        const isNumber = typeof integration.optionalSettings?.[key] === 'number';
                                ...
                                <div key={key} className="form-control">
                                    <label className="justify-start gap-4 cursor-pointer label">
                                        ...
                                        <div>
                                            ...
                                            {!required && (
                                                <span className="opacity-60 ml-2 label-text-alt">(Optional)</span>
                                            )}
                                        </div>
                                    </label>
                                </div>
                            ...
                            <div key={key} className="form-control">
                                ...
                                <input
                                    ...
                                    onChange={e =>
                                        handleSettingChange(
                                            key,
                                            isNumber ? parseInt(e.target.value, 10) || 0 : e.target.value
                                        )
                                    }
                ...
                <div className="modal-action">
                    ...
                    {configuredInstance && (
                        <button
                            ...
                            disabled={loading || testing}
                        >
                            {testing && <span className="loading loading-spinner loading-sm" />}
