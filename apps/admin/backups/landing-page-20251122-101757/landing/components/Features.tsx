interface FeatureItem {
  ...
  title: string;
  description: string;
  decorativeSvg: 'brain' | 'lightning' | 'bell' | 'hand' | 'chart' | 'arrow';
}

const features: FeatureItem[] = [
  {
    icon: 'lucide--brain',
    title: 'Understands Your Domain Automatically',
    description:
      'No configuration needed. Emergent learns from your documents, recognizing patterns and connections that matter to your work',
    decorativeSvg: 'brain',
  },
  {
    icon: 'lucide--network',
    title: 'Connects the Dots for You',
    description:
      "See relationships across projects, documents, and conversations that you'd never spot manually",
    decorativeSvg: 'lightning',
  },
  {
    icon: 'lucide--refresh-cw',
    title: 'Evolves as You Work',
    description:
      "Your knowledge base grows smarter with every document, adapting to your organization's unique needs",
    decorativeSvg: 'bell',
  },
  {
    icon: 'lucide--sparkles',
    title: 'Surfaces Insights Before You Ask',
    description:
      'Proactive intelligence that anticipates what you need—delivering relevant context at the perfect moment',
    decorativeSvg: 'hand',
  },
  {
    icon: 'lucide--users',
    title: 'Keeps Your Team Aligned',
    description:
      'Everyone works from the same up-to-date understanding, reducing miscommunication and duplicate effort',
    decorativeSvg: 'chart',
  },
  {
    icon: 'lucide--trending-up',
    title: 'Grows as Your Organization Grows',
    description:
      'From startup to enterprise, Emergent scales effortlessly with your team and knowledge base',
    decorativeSvg: 'arrow',
  },
];

const DecorativeSvg = ({ type }: { type: string }) => {
  if (type === 'lightning') {
    return (
  ...
  if (type === 'bell') {
    return (
      <div className="absolute end-3.5 top-3.5">
        <svg
          className="group-hover:animate-vibrate text-base-content/5 h-10 stroke-[1.5px] transition-all group-hover:h-11 group-hover:text-primary"
          viewBox="0 0 24 24"
        >
          <path
            fill="none"
    ...
    return (
      <div className="absolute end-4 bottom-3">
        <svg
          ...
          viewBox="0 0 24 24"
        >
          <path fill="none" stroke="currentColor" d="m18 8l4 4l-4 4M2 12h20" />
        </svg>
      </div>
    );
  }

  // Default brain icon with complex decorative paths
  return (
    ...
    >
      ...
      <div className="mt-8 grid gap-6 md:mt-12 md:grid-cols-2 lg:mt-16 xl:grid-cols-3 2xl:mt-24">
        {features.map((feature, index) => {
          return (
            <div
              className="bg-base-100 card group relative cursor-pointer overflow-hidden shadow transition-all hover:shadow-lg"
              key={index}
            >
              <div className="p-5">
                <div className="bg-base-200 inline-flex rounded-full p-2.5">
                  <span className={`iconify ${feature.icon} size-6`}></span>
                </div>
